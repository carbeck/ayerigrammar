...
of a CC cluster in the sample set. Furthermore, the only unvoiced single coda
...
noun compounds mostly come in head-initial order otherwise. Possibly, what
happened at the morpheme borders is that \rayr{tilaanF}{tilān} underwent
metathesis to *\rayr{ItFlaan}{*itlān} to match the rhyme of
\rayr{pNisF}{pangis}. *\rayr{pNisitFlaanF}{*pangisitlān} then underwent
irregular haplology (and shortening of the nominalizing suffix) to
\rayr{pNitFlnF}{pangitlan}.}
...
\rayr{/ye}{-ye}, the locative marker \rayr{/y}{\mbox{-ya}}, the dative and
participle marker \rayr{/ymF}{-yam}, as well as third-person animate pronoun
agreement suffixes, and the various first-person and third-person animate
pronominal clitics. \autoref{tab:verbsyll} shows exemplarily how verbs
resyllabify when suffixes are attached. Even though single-segment onsets are
strongly preferred, Cr, Cw, and especially C(C)j seem to be generally
...
\ex\label{ex:engvarstress}%
	English \citep[from][552]{halle1998}:\medskip\\
...
their effects on disyllabic words as the most common type. As shown in
(\ref{ex:firstlh}), heavy syllables in ultimate positions attract stress while
quasi-regular results are produced when they are in penultimate position and
the ultimate syllable is not heavy. Unfortunately, there are no disyllabic
examples for the feature sets \textsc{[+\,long, \mbox{–\,diph,} +\,ŋ]} and
\textsc{[+\,long, +\,diph, –\,ŋ]} in the first syllable (syllables of the type
/C(C(j))Vːŋ/ or /C(C(j))Vɪŋ/). If there were, they would group with
(\ref{ex:firstheavy}).
...
possible combinations between \textsc{[–\,long, \mbox{–\,diph,} +\,ŋ]} and the
\textsc{[+\,long]} row, as in (\ref{ex:-l-d+ŋ}).
...
followed by two long ones, and that of three long syllables in a row remain
...
necessary for it to be stressed. The chain of applying the stress rules is
illustrated in (\ref{ex:depangcati}).

\begin{figure}[h]
\ex\label{ex:depangcati}
