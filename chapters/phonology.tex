...
word. It is important to note here that phonemes which consist of more than 
segment -- affricates, diphthongs, and long vowels -- have been counted as only 
one of C (consonant) or V (vowel). The next subsections will elaborate on which 
sounds the Cs and Vs correspond to.

\begin{table}[hp]\centering
\caption[Relative frequency of syllable types per word]{Relative frequency of syllable types per word (n\,=\,5499)}
\begin{tabu} to \textwidth{H X[c] S[c] X[c] S[c] X[c] S[c] X[c] S[c]}
\tableheaderfont\toprule
Type
