...
the loudness of syllables. Stress is also assigned dynamically: it is not 
lexical, as in English, but assigned based on the shape of syllables in words:
...
whether a prefix or a suffix is added. German may be an even more illustrative 
example as it offers more ways to add affixes than English without the need to 
recurse to loanwords:

\ex
	\rc{German}%
	reden /ˈreːdən/ `talk'\\
	redete /ˈreːdətə/ `talked'\\
	geredet /gəˈreːdət/ `talked.\Ptcp{}'\\
	überredete /ˌyːbərˈreːdətə/ `persuaded' (over.talk-\Pst{}-1/3\Sg{})
\xe

In all these words, stress stays on (the first syllable of) the word stem, as 
is typical of modern Germanic languages, no matter whether one or several 
affixes are added. In Ayeri, on the other hand, stress moves around. To 
demonstrate, the complete declension paradigm for \xayr{niv}{niva}{eye} will be 
given here:
