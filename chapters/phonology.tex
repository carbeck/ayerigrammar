...
respectively, except if a homorganic nasal /n/ or /ŋ/ is preceding: for
...
`\Rel{}-\Loc{}-\Loc{}' that I introduced here [\dots]. Since this feature does not
...
% \footnotetext{I have gone years without dictionary entries for /uː/, but it
% has always seemed slightly odd to me to lack a vowel in that position when
% all other vowels can be long. Therefore, \xayr{bbuu\_anF}{babūan}{barbarian}
% and its adjective \xayr{bbuu}{babū}{barbarian (adj.)} were coined as
% \rayr{pFrMkye}{prankaye}---things `that you put in specifically to make
% things fit', another new coining this decision resulted in.\label{fn:ū}}
...
vowel is appended. To avoid a hiatus, a glide /j/ may be inserted\index{morphophonology}, so both of the
...
elaborate on which sounds the Cs and Vs correspond to. Moreover, medial
syllables have not been further distinguished by position in the word for the
sake of this analysis, so anything between the second and the fifth medial
syllable is treated the same. It would furthermore be possible to calculate the
frequencies of one syllable type following the other, however, no such
calculations have been carried out here.
...
places. Instead of /eːɪ/, there is an attestation of /uː/,
% (see \autoref{fn:ū}),
...
explanation.} in \xayr{/NsF}{\mbox{-ngas}}{almost}, a quantifier suffix that has
...
0.00 \dots{} 0.09 & g.t, h.t, h.v, k.s, l.n, lv.n, m.bj, m.d, m.ʤ, m.l, m.n,
	m.pr, m.r, n.dv, n.g, % \footnotemark{}
	n.h, n.w, ŋ.ʤj, ŋ.kw, ŋ.m, ŋ.n, ŋ.rj,
...
of it. In fact, English\index{English}, which also has phonemic stress in pairs
such as \fw{record} /ˈɹɛkəɹd/ (noun) and /ɹɪˈkɔɹd/ (verb), does a similar
thing, as (\ref{ex:engvarstress}) shows. Ayeri has no such minimal pairs.

\begin{figure}[h]
\ex\label{ex:engvarstress}%
	English \citep[adapted from][552]{halle1998}:\medskip\\
	\textit{admire} /ædˈmaɪəɹ/ --- \textit{admirable} /ˈædməɹəbl/\\
	\textit{carnivore} /ˈkaɹnɪvɔɹ/ --- \textit{carnivorous} /kaɹˈnɪvəɹəs/\\
	\textit{ignore} /ɪgˈnɔɹ/ --- \textit{ignorant} /ˈɪgnəɹənt/
\xe
\end{figure}
...
though. Simplifying a lot, English\index{English} essentially tries to
construct trochaic feet from the right edge of the word. If the last syllable's
vowel is not light, it is skipped and stress moves to the antepenultimate
syllable; this process is recursive for words with multiple feet, although some
suffixes introduce irregularities in rule application.} To demonstrate how word
stress moves around in Ayeri, the complete declension paradigm for
...
\item Ayeri assigns trochaic stress\index{stress} from the right edge of a word. A foot 
...
\item Syllables ending in /ŋ/ or ones containing a diphthong\index{diphthongs} are considered 
	heavy\index{syllables!heavy}. They attract stress\index{stress} and take it away from a preceding stressed 
	syllable if the following syllable is not stressed already.
\item Syllables containing a long vowel\index{vowels} are considered superheavy\index{syllable!superheavy} and override 
	both light and heavy\index{syllable!heavy} syllables in attracting stress\index{stress}, since long vowels 
	cannot be unstressed.
\item Primary stress\index{stress} is assigned to the last stressable syllable, or otherwise 
	the last heavy syllable\index{syllable!heavy}. In the rare case of two long/superheavy\index{syllable!superheavy}
...
\begin{enumerate}
\item[5.] Secondary stress\index{stress} is assigned to syllables that are eligible for word 
...
\rayr{sjliNFyNF}{saylingyang} all receive stress on the final syllable, since
...
in contrast to compounds that are not as established, or formed \fw{ad
...
compound is one formed by reduplication\index{reduplication}, for instance,
...
in that a heavy syllable takes stress from a preceding light one. This does not
change in reduplication\index{reduplication}.
...
much of a desiderate.\footnote{\citet{peterson2015} very appropriately notes
that if \textcquote[66]{peterson2015}{you're creating a language on your own
and you're the only speaker, intonation is usually not high on the list of
features to focus on, but intonational flavoring is well worth it (read:
crucial) when it comes to making an authentic language}. There must be
intonational patterns that I have been subconsciously applying. Very likely,
they are influenced by my native language, German.} Yet, of course, the spoken
words in these recordings do not sound like robot speech either, so there must
be intonational patterns. Since intonation contours are notoriously difficult
to display in print, I will give very approximate graphs of pitch in the
respective examples for each surveyed pattern. Certainly there will be other
patterns as well which would require more detailed gradations, but for the time
being, I will only try to briefly describe those that are most prominent.
