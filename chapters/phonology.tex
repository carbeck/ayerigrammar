...
the feature pair \textsc{[±\,diph, \raisebox{1.25ex}{\rotatebox{180}{±}}\,ŋ]} 
behaves the same way throughout and both features are in complementary 
distribution, we need not test iterations of it separately, but can subsume 
them under the label \textsc{[±\,heavy]}. The parameters that need testing, 
then, are \textsc{[±\,heavy]} in combination with \textsc{[±\,long]}. There are 
4 possible outcomes for these two features, which in the case of three syllables 
...
has so far been a rather neglected topic and I have made a handful of recordings 
in Ayeri without ever considering intonation in detail much.\footnote{Whenever 
this happens, the fallback is likely to be a mix of German and English, since 
those are the languages I am most familiar with---the former being my native 
language, and the latter the secondary language I studied formally for the  
longest time.} Since intonation contours are notoriously difficult to display in 
print, I will give very approximate graphs of pitch in the respective examples 
for each surveyed pattern. Certainly there will be other patterns as well which 
would require more detailed gradations, but for the time being I will only 
...
Since Ayeri does not use a particle or word order to mark yes–no questions as 
...
fall in the overall contour of the question. The first half of the clause is 
very gently rising, the second half very gently falling.
...
Lists have the general gradual downward slope of statements, but the individual
items can nonetheless be marked by relatively higher pitch on the primary accent 
of each item.
...
\subsubsection{Relative and Complement Clauses}
