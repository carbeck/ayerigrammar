...
object\index{grammatical function!primary object} actually form a VP constituent together, since V and O are not adjacent
...
up with a flat, non-configurational\index{configurationality} structure, all but lacking a VP, as shown
...
\xhead{I} than a \xhead{V}, projecting an IP, which frees up VP and \xhead{V}
...
there is a VP sister of the subject\index{grammatical function!subject} NP which itself does not have a head, but
contains the object\index{grammatical function!primary object} NP as a complement. In the case of Tagalog, S is
non-configurational\index{configurationality}, that is, while XP may contain a non-finite verb, the
subject\index{grammatical function!subject} and object\index{grammatical function!primary object} NPs are on equal footing regarding certain functional
...
in VP. \Lfg{}\index{Lexical-functional grammar} thus avoids empty categories on the assumption that there is no
...
the object\index{grammatical function!primary object} NP are dominated by VP---compare the definitions in
(\ref{ex:domcomdef}).
...
below it, thus also \xhead{I} and the object\index{grammatical function!primary object} NP. In addition, \xhead{I}
c-commands its sister node and all of its children, hence also the object\index{grammatical function!primary object} NP.
...
The terms `subject'\index{grammatical function!subject}, `topic', and `focus'\index{grammatical function!focus} were already used a number of times
...
NP and the IP here. Unlike \textsc{gg}, \Lfg{}\index{Lexical-functional grammar} treats tense as a semantic
...
a single clause. It is not necessary for the focussed NP to be coordinated with
...
of the topic NP appears as a clitic\index{clitics} in the shape of the corresponding NP's case
...
Ayeri, on the other hand, non-topic animate\index{animacy} agents\index{cases!agent} are marked on NPs by
...
relation to subjects is verb agreement with third-person\index{person} NPs. This was already
...
Person\index{person} agreement in Ayeri is fixed to the agent NP in canonical cases, whether
...
person\index{person} agreement on the verb; the agent NP remains the subject. This is a very
\Nom{}--\Acc{} trait.
...
the VP, are understood to modify the verb as an intensifier\index{intensifiers} and are thus
...
floated \rayr{sno}{sano} still refers to this NP rather than the locative\index{cases!locative} NP,
...
NP's plurality\index{number!plural}. We would expect the forms \rayr{ynFye\_aNF}{yanjang} and
...
pronominal clitic\index{clitics}, since verbs must not go uninflected. Since all NPs are
...
(\ref{ex:ayrsecpred1_1}), the topic NP, \rayr{migFrj}{Migray}, happens to be
...
a VP niece of the subject NP (\ref{ex:ayrctrl1_2}), or a VP sister of \xhead{I}
...
clause\index{complement clause} or the non-finite VP niece of the subject NP here. Due to the
...
strategy of adjoining the VP of the complement clause\index{complement clause}, containing all its
...
longer clause is. It is possible, however, to use the VP adjunction\index{grammatical function!adjunct} strategy
with intransitive complement clauses\index{complement clause}, as illustrated by (\ref{ex:ayrctrl5}),
since there are no objects\index{grammatical function!primary object} in the adjoined VP to become problematic in terms of
...
refer to it as `focus'\index{grammatical function!focus}, \citet{schachter1976} as `topic', and
...
subject NP still c-commands the object\index{grammatical function!primary object} NP and case marking unambiguously tells
us that the first NP is the object\index{grammatical function!primary object}.\footnote{There are limitations on
...
functions as an object\index{grammatical function!primary object} NP cannot normally outrank what functions as a subject
...
superficially empty subject NP to the left of VP. Either way, the object\index{grammatical function!primary object} NP
...
antecedent, which is a possessor inside an object\index{grammatical function!primary object} NP. In order to reach a
...
thus, that Ayeri's object\index{grammatical function!primary object} NP is embedded in a VP which finds its (extended)
