...
appear not in place, but floated to the next page. Furthermore, note that the
analyses following hereafter are likely neither complete nor flawless; they are
merely first steps.} Since Ayeri is a verb-initial language\index{constituent
order}, it is probably rather comfortably analyzed in terms of
Lexical-Functional Grammar (\cite{bresnan1982}; more recently,
\cite{bresnan2016}; \cite{dalrymple2001}; \cite{falk2001}), since
\Lfg{}\index{Lexical-functional grammar} does not require complicated
derivations below the surface structure of sentences.\footnote{Passivization,
for instance, is assumed to be a lexically motivated alternation in predicate
structure (\Subj{} is blocked, so the nominative is assigned to \Obj{}, and the
original \Subj{} is expressed by an \Oblq{agt}), rather than an internal
derivational process \citep[23\psqq]{bresnan2016}.} It will be assumed here
that, even though Ayeri's unmarked constituent order\index{constituent order}
is VSO with predicate and predication not adjacent to each other, it is
configurational\index{configurationality} in that there is a verb phrase (VP)
which c-commands a number of other constituents as complements and adjuncts in
...
pruned of empty terminal nodes and non-branching preterminal nodes, since these
do not provide structurally or semantically relevant information
\citep[119--128]{bresnan2016}. Nodes are defined in their status by functional
annotations rather than by their position in the tree alone.}
...
abstractions of noun phrase (NP), verb phrase (VP), verb (\xhead{V}), etc.\ \citep[42]{bresnan2016}.
...
& \astruct{\tikzmark{verb}verb}{\tikzmark{x}x, \tikzmark{y}y}\medskip \\
...
& \begin{forest} baseline, shorter edges,
[\xbar{V}
	[\subnode{V}{V}]
	[NP \mysn{NP}
		[\xbar{N} \mysn{Nbar}]
...
to which part of the f-structure. Regarding the different functions
distinguished, \Lfg{} assumes the functional hierarchies given in
(\ref{ex:functions}), following \citet[97, 100]{bresnan2016}.
...
phrase-structure rules and c-structure trees together with arrows which
symbolize inheritance of feature information from the current level (↓) of the
tree to the next (↑). For instance, `\pass{\Subj}' means that the
...
questions---first and foremost, whether Ayeri actually has a VP, or in
terms of generative grammar: whether it is configurational. As we have seen,
Ayeri definitely has a constituent structure as far as NPs, adjective phrases 
(APs), adpositional phrases (PPs), etc. are concerned. However, due to VSO word
order, it is not obvious whether verb and object actually form a VP constituent
together, since V and O are not adjacent to each other. Since Ayeri marks topics
in terms of morphology, it will also be necessary to discuss how this mechanism
works and how it relates to the notion of the subject.

A discussion of subject, topic, and configurationality is interesting also in
that Ayeri's syntactic alignment looks superficially similar to the
...
more conventional. Austronesian alignment is therefore not necessarily the best
...
As mentioned above, Ayeri's unmarked constituent order\index{constituent order} gives the verb\index{verbs} first, and then,
...
causative case\index{case!causative}. Ayeri's basic constituent order\index{constituent order} is thus VSO, a trait it has in common
...
% German, however, since the infinite content verb \xhead{V} appears at the
...
Such a structure does not do Ayeri justice in that, if S, O, and V
...
however, allows precisely this. In spite of not being adjacent, V and O must
form a unit of some kind. The verb\index{verbs} canonically agrees\index{agreement} with S by default\index{default};
situations where it agrees\index{agreement} with O are very restricted. Moreover, Ayeri has
NP\index{phrase types!noun phrase}--XP constructions where XP is not a maximal projection of a verb, so NP\index{phrase types!noun phrase} and
XP are probably contained in a small-clause constituent S\index{phrase types!small clause} separate from the
verb at least for the purpose of copular clauses\index{copular clause}. Furthermore, the verb\index{verbs} in the
initial position shows inflection, so we might rather construe it as an
\xhead{I} than a \xhead{V}, projecting an inflectional phrase (IP)\index{phrase types!inflectional phrase}, which frees up VP\index{phrase types!verb phrase} and \xhead{V}
...
[IP
	[\anno{\mysn{welshcstruct_IP} I}
		[{\mysn{welshcstruct_I} \textit{gwelodd}\\ `see-\Tsg{}.\Pst{}'}]
	]
	[\anno{S}
		[{\anno[\pass{\Subj}]{\mysn{welshcstruct_S} NP}}
			[{\textit{Siôn}\\ `John'}]
		]
		[\anno{\mysn{welshcstruct_VP} VP}
			[{\anno[\pass{\Obj}]{\mysn{welshcstruct_O} NP}}
				[{\textit{ddraig}\\ `dragon'}]
			]
		]
	]
...
appears in \xhead{I}, which is a daughter of S. S, in turn, is a small clause\index{phrase types!small clause}
containing the arguments of the verb. In the case of Irish\index{Irish} and Welsh\index{Welsh}, however,
...
non-configurational\index{configurationality}, that is, while XP may contain an infinite verb, the
...
the verb\index{verbs} is not lost; it is merely now provided by the verb\index{verbs} in
...
\posscite{schachter2015} and \posscite{kroeger1991} respective analyses of
...
subject,\footnote{\posscite{cowan1995} sketch may be based on
...
note that the handbook article quoted from here was published posthumously.}
...
these notions in more detail. As we will see, all of subject, topic, and focus relate to
...
two varieties of clauses: intransitive\index{verbs!intransitive} ones, where the verb has just one core
argument, and transitive\index{verbs!transitive} ones, where the verb has two core arguments. A basic
...
\citet{comrie1989} illustrates this difference with an example from
...
\fw{her} in (\ref{ex:engchuk_eng}).} While English\index{English} treats the
actor of the intransitive sentence (\ref{ex:engchuk_eng1}) the same as that of
the transitive one (\ref{ex:engchuk_eng2})---both sentences use \fw{I} in the
nominative---Chukchi appears to use a different pronoun for the actor of the
intransitive sentence (\ref{ex:engchuk_chuk1}) than for the actor of the
transitive one (\ref{ex:engchuk_chuk2})---absolutive \fw{ɣəm} versus ergative
\fw{ɣəmnan}, respectively. At least in Standard English\index{English}, it would
be ungrammatical to use the pronoun \fw{me} in place of \fw{I} in
(\ref{ex:engchuk_eng2}), since \fw{me} can only be used for first-person objects
of the verb, but not for subjects of transitive clauses.
...
sentences, this surfaces in the specifier of TP}. However, as
...
(\ref{ex:basicsvo2}). For systemic reasons, \citet{carnie2013} refers to a determiner phrase (DP)
subject which serves as the specifier of a tense phrase (TP). This corresponds to the subject
...
\Lfg{}\index{Lexical-functional grammar} defines a subject function, \Subj{}. Which argument of the verb the
...
example of Warlpiri\index{Warlpiri} in (\ref{ex:warlastruct}). The only
condition is that grammatical functions be uniquely defined within their minimal
f-structure \citep[45]{bresnan2016}.\footnote{The functional bundle of a
predicator and its arguments \citep[230, 250]{bresnan2016}. This is
essentially \Lfg{}'s equivalent of the logical form (\textsc{lf}).} As
(\ref{ex:warlastruct}) shows, multiple NPs in different positions in the
constituent structure may feed semantic information to a single function defined
by the argument structure of the verb.
...
\begin{forest}
[IP
	[{NP~\mysn{warlastruct_ERGNP1}}
		[{...-\Erg{}}, roof]
	]
	[\xbar{I}
		[I, minimum width=5em]
		[S, minimum width=5em
			[{\mysn{warlastruct_V}~V}]
			[{NP~\mysn{warlastruct_ABSNP1}}
				[{...-\Abs{}}, roof]
			]
			[{NP~\mysn{warlastruct_ERGNP2}}
				[{...-\Erg{}}, roof]
			]
			[{NP~\mysn{warlastruct_ABSNP2}}
				[{...-\Abs{}}, roof]
			]
		]
	]
...
\Subj{} is assigned the features [–\,r, \mbox{–\,o}], since the subject is not
restricted to a certain semantic role, nor needs to have a semantic
role.\footnote{This ought to make \posscite{kroeger1991} analysis compatible to
\Lfg{}\index{Lexical-functional grammar} as well.} Also, subjects do not complement transitive predicators like
objects do, so they are not `objective'. \posscite{bresnan2016} lexical mapping
theory assumes that all languages have subjects, which goes counter to
\posscites{schachter1976}{schachter2015} claim that subjects are possibly not
...
\fw{leitmotif}}. Thus, the topic is
...
contrastive focus, which raises the prominence of a certain NP within
...
on the verb is thus related to nominal case markers in Ayeri, Tagalog\index{Tagalog} uses a
...
% Hence, I will only give basic information here.

\citet{kroeger1991} mentions that Tagalog\index{Tagalog} has optional number agreement of
...
Person\index{person} agreement in Ayeri is fixed to the agent\index{case!agent} NP\index{phrase types!noun phrase} in canonical cases, whether
...
agreeing itself in person with the agent: \rayr{AgYaanF}{Ajān} is a male name;
...
person agreement\index{agreement} on the verb; the agent\index{semantic role!agent} NP\index{phrase types!noun phrase} remains the subject\index{grammatical function!subject}. This is a very
\Nom{}--\Acc{} trait.
...
patient/theme or the recipient. The actor of the Tagalog\index{Tagalog} sentence is also
...
actor voice according to the asker as the actor (A) being the subject.

In order to now investigate what the situation is in Ayeri, let us return to
our initial set of examples. These examples featured two animals which
are treated both as animate neuters. Anaphoric reference is therefore potentially
...
coordinated\index{coordination} intransitive clause can theoretically license either of them as its
...
the verb in the coordinated\index{coordination} clause could theoretically pick either the agent\index{semantic role!agent} NP
...
slightly different, however, if the reference of the verb is unambiguous\index{ambiguity}, for
...
the verb in the second conjunct, \xayr{sr/}{sara-}{leave}. Besides, this verb is
...
As before, there are two masculine NPs in the first conjunct which form suitable
antecedents on behalf of being animate masculine as required: the agent (A)
\rayr{AknF}{Akan} and the recipient (R) \rayr{mrnF}{Maran}. Of the remaining
non-topic\index{grammatical function!topic} NPs\index{phrase types!noun phrase},
Ayeri considers the agent\index{semantic role!agent} to rank higher as a
secondary topic\index{grammatical function!topic} on the thematic
hierarchy\index{hierarchy} than the recipient\index{semantic role!recipient},
compare (\ref{ex:themhier}). The agent\index{semantic role!agent} hence forms
the preferred controller for \rayr{sryaaNF}{sarayāng}. In cases where the
topic\index{grammatical function!topic} in the first conjunct can safely be
ruled out as the controller of the pronominal in the second conjunct, the
syntactic pivot defaults\index{default} to the highest-ranking semantically
coherent NP\index{phrase types!noun phrase}. Ayeri therefore mostly groups the
intransitive\index{verbs!intransitive} subject\index{grammatical
function!subject} and the transitive agent\index{semantic role!agent} together.
...
seen above in (\ref{ex:ayrpivot}): the S of an intransitive
clause\index{verbs!intransitive} receives the same case marker as the A of a
transitive clause: \rayr{/ANF}{-ang}/\rayr{ANF}{ang} for animate\index{animacy}
referents, and \rayr{/reNF}{reng}/\rayr{ENF}{eng} for inanimate\index{animacy}
referents (compare \autoref{subsubsec:agent}). The case described initially,
where the topic\index{grammatical function!topic} marking determines the
controller of the coordinated\index{coordination} intransitive
clause\index{verbs!intransitive}, is essentially a strategy to
disambiguate\index{ambiguity} between two possible controllers for the same
target. This is broadly reminiscent of Tagalog's\index{Tagalog} syntax. On the
other hand, when only one of the referents in the transitive conjunct is
eligible as the controller of the subject\index{grammatical function!subject} of
the intransitive\index{verbs!intransitive} conjunct at the same time, A and P
are regularly indicated by person agreement\index{agreement}, since Ayeri
requires a resumptive pronominal clitic\index{clitics} in the intransitive
clause\index{verbs!intransitive}, as indicated above. The affix on the verb thus
has the status of a pronominal predicator, compare (\ref{ex:ayrpivot5}).
...
(\ref{ex:ayrpivot5_2}) the verb of the intransitive\index{verbs!intransitive} conjunct, \xayr{sryeNF}
...
English\index{English}, consider, for instance, example (\ref{ex:engqfloat}).
...
constitute the subject NP according to voice and case marking. Example
(\ref{ex:tagqfloat_2}), then, represents the opposite case, where it refers to
the letters, which are marked as the subject this time. Of course, it is equally
possible in English\index{English} to say \fw{The letters are all written by the
children}, where \fw{the letters} is the subject that the floated \fw{all}
refers to.
...
\xayr{ynNF}{yanang}{boy(s)}, as the agent\index{semantic role!agent} NP, remains the subject\index{grammatical function!subject}. The
floated \rayr{sno}{sano} still refers to this NP rather than the locative NP,
\xayr{ljye}{layye}{(at) the girls}. This fact is also reflected in morphology by
the lack of plural marking on \rayr{ynNF}{yanang}, since \rayr{sno}{sano}
indicates the NP's plurality and additional plural marking would be considered
redundant. We would expect the forms \rayr{ynFye\_aNF}{yanjang} and \rayr{lj}
{lay} if \rayr{sno}{sano} were to refer to `the girls' rather than `the boys',
as in (\ref{ex:ayrqfloat3_2}).
...
finite verb. Ambiguity\index{ambiguity} may be in the phrase structure of the clause here, but
...
verb. This is also possible for the purpose of quantification of pronouns\index{pronouns!personal} with
...
\rayr{sitNF/henF}{sitang-hen} in both examples in (\ref{ex:pronquant}). See
...
relativized. He refers to \posscite{keenancomrie1977} accessibility hierarchy\index{hierarchy} of
...
discussion thus serves the purpose only of illustrating how Ayeri handles
relative clauses. As shown below, non-topic\index{grammatical function!topic}
NPs\index{phrase types!noun phrase} may be relativized, and relative clauses not
uncommonly contain their own agent\index{semantic role!agent} NP\index{phrase
types!noun phrase}. The relativized NP\index{phrase types!noun phrase} may even
be referred to in the relative clause by a resumptive
pronoun\index{pronouns!resumptive} or pronominal clitic\index{clitics}, since
verbs\index{verbs} must not go uninflected. First consider (\ref{ex:ayrrel}).
...
In (\ref{ex:ayrrel}), \xayr{Inuny}{inunaya}{the fisherman}, is both the
topic\index{grammatical function!topic} of the clause and modified by a relative
clause. He is referenced anaphorically by the \TsgM{}.\Aarg{} suffix
\rayr{/yaaNF}{\mbox{-yāng}} on the verb in the relative clause, since he is the
actor in both. However, as the examples in (\ref{ex:ayrrel2}) show, these
circumstances are not requirements for grammatical statements.
...
In (\ref{ex:ayrrel2_1}), the recipient NP\index{phrase types!noun phrase} \xayr{gnFymF}{ganyam}{to the child}
is not the topic\index{grammatical function!topic} of the clause, but it is modified by a relative clause anyway.
The relativized NP\index{phrase types!noun phrase} is again represented in the
relative clause through verb morphology. The topic\index{grammatical
function!topic} marker on the verb identifies the person suffix on
the verb as the clause's topic\index{grammatical function!topic}. In (\ref{ex:ayrrel2_2}), it is likewise not the
topic\index{grammatical function!topic} NP\index{phrase types!noun phrase} which is relativized, but the patient NP \xayr{InunFlej}{inunley}
{fish}. This NP\index{phrase types!noun phrase}, however, is not represented in the relative clause because the
verb does not inflect for the role of the patient, which is the same in the
relative clause. There is no morphology to alter the
voice of the verb\index{verbs} in a way for the matrix clause's patient
\index{semantic role!patient} NP\index{phrase types!noun phrase} to become the
...
cases, no case-marked noun or topic\index{grammatical function!topic}
is contained in the relative clause.
...
Secondary predicates in Tagalog\index{Tagalog} are interesting in that depictive
adjectives always modify the nominative argument according to
\citet{kroeger1991}; compare (\ref{ex:tagsecpred}).
...
since `Maria' is not the subject of the clause. Examples of structurally
equivalent phrases in Ayeri can be found in (\ref{ex:ayrsecpred1}).
...
Tagalog\index{Tagalog}, the reference of the adjective\index{adjectives} should change in the way shown in
...
{drunk}: \rayr{migFrj}{Migray} is drunk. What is more, (\ref{ex:ayrsecpred1_2}) generates the same
...
the topic of the clause. By structure alone, \rayr{niysF}{Niyas} could also be
...
in the morphological marking of the privileged NP have no impact on the relation
between adjective and noun. The marking on the verb is therefore not used for
...
infinite, like in English\index{English}. Unlike in Tagalog\index{Tagalog}, it cannot carry any marking for
...
If the topic\index{grammatical function!topic} is actually the subject\index{grammatical function!subject}, it should be possible to raise
...
fact that the subordinate verb appears in an infinite\index{verbs!infinite} form when raising occurs
...
topic\index{grammatical function!topic} marking, it would not be possible for the infinite verb\index{verbs!infinite} to mark the
...
permissively, case\index{case} marking is nonetheless based on semantic roles rather than
...
assigning it patient case\index{case!patient}, though, compare (\ref{ex:ayrrais5}).
...
subject\index{grammatical function!subject} of the subordinate verb. The main clause predicate is thus thought to
...
mood form an exception as well \parencite[36--37, 96--97]{kroeger1991}. 
\citet{kroeger1991} illustrates the main pattern of control in
...
with a full complement clause\index{complement clause}, as in
(\ref{ex:ayrctrl1_1}), a VP\index{phrase types!verb phrase} niece of the
subject\index{grammatical function!subject} NP (\ref{ex:ayrctrl1_2}), or a
VP\index{phrase types!verb phrase} sister of \xhead{I} (\ref{ex:ayrctrl1_3}). In
both (\ref{ex:ayrctrl1_1}) and (\ref{ex:ayrctrl1_2}), it is necessarily the
actor which is coreferenced, as the bottom arrow shows. In
(\ref{ex:ayrctrl1_3}), the bottom arrow instead shows the relation of verb
agreement\index{agreement}. The arrow on top, as before, shows what the
...
As with raising verbs, verbal complements\index{complement clause} appear in an infinite form, the
participle\index{participle}. For (\ref{ex:ayrctrl1_2}) the reason may be that there is no overt
agent\index{case!agent} in the clause to agree with, and agreement\index{agreement} with the patient\index{case!patient} does not make
...
person features. If the topic\index{grammatical function!topic} marking on the finite verb is altered as in
...
the verb between both versions of the sentences. We have seen above that Ayeri does
not allow to-object raising, since it is not possible to assign patient case\index{case!patient} to
...
(\ref{ex:ayrctrl3}ab), as well as to complement\index{complement clause} the
verb in the matrix clause with an infinite clause with object control
(\ref{ex:ayrctrl3_3}). However, the strategy of adjoining a VP\index{phrase
types!verb phrase} to IP\index{phrase types!inflectional phrase} in order to
make the subordinate verb's arguments available for
topicalization\index{grammatical function!topic} by the matrix clause's verb is
not available here because this would cause a doubling of case roles and alike
grammatical functions. Such a configuration is considered ungrammatical in
Ayeri, compare (\ref{ex:ayrctrl3_4}). As we will see below, however,
this is not an issue for intransitive complement clauses\index{complement
clause}.
...
clause\index{complement clause} or the infinite VP\index{phrase types!verb phrase} niece of the subject\index{grammatical function!subject} NP here. Due to the
...
As \autoref{tab:tagayrcomp} shows, Tagalog\index{Tagalog} and Ayeri are not
really similar in syntax despite superficial similarities in morphology.
According to \posscite{kroeger1991} thesis---which critically reviews and
updates \posscite{schachter1976} survey by leaning on \Lfg{}
theory---Tagalog\index{Tagalog} prefers the argument which corresponds to the
marking on the verb for most of the traits usually associated with subjects.
According to \posscite{kroeger1991} analysis, this NP is the nominative
argument, thus. \citet{schachterotanes1972} refer to it as `focus',
\citet{schachter1976} as `topic', and \citet{schachter2015} as
`trigger'.\footnote{`Trigger' is also the term often seen in descriptions of
constructed languages in this respect.} \citet{kroeger1991} finds in his survey
that the nominative argument is largely independent from the actor, so that the
logical subject is not necessarily the syntactic subject; what
\citet{schachter1976} calls `topic' also does not behave like a pragmatic topic
...
Essentially, what Tagalog\index{Tagalog} does, according to \posscite{kroeger1991} analysis, is
...
In conclusion, is Ayeri a so-called `trigger language'?\index{grammatical
function!trigger@`trigger'} Yes and no. Ayeri incorporates the morphological
feature of marking a certain privileged argument of the verb (the topic) on the
verb and may therefore be counted among `trigger languages'\index{grammatical
function!trigger@`trigger'} by a very broad definition.\footnote{It seems to me
that what conlangers call `trigger language'\index{grammatical
function!trigger@`trigger'} mostly refers to just the distinct morphological
characteristic of languages like Tagalog\index{Tagalog} by which a certain NP is
marked on the verb with a vague notion that this NP is in some way important in
terms of information structure.} However, the real-world Austronesian alignment
as a syntactic phenomenon is more extensive than that according to the
discussion of the various effects described in \posscite{kroeger1991} survey.
Ayeri, in syntactically behaving rather consistently like a \Nom{}--\Acc{}
language, misses the point completely if `trigger language'\index{grammatical
function!trigger@`trigger'} is understood to also entail syntactic
characteristics of Philippine languages.

\section{Establishing configurationality}
\label{sec:config}
\index{configurationality|(}

As mentioned above, Ayeri's unmarked constituent order is VSO, and unlike
Tagalog\index{Tagalog}, it mostly displays correlations between the agent and
syntactic traits usually associated with subjects. I will assume, therefore,
that the agent\index{semantic role!agent} argument is, in fact, the syntactic
subject\index{grammatical function!subject} for all intents and purposes. It was
also pointed out above that not grouping V and O together does not automatically
result in non-configurationality at the sentence level. Moreover,
\citet[128]{speas1990} points out that free word order\index{word order} alone
is not sufficient evidence to claim non-configurationality either. While Ayeri
canonically marks case overtly on NPs\index{phrase types!noun phrase}, and
NPs\index{phrase types!noun phrase} have a certain degree of freedom with
regards to their ordering, it does not mean that any order is always acceptable,
much like *\fw{the yellow American big school bus} is not acceptable in
English\index{English} even though all adjectives equally describe \fw{school
bus} with no apparent ranking implied.
...
consistently verb-first\index{constituent order} and does not permit nominal material to precede a
...
pronominal binding\index{binding} here, however, since pronouns\index{pronouns} must not precede their binder
...
Since Ayeri is very strict about the placement\index{constituent order} of the verb and does not share
English's\index{English} strategy to emphasize a verb with the equivalent of \fw{do}, it is
...
read (it) indeed} in Ayeri.} However, it is possible to reproduce 
(\ref{ex:engvconst}bc) in Ayeri, as illustrated in (\ref{ex:ayrvconst1}).
...
conjunct's verb as well as its object\index{grammatical function!primary object} are copied to the second conjunct, with
the object\index{grammatical function!primary object}/topic of the first conjunct being dropped in order to avoid
repetition. Replacement by a pro-form and dropping are therefore not good measures
...
subject\index{grammatical function!subject} NP, while there are idioms consisting of a set combination of a verb
...
Ayeri allows, for one, for intransitive verbs\index{verbs!intransitive}, like
\xayr{nib/}{niba-}{rest}. Secondly, it allows for transitive
verbs\index{verbs!transitive}, like \xayr{koMdF/}{kond-}{eat}, to be used
intransitively. There are no verbs, however, which require an
object\index{grammatical function!primary object} but do not optionally or
obligatorily require a subject. While the agent\index{case!agent} of a
transitive verb\index{verbs!transitive} may be dropped, the
patient\index{case!patient} argument triggers verb agreement\index{agreement}
which is canonically with the agent\index{semantic role!agent} subject---compare
example (\ref{ex:ayragr2_2}) above, which is repeated in (\ref{ex:passnosubj})
for convenience.
...
Even for verbs\index{verbs} expressing impersonal actions, like the weather verb in
...
these sentences, the pronoun\index{pronouns} and the NP\index{phrase types!noun phrase} containing the name 
are supposed to bind each other.
...
referring to the grammaticality of coreference between reflexive
pronouns\index{pronouns!reflexive} (`ana\-phora'), personal
pronouns\index{pronouns!personal}, and R-expressions (references to
...
In \Lfg{}\index{Lexical-functional grammar}, however, due to its being based
primarily on functional structure, the condition for binding is not based on the
c-structure of a clause. Rather, a pronoun\index{pronouns} is required to have
an antecedent in the minimal f-structure of the predicator, that is,
\textquote{the \Pred{} element and all of the elements whose attributes are
functions designated by the \Pred{}} \parencites[230, 250]{bresnan2016}. For the
examples above, this means that a reversal of dependency relations is not
possible, unless one were to assume that an \Obj{} could syntactically outrank a
\Subj{}. Due to \Lfg{}'s\index{Lexical-functional grammar} design, even if the
structure of the clause were flat, it is assumed that every language has
grammatical functions, so that what functions as an object\index{grammatical
function!primary object} NP\index{phrase types!noun phrase} cannot normally
outrank what functions as a subject NP\index{phrase types!noun phrase}. Besides,
Ayeri does fulfill the requirements based on constituent structure sketched out
above as well as the functional requirements; compare examples
...
suffix \xayr{/ye}{-ye}{she} is supposed to be co-indexed with \rayr{mli}{Mali},
...
antecedent, which is a possessor inside an object\index{grammatical function!primary object} NP\index{phrase types!noun phrase}. In order to reach a
...
Taking the above into account and looking only at the consituent structure, Ayeri
...
head in \xhead{I} instead of in \xhead{V}, if a full subject NP is present at
