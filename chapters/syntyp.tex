...
terms of generative grammar: whether it is configurational\index{configurational}. As we have seen,
...
A discussion of subject, topic, and configurationality\index{configurational} is interesting also in
...
verb's arguments: subject (agent\index{cases!agent}), direct object (patient\index{cases!patient}), indirect object
...
up with a flat, non-configurational\index{configurational} structure, all but lacking a VP, as shown
...
verb at least for the purpose of copula clauses. Furthermore, the verb in the
...
non-configurational\index{configurational}, that is, while XP may contain a non-finite verb, the
...
the object NP are dominated by VP---compare the definitions in
(\ref{ex:domcomdef}).
...
languages do not automatically need to be considered `non-configurational'\index{configurational} and
...
modified by enclitic\index{clitics} intensifiers indirectly by using
...
clauses in Ayeri may even just consist of a predicative adjective\index{adjectives}. In these
...
adjectives\index{adjectives} relating to the object inhabit the same postverbal position.
...
verb which determines whether the postverbal predicative adjective\index{adjectives} refers to
...
between adjective\index{adjectives} and noun. The marking on the verb is thus not used for
...
non-actor topics in the complement clause\index{comparison} in the (a) examples which we attempt
...
of example. That is, the subject of the complement clause\index{comparison} in
...
subordinating verb with a full complement clause\index{comparison}, as in (\ref{ex:ayrctrl1_1}),
...
As with raising verbs, the complement\index{comparison} verbs appear in a non-finite form, the
...
not allow to-object raising, since it is not possible to assign patient case\index{cases!patient} to
...
(\ref{ex:ayrctrl3}ab), as well as to complement\index{comparison} the verb in the matrix clause
...
clause\index{comparison} or the non-finite VP niece of the subject NP here. Due to the
...
is no ambiguity in anaphoric reference in the complement clause\index{comparison}.
...
strategy of adjoining the VP of the complement clause\index{comparison}, containing all its
...
with intransitive complement clauses\index{comparison}, as illustrated by (\ref{ex:ayrctrl5}),
...
hypothesis that Ayeri is configurational\index{configurational}. Since it clearly prefers agent NPs
...
would mean that the verb carried not simply the third-person agreement\index{agreement} suffix
...
obliged to carry the third-person pronominal clitic\index{clitics} \xayr{/yaaNF}{-yāng}{he}
...
an inanimate\index{animacy} third-person pronominal of some kind.
...
core f-structures, $g$ and $f$. The object pronoun, \xayr{yesF}{yes}{her}, is
