...
compounds of seemingly arbitrary length to be strung together, like in the 
following example from German:

\ex\begingl\rc{German}%
	\gla Donaudampfschifffahrtsgesellschaftskapitän //
	\glb Donau-dampf-schifffahrt-s-gesellschaft-s-kapitän //
	\glc Danube-steam-shipping-\Gen{}-society-\Gen{}-captain //
	\glft `Captain of the Danube steamshipping society' //
\endgl\xe

German is said to be especially notorious of this for endocentric noun 
compounds. Ayeri, on the other hand, allows only two elements in compounds. 
Furthermore, this section on compounds is located within the section on nouns 
because Ayeri only possesses compounds involving nouns, and the majority of 
these also results in a noun.
...
Synthetic compounds consist of a verb and a noun which is an argument of the 
action denoted by the verb, such as \fw{…} in \fw{…} \citep[?]{bauer2001}. 
Ayeri possesses compounds like this as well, and the regular case again follows 
the constituent order of verbs and nouns \citep[?]{gaeta2008}: Ayeri is a VO 
language, and thus the verb as the head of the compound is found on the left 
side with its nominal modifier following it:
...
\fw{caused by} the sun; and a waysign shows the way (\rayr{ssaanF}{sasān} is the 
object here). Even though \rayr{kronF}{karon} may serve as an agent (or a 
...
well, since verbs always go first in Ayeri sentences, and any other NPs, whether 
actor or undergoer, are following.
...
suffix \rayr{/ymF}{-yam} which is lexicalized as part of the word and marks it 
as a tool in a way: something that is being spread on a surface. A few more 
such verbal derivations can be found as well, though not compounds, among 
others:
...
\pex
	...
	\a \makebox[10.5em][l]{\xayr{shymF\larger }{sahayam}{future}}
...
example words (\ref{ex:compvbrev}a--d), an interpretation of the second part as 
a deverbal noun is possible: a labyrinth as a way or path which requires 
guessing, a plane a machine for flight, and radio as a sending of waves.
...
% \xayr{AnFlaagon/}{anlāgona-}{pronounce}, lit. `bring-out' → verb!
