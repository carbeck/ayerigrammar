...
be reduced to [ʤ] \orth{-j} before case\index{case} suffixes\index{suffixes} beginning with /j/ or with a
...
is marked as an agent\index{semantic role!agent}, not as a patient, since that would be semantically
incongrouous.
...
will not do so, however, if the order\index{constituent order} of arguments is simply scrambled, as in
...
agreement\index{agreement} with the nearest NP\index{phrase types!noun phrase} or agreement\index{agreement} with all (resolution\index{resolution}) is
...
in the way of a special clitic in \posscite{zwicky1977} terminology, since no
...
not behave like transitive verbsinfinite as in English\index{English}. Thus, what would be the object
