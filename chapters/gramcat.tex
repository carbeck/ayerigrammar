...
these cases. It may thus be duly noted here that Ayeri indeed possesses 
indefinite pronouns.\footnote{As it is a fictional language, the scientific 
value of this assertion to linguistic typology necessarily remains doubtful, 
however.} In order to classify languages, \citeauthor{haspelmath1997} 
generalizes the map displayed in \autoref{fig:haspeltab} based on a sample of 
100 languages from all continents, although he notes that this sample has a 
European bias due to the availability of data \citep[2]{haspelmath1997}. 
Languages typically form continguous areas on the map, even though they may 
carve it up quite differently, and with overlaps between the different semantic 
groupings 1--9.
...
verb-initial and verb–object word order, noun–genitive word order, and having 
...
\citet{wals46A}, indeed suggests areal clustering, especially for generic-noun 
based systems (Africa and Southeast Asia). \citetitle{wals} classifies 194 out 
of 326 languages (60\pct) as possessing interrogative-based indefinite pronouns 
...
data for constituent-order with the map on indefinite-pronoun systems does not 
yield a convincing result.} He still presumes, however, that word-order typology 
may have an effect on the formation of indefinites insofar as it correlates with 
grammaticalization more generally \citep[239]{haspelmath1997}.
...
array like the one presented in \autoref{fig:indeftab}. At first sight, it is 
obvious that Ayeri does not distinguish `every' and `any' the way English does; 
there is no distinction in polarity (affirmative versus negative) the way 
English has it:
...
also find in the demonstrative pronouns, and which also exists as a 
nominalization of an otherwise lost morpheme, \xayr{nYaanF}{nyān}{person}. In 
the same way, the pronouns related to the notion of place have a \rayr{y/}{ya-} 
or \rayr{ynF/}{yan-} part, which we also find in 
\xayr{yno}{yano}{place}.\footnote{\rayr{yno}{yano} itself is an old 
nominalization and very likely related as a morpheme to the locative suffix 
\rayr{/y}{-ya}.} In a regular continuation of this pattern, the indefinite 
pronouns of time all have an element related to \xayr{tdj}{taday}{time} in 
them, which is obscured somewhat by palatalization in \rayr{tdYrilF}{tajaril}. 
The exception to this series, then, is \rayr{dYaanFymF}{jānyam}, which is the 
multiplicative numeral formed from \xayr{dYaanF}{jān}{zero}, thus rather means 
`zero times' or `not once' than `never', although it can also be used 
emphatically for the latter. The series of manner pronouns is an absolute 
exception in that it must be a residue from an older layer of 
grammaticalization in that \rayr{Arem/}{are-} is not a recognizable morpheme in 
the modern language.\footnote{I probably made this up as I was going, many years 
ago, and without considering systematic implications, as I was unaware of 
them at the time.} \rayr{mirnrilF}{miranaril} is a regular formation from 
\xayr{mirnF}{miran}{way, manner} combined with the quantifier (!) for indefinite 
amounts, \xayr{/ArilF}{-aril}{some}.

This observation leads to the second regular feature, that is, affixes as 
modifiers to generic nouns. The every/any series regularly features the 
morpheme \rayr{EnF}{en}, either prefixed or suffixed, which is related to the 
quantifier \xayr{/henF}{-hen}{every, all, each} and can presumably found even 
on \rayr{AreenF}{arēn} in spite of its obscure lexical base. In the same 
...
\rayr{mirnrilF}{miranaril} it is probably due to analogy with the other 
pronouns in this series. In any event, \rayr{metj}{metay} has the semantically 
`proper' \rayr{me/}{me-} prefix related to absolute 
inspecificity.\footnote{Compare German \fw{irgendjemand} and French 
\fw{n'importe qui} `no matter who'.} This alternation is employed to distinguish 
between the meaning of `sometime', that is, occurring once an unspecified point 
in time, and \xayr{tdYrilF}{tajaril}{sometimes}, that is, occurring recurringly 
at inspecific times. Lastly, the negative series is reguarly marked by the 
negative suffix \rayr{/Oj}{-oy}, which also occurs with adjectives and verbs 
(see sections \ref{subsec:adjneg} and \ref{subsubsec:verbneg}). An outlier in 
this series is the person/thing-related indefinite pronoun, \rayr{rnY}{ranya}. 
The etymologic connections of the \rayr{r}{ra} part are not presently known, 
perhaps the postposition \xayr{rnF}{ran}{against} is related.
...
to express direct negation (7). Since double negation---that is, for emphasis, 
not in the strictly logical sense---is possible, the `none' series may also be 
use for indirect negation (6). Moreover, Ayeri uses the `every' series for both 
comparison (8) and free choice (9). Besides this, absolute-indefinite 
\rayr{me/}{me-} can be used for (3)--(6) in combination with a noun to attach 
to. \autoref{fig:haspeltabayr} shows the groupings for Ayeri; 
...
\a specific, known to the speaker:\vspace{.5em} % 1
	\beginsubsub
	\b{a.} \begingl
		\gla Ang sahaya arilinya, leku, sinyāng adaley! //
	...
	\b{b.} \begingl
		\gla Le ilta ningyang arilinya vayam. //
		\glb Le ilta ning=yang arilinya-Ø vayam //
		\glc \PatTI{} need tell=\Fsg{}.\Aarg{} something-\Top{} 
			\Ssg{}.\Dat{} //
