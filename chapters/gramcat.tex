...
exemplified in (\ref{ex:negallo}), as well as showing the \fw{-u} allomorph. 
Moreover, example (\ref{ex:irrneg}) shows that negative marking usually follows 
irrealis marking when suffixes are stacked: \rayr{/ONF}{-ong} + \rayr{/Oj}{-oy} 
→ \rayr{/ONoj}{-ongoy}.
...
that is, imperative clauses do not require an overt second person agent. No 
distinction is made between singular and plural, so that the marker is 
\rayr{/U}{-u} in either case.

\pex
\a\begingl
	\gla Tangu yām! //
	\glb Tang-u yām //
	\glc listen-\Imp{} \Fsg{}.\Dat{} //
	\glft `Listen to me!' //
\endgl
