...
In cases where a comparee is left unexpressed or the property compared 
contains a full NP, Ayeri uses clitic suffixes on adjectives for comparison:
...
In (\ref{ex:sfxcomp1}), the property under comparison, \xayr{tiNrti\_asF 
bnF/ENF}{tingracas ban-eng}{a better musician}, is a complex NP, while in 
(\ref{ex:sfxcomp2}) the comparee is missing. The example in 
(\ref{ex:sfxsupl}) similarly expresses an absolute without giving a group of 
entities to draw from. In all these cases, it is, however, also possible to use 
a more complex analytic construction using verbs which will be covered at a 
later point.
