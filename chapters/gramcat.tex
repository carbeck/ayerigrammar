...
(\ref{ex:n-adj-rel}), the relative pronoun is inflected to agree in case with 
its antecedent in formal language: \rayr{ssF}{sas} agrees in case with 
...
In cases where comparison is not constructed as a predicative nominal, Ayeri 
uses clitic suffixes on adjectives:

\pex\label{ex:sfxcomp}
\a\label{ex:sfxcomp1}\begingl
	\gla Ri tavya ang Diyan tingracas ban-eng dabayaman. //
	\glb Ri tav-ya ang Diyan tingrati-as ban=eng daba-yam-an-Ø //
	\glc \InsT{} become-\TsgM{} \Aarg{} Diyan musician-\Parg{} good=\Comp{} 
		practice-\Ptcp{}-\Nmlz{}-\Top{} //
	\glft `By practicing, Diyan became a better musician.' //
\endgl
...
In (\ref{ex:sfxcomp1}) the comparee is missing; (\ref{ex:sfxsupl}) expresses an 
absolute and is also constructed using a more simple and widespread synthetic 
alternative to a rather complex analytic construction that will be covered at a 
later point. The same goes for the following synthetic comparative construction:

\ex\label{ex:sfxcomp2}\begingl
