...
illustrate, (\ref{ex:noungendex}) gives a few examples for each category.
...
for case, which is governed by the verb. Since in Ayeri, case marking is at
least partially based on semantics rather than just purely function or
structure which causes a few exceptions, it is better, in my opinion, not to
...
% \footnote{This is the main reason I 
...
% patient topics, respectively.}
Thus, the first NP after the verb in all of the
...
Ayeri. The following example shows the addressee of a speech act in the dative
...
agentless sentences and also the subjects of unaccusative verbs
\citet{perlmutter1978}, that is, verbs whose syntactic subject is not
...
ablative function:
%, first without qualification by a preposition, then with the
...
% 	\glc come-\Imp{} \Dir{} at.bottom tree-\Gen{} Niva //
