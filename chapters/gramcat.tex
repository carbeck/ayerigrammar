...
verb\index{verbs}, since the agent\index{semantic role!agent} NP is also topicalized\index{grammatical function!topic}. The verb\index{verbs} also agrees\index{agreement} in person
and number with the agent\index{semantic role!agent} NP by way of the person marker \rayr{/y}{-ya} for
...
\rayr{/yo}{-yo}. The (non-topicalized) NP's case marking\index{case} shows that the agent\index{semantic role!agent} 
...
agreement\index{agreement} on the verb\index{verbs} uses the marker \rayr{s}{sa} according to the NP being
