...
\begin{tabu} to .75\textwidth{S[2] X[4c] X[4c] X[4c]}
\tableheaderfont\toprule
...
\autoref{fig:detpron} gives the declined forms for both proximal and distal. 
...
in that the most frequently used words in a language are usually also tend to be 
the shortest \citep[25--27]{zipf1935}.} A further reason for dropping the 
...
NPs to serves as a indefinite demonstrative:
...
To express `the X one', however, if X is an adjective, it is necessary to use 
the full demonstrative pronoun, since adjectives do not decline otherwise:
