...
With regards to (\ref{ex:lingnn}) it is also necessary to mention 
\citeauthor{hagege2010}'s `Proof by Anachrony Principle' 
...
At the beginning of this section it was shown that prepositions in Ayeri do 
cannot receive number and case marking, which are otherwise typical features of 
nouns. What is possible with regards to affixes, however, is adding degree 
suffixes to prepositions, since these suffixes are clitics selecting phrasal 
heads rather than inflections:
...
% - examples of personal pronouns goverened by pre-/postpositions → treated in 
% analogy to nominal NPs (according to Hagège 2010: 139, this is peculiar 
% crosslinguistically!)
% 
% - examples of other kinds of pronouns governed by pre-/postpositions (e.g. 
% interrogatives)
% 
% - Prep. together with demonstrative prefix possible? → some languages impose 
% limits to defniteness marking of the governed term (e.g. definitness marking 
% only when NP is modified), OTOH, Ayeri doesn't really mark definiteness as 
% such—no restrictions apply to topic marking (which interacts w/ definiteness, 
% however) (Hagège 2010: 142–143) → "As far as goverened terms are concerned, 
% weakly grammaticalized as well as strongly grammaticalized members on the 
% grammaticalization path of noun determiners mentioned at the beginning of the 
% present section, namely demonstratives and noun markers respectively, are not 
% precluded by Adps: only those members on this path that are at the intermediate 
% stage, namely definite articles, are often precluded," ditto for not strongly 
% grammaticalized Adps like relational nouns (Hagège 2010: 143). This means it's 
% not a surprise Ayeri allows Adp. + Dem. + NP
...
\xayr{koNF}{kong}{inside}, \xayr{liNF}{ling}{on}, \xayr{mrinF}{marin}{before}, 
\xayr{mN lug}{manga luga}{through}, \xayr{mNsh}{mangasaha}{towards}, and 
\xayr{pNF}{pang}{behind} come to mind as doubling for `within', `while', 
`during', `in + \emph{time}', and `ago', respectively. Since postpositions are 
not primarily derived from nouns, there are forms dedicated to expressing 
temporal relationships, namely, \xayr{mshtj}{masahatay}{since}, 
...
even more prominent in temporal postpositions in that all of the words listed 
