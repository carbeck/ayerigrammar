...
\ex
	\makebox[7em][l]{\rayr{\larger smNF}{samang}}
		\makebox[7em][l]{$12^{(2-1) \times 4} = 12^{4}$}
		\quad myriad
		
	\makebox[7em][l]{\rayr{\larger kynNF}{kaynang}}
		\makebox[7em][l]{$12^{(3-1) \times 4} = 12^{8}$}
		\quad myriad myriads
		
	\makebox[7em][l]{\rayr{\larger yonNF}{yonang}}
		\makebox[7em][l]{$12^{(4-1) \times 4} = 12^{12}$}
		\quad myriad myriad myriads
		
	\makebox[7em][l]{\rayr{\larger IrinNF}{irinang}}
		\makebox[7em][l]{$12^{(5-1) \times 4} = 12^{16}$}
...
\begin{figure}[tp]\centering
\caption{Simple fractions from ¹⁄₂ to ¹⁄\tsub{\elv}}
\begin{tabu} to .75\linewidth {X[1c] X[2c] X[1c] X[2c]}
\toprule\tableheaderfont
Numeral
	& Word
...
\ex
	\ayr{\larger memlFlnF/hen}\\
	\fw{memallan-hen}\\
	`$\frac{1}{\ten8_{12}}$' (=\,$\frac{1}{128_{10}}$)
\xe

\noindent However, this may become confusing if numerators are used, so %

\ex\ljudge\ques
	\ayr{\larger mmenNF Ito menFlnF/yo kj}\\
	\fw{memenang ito menlan-yo kay}\\
	`$\frac{3}{714_{12}}$' (=\,$\frac{3}{1024_{10}}$)
\xe

\noindent would be expressed less ambiguously as

\ex
	\ayr{\larger menNnF Ito menFlnF/yo nerFnnFyen kj}\\
	\fw{menangan ito menlan-yo nernanyena kay}\\
	`three of the 1\,024th part'
\xe
...
the heads of NPs, this also means that the ordinal numeral forms the head of the 
NP it occurs in, instead of modifying the entity being counted like an ordinal 
numeral does. This is illustrated in (\ref{ex:ord}) below.
...
So far, only single-digit ordinals were used ...
