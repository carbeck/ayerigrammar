...
Examples like (\ref{ex:lingpr}) show the obligatoriness of prepositions in 
Ayeri, since it is not possible to express the relation with a regular noun in 
every context. This is further proof that Ayeri has prepositions rather than 
relational nouns, even though the origin of prepositions as relational nouns is 
still transparent \citep[174]{lehmann2015}. Specialization is also apparent 
from the fact that prepositions in Ayeri, in spite of their nominal origin 
cannot be freely modified by adjectives and relative clauses like regular 
nouns. Thus, while \rayr{AvnF}{avan} as a noun can mean `soil' or `ground' and 
...
% What about degree/measure suffixes? Do they cliticize to the end of the PP? 
% What if an adjective is intervening, e.g. pang natrangya tado=ikan = way 
% behind the old temple or behind the very old temple?

Another telltale sign of grammaticalization, according to 
\citet[159]{lehmann2015} is that of coordination reduction: with increasing 
grammaticalization, the ability to coordinate terms decreases, so that:

\pex
	\a A--X and B--X ≠ [A and B]--X
	\a A--X and A--Y ≠ A--[X and Y]
\xe

[Is this possible in Ayeri?]
...
\xayr{paNF}{pang} come to mind as doubling for `within', `while', `during', 
`in + \emph{time}', and `ago', respectively. Since postpositions are not 
primarily derived from nouns, there are forms dedicated to expressing temporal 
relationships, namely, \xayr{mshtj}{masahatay}{since}, 
