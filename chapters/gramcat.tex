...
compounds\label{loosecomp}), inflection often is deferred to the noun head 
noun instead of the edge of the compound as a whole; the modifier is treated as 
an adjunct in this case, and stays uninflected:
...
series is reguarly marked by the negative suffix \rayr{/Oj}{-oy}, which also 
...
is, the second person of \rayr{silFvF/}{silv-} is not *\rayr{silFvv}{*silvava}, 
as one might expect, but \rayr{silFvFv}{silvva}. A further exception to this 
...
form of \xayr{krFsF/}{kars-}{freeze} is not *\rayr{krFsv}{*karsava} as 
expected, but \xayr{krFsFv}{karsva}{you freeze}.
...
possible for the \rayr{m/}{mə-} prefix to be used to refer to `regular' past 
...
be used, that is, first and foremost the adverb \xayr{Iri}{iri}{already}, which 
...
here and keeps the spelling \ayr{/Oyj} \orth{-oyay} for /-uay/ 
...
also has a verb that generally means `do', namely, \rayr{mir/}{mira-}. However, 
...
particle \rayr{d/}{da-} may be added to the formerly subordinating verb in order 
...
expects their opposite to decline. The negative particle \rayr{voj}{voy} is 
...
and/or quantity. Purely functional adverbs like \xayr{Iroj}{iroy}{not yet} or 
\xayr{mritj}{maritay}{about to} may allow degree adverbs to modify them at 
least.
...
it.\footnote{Conversely, compare the adverbializing suffixes, English \fw{-ly}, 
French \fw{-ment}, for instance.} \rayr{IpnF}{ipan} can thus also be used to 
mean `extremely':
...
\xayr{AreenF}{arēn}{however, anyway} and \xayr{naarY}{nārya}{although, 
though; nevertheless} do all the work. Like adverbs expressing stance, they may 
...
case agreement of the whole CP with the postposition, since there is 
...
\xayr{pnFtY}{panca}{finally, eventually, in the end},
...
\autoref{subsec:dempro}), and indefinite pronouns like \xayr{yaarilF}{yāril}{for 
some reason; somewhere} (see \autoref{subsec:indefpro}), Ayeri also possesses 
...
\xayr{d/}{da-}{such} are unattested. Thus, instead of the hypothetical form with 
\rayr{d/}{da-}, a full-NP adverbial with a generic noun has to be used: 
...
According to \citet{wals131}, the vast majority of the 196 sampled languages in 
this chapter of \citetitle{wals} either count in tens or have a mixed 
vigesimal-decimal system, while only five languages use a different base than 
10. Ayeri, on the other hand, uses a duodecimal system and is thus very 
untypical compared to real-world languages in using a number base other than 
10.\footnote{I chose to use 12 as a numerical base because I simply wanted to 
toy with it. Also, I originally conceived of Ayeri speakers as humanoid but not 
necessarily human, which meant that they would not need to have evolved to have 
five fingers on each hand---for an earlier fictional language of mine, Daléian, 
I used an octal system reasoning that speakers would only have four digits. In 
any case, a duodecimal system could work reasonably well with human hands if 
you counted not only the fingers, but also the hands themselves. 
Finger-counting in Ayeri's duodecimal system would probably be similar to 
counting in the senary system of Nen described in \citet{evans2009} (as quoted 
in \cite{dixon2012}): \textcquote[73--74]{dixon2012}{In counting, Nen speakers 
`first count off the five fingers with a finger of their other hand, and then 
on the sixth they place their counting finger on the inside of the wrist'}. 
Even though they only occur rarely, duodecimal numeral system are not entirely 
unheard of in natural languages, so for instance, \citet{caingair2000} inform 
that in Maldivian, the \textcquote[21]{caingair2000}{decade plus numeral system 
is currently in fashion, but with some remnants of an older system as well. The 
...
are semantic primes, that is, their meanings cannot be readily recognized as 
derived from body parts \citep[74]{dixon2012} or from internal arithmetic like 
9 as `ten lacking one', for instance. The numerals \xayr{Iri}{iri}{five} and 
\xayr{henF}{hen}{eight} may be an exception, since as an adverb, 
\rayr{Iri}{iri} also means `already', which might refer to the fact that a full 
hand has been counted off; \rayr{henF}{hen} as an adverb also means `all'. 
Ayeri also appears extremely sophisticated as far as the upper limit of numeral 
systems is concerned in possessing a way of forming large numerals by a 
...
modifiers are placed after nouns. The full table of cardinal numerals from $0 
\times 12^0$ (0) to $11 \times 12^0$ (\elv) is given in 
\autoref{fig:cardinals}.\footnote{For the sake of typographic simplicity, I 
will 
...
index `12' indicates base 12, respectively.} An example of simple 
modification by a numeral is given in example (\ref{ex:nummod}):
...
suffix \rayr{/lnF}{-lan} to the numbers from 0 to \elv\ are given in 
...
\pex
\a %
	\rayr{\larger menFlnF/men}{menlan-men} (11), \\
...
In order to form yet higher numbers, the suffix \rayr{/nNF}{-nang} is 
appended to numerals: \rayr{menNF}{menang} (←~\xayr{menF}{men}{1} + 
\rayr{/nNF}{-nang}), \xayr{smNF}{samang}{2} (←~\rayr{smF}{sam} + 
\rayr{/nNF}{-nang}), \rayr{kjnNF}{kaynang} (←~\xayr{kj}{kay}{3} + 
\rayr{/nNF}{-nang}), etc. While \rayr{menNF}{menang} is used for 100, the 
numerals of this series following it each multiply by the factor of a 
duodecimal myriad (=~20\,736\tsub{10}). Thus, we get the following series:
...
The numeral which the myriad-multiple word is based on essentially indicates 
...
myriad-multiples serve as unit words in turn, and thus can be modified by 
...
Unit words may also be used as (inanimate) nouns, so it is possible to speak of 
...
going to deal with how to express fractional amounts. The main way to express 
common fractions like $\frac{1}{2}$, $\frac{1}{3}$, $\frac{1}{4}$, etc. is to 
prepend \xayr{menF}{men}{one} to the denominator; the full paradigm for 
$\frac{1}{2}$ to $\frac{1}{\elv}$ is given in \autoref{tab:smallfrac}. Note 
that a number of these fractions are irregular due to assimilation of 
consonant clusters. In order to introduce a numerator, the fraction numeral is 
taken as the base and modified by a regular cardinal numeral, as 
...
Ordinal numerals are formed by nominalization from ordinal numerals. This may 
be another slightly odd strategy, however, it is at least attested in Classical 
...
Unfortunately, neither \citet{chungetal2014} nor \citet{beyer1992} say 
whether Classical Tibetan treats these derived forms as nouns or as numerals, 
or whether it makes that distinction at all. In Ayeri, however, ordinals are 
firmly treated as noun-like nominal elements due to the derivational suffix 
...
for the ordinal numerals from 0 to \elv\ can be found in \autoref{fig:ordinals}.
...
\a\label{ex:ordadj}\begingl
	...
	\glc \Aarg{} arrive-\TsgM{} \Top{} Mahān finish-\Loc{} like=first 
		worthy //
	\glft `Mahān arrives at the finish as worthy first.' //
\endgl
...
references, as far as case marking is concerned. Since ordinals are 
treated as nominals, they can also be modified by both a relative clause, as 
(\ref{ex:ordrel}) shows, as well as an adjective, as shown in 
...
So far, only single-digit ordinals have been used. In order to form higher 
ordinals, the head unit word receives the nominalizer, otherwise the number 
word as such is nominalized with the rest trailing as a modifier. Essentially, 
an ordinal in the `teens'\footnote{More specifically, $a \times 12^1 + b$ with 
$\{a,b \in \textbf{Z} \mid 0 < a,b < 12^1\}$.} behaves like a `tight' noun 
compound, while ordinals involving unit words for powers of 12 higher than 1 
behave as `loose' compounds (compare \autoref{subsubsec:endocomp}, 
p.~\pageref{loosecomp}). 
...
\rayr{kjlnF/miynFlej}{kaylan-miyanley}. This is analogous for such nouns as 
...
Whereas ordinals are derived from cardinal numerals through nominalization, 
multiplicative numerals are derived from ordinals (compare 
\autoref{fig:ordinals}) by putting them in the dative case: the suffix 
\rayr{/ymF}{yam} is added to the ordinal form of the numeral. The resulting 
multiplicative numeral can thus be used as an adverbial to mean `for the $n$th 
time', or as an adverb to mean `$n$ times'. Context helps to disambiguate 
...
Compound numerals are treated analogously to ordinals, that is, for 
composite numerals smaller than $12^2$, the derivational marking is placed 
...
the marking that marks it as a multiplicative numeral while the rest trails 
...
The derivative afffix in this case is the instrumental case marker, 
\rayr{/Eri}{-eri}. Distributive numerals refer to groups of $n$, as example 
(\ref{ex:distnum}) shows.
...
The formation of composite numerals mirrors that of mulitplicative numerals as 
well, in that composite numerals below $12^1$ are treated as single units 
...
Ranges of cardinal numbers are conceptually similar to stretches of way, 
hence they are predestined to be expressed by prepositional phrases, or in 
Ayeri, by any of the cases which can be used for locative purposes (dative, 
genitive, locative). However, Ayeri treats cardinal numerals more like 
adjectives than nouns, so using means of case marking is not possible, while 
it should not be a problem to use adpositions since these take NPs and CPs 
as complements, and thus an adjective should not be problematic as a complement 
either, since it is embedded in an NP. Thus, ranges of cardinal numbers may be 
expressed using the postposition \xayr{pesnF}{pesan}{(up) until}. When counting 
starts at \xayr{menF}{men}{one}, this numeral may be dropped, like in English 
`count to ten' instead of `count from one to ten'.
...
Since ordinal numerals, however, are treated as nouns, they may receive case 
marking, which means that, in contrast to cardinal numerals, it is possible to 
express a range using a combination of the genitive and the dative case, or 
again \rayr{pesnF}{pesan} with its prepositional object in the locative case.
...
placed in between the conjuncts, and works on any syntactic level. Namely, it 
...
equally works at all levels of syntax---lexical heads, phrases, and clauses. 
Inclusive and exclusive `or' are not formally distinguished in Ayeri by the 
disjunction \rayr{soyNF}{soyang} alone, so context is necessary to 
contrast between them. Alternatively, a construction akin to English `either 
... or' may be used to make the distinction explicit (see 
\autoref{subsec:corrconj}).
...
\xayr{le—gli}{le ... gali}{juice}, which together form the object complement of 
...
`both ... and': the compound construction emphasizes that two options are 
...
The example in (\ref{ex:bothand}) is literally `I like grains and beans 
equally', with two NPs in alternation and objects of a transitive verb, 
\xayr{vtYF/}{vac-}{like}. With predicative adjectives, the verb 
\xayr{km/}{kama-}{(be) equal} may be used:
...
\xayr{/ENF}{-eng}{more, rather} or its opposite, \xayr{/kj}{-kay}{less, little}:
...
not both---that is, logical \textsc{xor}---is expressed by the 
construction \xayr{kmo soyNF}{kamo ... soyang}{equally ... 
or}, as illustrated by (\ref{ex:eitheror}).\footnote{Interestingly, it looks as 
though I slightly plagiarized English here in idea, albeit unwittingly: the 
etymology of \fw{either} is given as being from Old English \fw{ǽghwæðer, 
ǽgðer}, from Germanic \fw{*aiwon} `always' + \fw{*gihwaþaro-z} `each of two', 
cf. \citet[either, adj. (and pron.) and adv. (and conj.)]{oed}. On the other 
hand, the collective wisdom of the internet's fictional-language community 
holds that one cannot truly innovate grammatical structures; there will always 
be a natural language which evolved a given construction before, and possibly 
with more complications involved. This situation is referred to as 
`\textsc{anadew}', an acronym for `a nat[ural~]lang[uage] already did even 
worse' or something along these lines.} For its negative opposite, `neither ... 
nor', negation must be used, which is displayed in (\ref{ex:neithernor}).
...
`Conjunctive adverbs' here refers to adverbs which have the distribution of a 
conjunction. Whereas adverbs are normally placed either after the verb or at 
the end of a clause (compare \autoref{sec:adverbs}), these words are usually 
found as introducing clauses as they connect two otherwise independent 
statements to show their relation to each other. Their meaning extends that of 
the `pure', logical conjunctions \xayr{nj}{nay}{and} and 
\xayr{soyNF}{soyang}{or}, however.\footnote{Logical `not' is usually expressed 
by a negative suffix on the adjective or the verb, compare sections 
\ref{subsec:adjneg} and \ref{subsubsec:verbneg}, respectively.}
Part of this small class of words are the expressions
\xayr{bt}{bata}{if, whether},\footnote{Conditional protasis and apodosis are 
usually unmarked in Ayeri, however, it may still be desirable occasionally to 
use a particle to indicate them explicitly.}
...
lack a verb to apply verb morphology to. The conjunctive adverb thus fulfills, 
as a substitute, the most important distinctions otherwise expressed by the 
verb.
...
\a\label{ex:butnot}\begingl
	\gla Yang temisena cuyam, nāroy yāng petau. //
	\glb Yang temis-ena cuyam nāroy yāng petau //
...
reduplication of the verb expresses iterative aspect, which expresses `X again' 
and `X back', depending on context. The reduplicated form 
...
*\rayr{k/kdaare}{*ka-kadāre}. As a conjunction, however, the is highly 
frequent, so it does not seem odd that it has assumed a phonologically more 
simple, yet recognizable form. The conjunctive adverb in (\ref{ex:butnot}) 
