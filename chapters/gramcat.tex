...
regularly feature in the noun phrase (NP) or in combination with it---pronouns,
...
grammatical properties. Besides common nouns, there are also proper
nouns (that is, names) and nominalizations. Nouns, as the heads of NPs\index{phrase types!noun phrase}, are
assigned case by the verb, so case is a third grammatical property
nouns display. For an illustration of the declension paradigms, compare Tables
...
marking; verbs, then, agree\index{agreement} with agent NPs in number in the canonical case. 
...
be reduced to [ʤ] \fw{-j} before case\index{case} suffixes\index{suffixes} beginning with /j/ or with a
...
like \xayr{—/En menF}{\dots-ena men}{one of~\dots} (\dots-\Gen{} one), like in
(\ref{ex:dualplural}).
...
are derived from regular words in the language. For instance, the
...
\a\begingl
	\gla Ang @ apayen sa @ Yan-ikan. //
	\glb ang= apa=yen.Ø sa= Yan=ikan //
	\glc \AgtT{}= laugh=\TplF{}.\Top{} \Parg{}= Yan=all //
...
(Tables \ref{tab:anideclcons}--\ref{tab:inandeclvow}), Ayeri's NPs\index{phrase
types!noun phrase} are marked for case, which is governed by the
verb\index{verbs} or assigned to adjuncts\index{grammatical function!adjunct}
freely depending on their purpose or meaning. Notably, in Ayeri, case marking is
at least partially based on semantics rather than purely on function or
structure. This causes a few exceptions, so it is better, in my opinion, not to
use the classic labels of nominative (S/A) and accusative (O), or of absolutive
(S/P) and ergative (O) for the first two core roles. Instead, I will be using
the terms `agent' and `patient', which I hope brings about some more clarity,
especially when discussing the mentioned exceptions later on. For a discussion
of how Ayeri deals with subjecthood, see \autoref{subsec:subjecthood}.
...
\posscite{fillmore1968} caveat. However, importantly, `agent' in Ayeri is a
macrorole that may be applied to, for instance, instruments\index{semantic role!instrument}, experiencers, and
less typical actors as well, especially, in absence of more prototypical
...
the suffix\index{suffixes} \rayr{/reNF}{-reng} for inanimate\index{animacy} referents; names\index{nouns!proper} and verbal topic\index{grammatical function!topic}
...
compare (\ref{ex:ditragt}). Moreover, as (\ref{ex:causagt}) shows, the causees\index{causation}
are marked as agents\index{semantic role!agent}, not as a patients, since that would be semantically
incongrouous.
...
\rayr{/lej}{-ley}; for names\index{nouns!proper} and verbal topic\index{grammatical function!topic} agreement\index{agreement}, the markers are
...
Example (\ref{ex:agtnotnom}) shows that what changes in passivization is the
verb: it now agrees\index{agreement} with the next argument in line: the patient
NP\index{phrase types!noun phrase}. Scrambling the order of NPs does not make
the verb\index{verbs} simply agree with whatever follows it, as illustrated in
(\ref{ex:verbscram}). However, it can nonetheless be assumed that verb
agreement\index{agreement} in Ayeri historically
developed\index{grammaticalization} along these lines, which will become
especially apparent in the discussion of pronouns.\footnote{Mismatches in
agreement\index{agreement} in connection to scrambling such as exemplified by
(\ref{ex:scramfalse}) are to be expected. \citet{corbett2006}, notes
that with regards to agreement\index{agreement} in NP\index{phrase types!noun
phrase} conjuncts, \textcquote[62]{corbett2006}{distant
agreement\index{agreement} is rare, and that agreement\index{agreement} with the
nearest NP\index{phrase types!noun phrase} or agreement\index{agreement} with
all (resolution\index{resolution}) is much more common}.
% If there were an extensive corpus of texts written by Ayeri speakers, it might
% be interesting to gather statistics on the number of words between target and
% controller in relation to the prevalence of agreement\index{agreement}
% mismatches.
}
...
animate and inanimate\index{animacy} entities. Names\index{nouns!proper} and verbal topic\index{grammatical function!topic} agreement\index{agreement} are marked
...
dative NP\index{phrase types!noun phrase} is fronted to occur after\index{constituent order} the verb in contrast to regular recipients,
...
The genitive is used to mark possessors; attributive genitive NPs follow\index{constituent order} the
...
Tables \ref{tab:anideclcons}--\ref{tab:inandeclvow} above. Names\index{nouns!proper} and verbal
...
possession. Apart from possession, the genitive can also be used to
...
verb. It is also the case in which adpositional objects\index{grammatical function!adpositional object} normally appear,
...
\rayr{/yey}{-jya}.} names\index{nouns!proper} and verbal topic\index{grammatical function!topic} agreement\index{agreement} use the marker 
...
the adpositional object\index{grammatical function!adpositional object} is marked for locative case, as we see in
...
agreement\index{agreement} with causer NPs\index{phrase types!noun phrase}, even though their action logically supersedes or
...
verbal topic\index{grammatical function!topic} agreement\index{agreement} use the marker \rayr{saa}{sā}. As stated above, verbs do
...
when ending in a consonant; names\index{nouns!proper} and verbal topic\index{grammatical function!topic} agreement\index{agreement} are marked by
...
The instrumental may also be used for cases where the instrumental
NP\index{phrase types!noun phrase} acts as a nominal
complement\index{grammatical function!closed complement} describing an
attribute of its antecedent head, as in (\ref{ex:nounadjc}). Here,
\xayr{bri}{bari}{meat} is marked as an instrumental since it serves as an
attribute of \xayr{ksu}{kasu}{basket}. The instrumental NP\index{phrase
types!noun phrase} describes what its antecedent contains or entails more
specifically: it is a basket \fw{with} meat in it. Note, however, that this use
of the instrumental is different from expressing accompaniment. Thus, it is not
possible to use the sentence in (\ref{ex:wrongcomit}) to express `Ajān comes
(together) with Pila'.
...
\begin{figure}
\ex\label{ex:viains}\ljudge\ques
\begingl
...
Case morphology is applied to nouns in Ayeri basically whenever NPs\index{phrase types!noun phrase} serve as
...
(\ref{ex:vocnoun}) and \rayr{dirsF}{Diras} in (\ref{ex:vocname}) would be the implied agents\index{semantic role!agent}
...
addressees occur as appositions. Another case where nouns are not
...
in \autoref{subsec:clitics} (p.~\pageref{clitics_prenoun_ku}\,ff.), it behaves in
the way of a special clitic in \posscite{zwicky1977} terminology, since no
corresponding full form exists in its place. Example (\ref{ex:kucasemkd}) 
...
NP when a proper noun is marked by a case-marking particle. Example
...
\citet{gaeta2008}, then, adds to \posscite{bauer2001} research, based on a
...
syntax}. Secondly, \textcquote[135]{gaeta2008}{[s]yntax seems to be
...
`harmonic' if it follows the normal constituent order\index{word order} of the language and
...
with adjectives\index{adjectives} are interesting in that this is also the
normal order\index{word order} for free adjectives\index{adjectives}. To
illustrate, some tests will be necessary to show that these
adjectives\index{adjectives} form a unit with the head noun and are unable to
undergo comparison, for instance.

Synthetic\index{synthesis} compounds exist in Ayeri and produce nouns. These are
compounds in which \textcquote[701]{bauer2001}{the modifying element in the
compound is (usually) interpreted as an argument of the verb from which the head
is derived}. There are also a number of coordinative compounds. This group,
however, is lexicalized and not productive. Exocentric compounds constitute the
minority of the sample. In the following, I will give examples for each type. It
needs to be noted as well that unlike Germanic languages, Ayeri does not allow
compounds of arbitrary length to be strung together, like in the ridiculous but
no less real example from (former) German\index{German} legislation in
(\ref{ex:REUeAUeG}).
...
modified by a dependent noun. Ayeri exhibits rather strict head-initial
word order\index{word order}, so it comes as no surprise, following \citet{gaeta2008}, that most of
these compounds follow this order\index{word order} as strictly: the second noun modifies the first,
...
modifier and head are various: a grape is a berry \emph{used} to 
...
as though it could have plausibly been a noun once. This noun 
may have been grammaticalized\index{grammaticalization} into a reflexive morpheme of a more 
...
causatives\index{causation} of a number of verbs. Besides these irregularities, there is also at
...
the constituent order\index{word order}, here that of verbs and nouns: Ayeri is a VO language,
and thus, the verb\index{verbs}, as the head of the compound, is usually found on the left side
with its nominal modifier following\index{word order} it \citep[129--133]{gaeta2008},
...
well, since verbs always come first\index{constituent order} in Ayeri sentences, and any other NPs,
...
combinations is also questionable, since they all appear to be formed with
...
made or intended for spreading on a surface. A few more such verbal derivations\index{derivation}
can be found, though not compounds, in those words listed in
(\ref{ex:yamderiv}) among others.
...
Neither of the two elements recognizably forms the head in these examples, but
both are typical components of the thing the compound signifies.
\citet{bauer2001} mentions that coordinative adjective\index{adjectives}
compounds are rare, or at least rarely documented in the grammars he surveyed
\citet[699]{bauer2001}. In our sample, only the compound in
(\ref{ex:adjadjcomp}) is included. This compound forms a noun from the
combination of two adjectives\index{adjectives}, insofar it is relevant to this
section even though the component parts are not nouns.
...
it is not a very productive group.
...
this is a superstition that you have too much blood in your head, which is
...
constituent parts.\footnote{This situation probably arose either because I
tweaked the constituent so much as to not be readily recognizable anymore, or
because I forgot to make an entry in the dictionary, or even deleted or changed
it.} The words in question are listed in (\ref{ex:mystcomp}).
...
modifier, and *\rayr{vetj}{vetay} may have once been intended to mean `money' or
`fee' or something along these lines. Digging through old archives and
translations, it could be determined at least that \rayr{bkrF}{bakar} once
must have meant `lie', and that \rayr{terj}{teray} was indeed a word for `sad'.
...
above, so does Ayeri. A few examples of diminutive\index{diminutive}
...
Ayeri's diminutive\index{diminutive} reduplication involves doubling the noun root of a word. Besides the
...
marked as an agent in that the agent\index{case} suffix\index{suffixes} \rayr{/ANF}{-ang} is appended to
...
*\rayr{veneyNF/veneyNF}{*veneyang-veneyang}. Likewise, the reduplicated
form is not treated in the way of an endocentric compound\index{compounds}, so case\index{case} and
plural\index{number!plural} marking cannot be appended to the first element: *\rayr{veneyNF
...
Example (\ref{ex:compredup}) displays the simple case of a transparent endocentric 
...
few. Example (\ref{ex:nn-nn}) provides instances of such renominalizations.
...
\rayr{/ANF}{-ang}, though they are irregular and non-productive; compare 
(\ref{ex:nounamderiv}) and (\ref{ex:nounangderiv}). The \rayr{/AmF}{-am}
derivations in (\ref{ex:nounamderiv}) seem to have a connotation of being
tools used for the action they derive from; the \rayr{/ANF}{-ang} derivations
listed seem to derive a more abstract related term.
...
Besides agentive\index{semantic role!agent} \rayr{/my}{-maya} and \rayr{/vy}{-vaya}, there is also a derivational suffix\index{suffixes} for makers of
things, \rayr{/Ati}{-ati} (contracting\index{morphophonology} to \rayr{/AtYF/}{-ac-} before a vowel).
This suffix\index{suffixes} is not too productive, and sometimes irregular, as
...
\ex\labels\label{ex:toolderiv}
	...
	\xayr{\larger npF/}{nap-}{burn}
		...
		\\
	\xayr{\larger pFr/}{pra-}{glitter, gleam}
...
\citep[35]{payne1997}. As we have seen at the beginning of this section,
Ayeri can derive \xayr{vehnF}{vehan}{building, construction}
...
In order to illustrate properties of derived nouns in Ayeri,
the examples in (\ref{ex:vbnomz}) condense several properties into one.
For instance, (\ref{ex:nomz-sbj-adj}) shows that \rayr{vehaanF}
...
nouns from the participle\index{participle} of a verb describing the action as such---a gerund.
Example (\ref{ex:kafkagerund}) again draws on the Ayeri translation of Kafka's short
...
gerund derivation \xayr{hruymnF}{haruyaman}{beating}: I wrote there that
\fw{haruyaman} `beating' was used instead of \fw{haruan} `beat(ing)' in order to
emphasize the process of beating as an incomplete action 
\citep[14--15]{becker:kafka:imperial}.
...
The participle\index{participle} marker in Ayeri has possibly been
grammaticalized\index{grammaticalization} from the dative
case\index{case!dative} marker, or it is at least synchronically
homonymous. In order for case\index{case} marking to operate, this formation has
to be nominalized, which is done in the usual way by appending \rayr{/AnF}{-an},
thus yielding the suffix\index{suffixes} cluster \rayr{/ymnF}{-yaman} for the
derivation of verbs as gerunds. If the gerund is marked for dative
case\index{case!dative}, the suffix\index{suffixes} cluster
*\rayr{/ymnFymF}{*-yamanyam} undergoes haplology to a simple nominalized form
with the suffix\index{suffixes} cluster \rayr{/AnFymF}{-anyam}. See
...
passage \rayr{pluNFymnF — n byokivo}{palungyaman \dots na Bayokivo} forms the topic
...
Looking at previous examples of gerunds, we can deduce that gerunds in Ayeri do 
not behave like transitive verbs as in English\index{English}. Thus, what would be the object
of the former verb appears in the genitive case\index{case!genitive} in Ayeri. As in English\index{English},
however, gerunds in Ayeri cannot be pluralized\index{number!plural}; compare (\ref{ex:grndplur}).
On the other hand, it is possible to quantify\index{quantifiers} gerunds, as
well as to modify them by possessors\index{semantic role!possessor}. The two
sentences in (\ref{ex:grndmod}) exemplify these uses.
...
reflexive and reciprocal expressions. In the following, each class of pronouns
will be discussed regarding its morphological properties.
...
person, \rayr{/tF/}{\mbox{-t-}} to the pronoun stem, which can still easily be 
...
instance, the use of [\dots]\ English \fw{you} for the second person singular}
...
counterparts would be, also in the non-core cases\index{case}. Example (\ref{ex:pronfull}) shows a
sentence with full subject and object NPs; example (\ref{ex:pronagt}) shows a variation
...
distribution\index{complementary distribution} with pronouns\index{phrase types!determiner phrase}, since pronouns are anaphoras for NPs\index{phrase types!noun phrase}. Thus also, an
NP\index{phrase types!noun phrase} which contains an adjective is wholly replaced by a personal-pronoun determiner phrase 
(DP), as in (\ref{ex:procompldist}).
...
also the ones showing as verb agreement\index{agreement}. An important difference in this 
respect, however, is that the third person singular inanimate\index{animacy} verb agreement\index{agreement} 
...
Another important property of pronouns is that agent\index{case!agent} pronouns
(and patient\index{case!patient} pronouns under certain circumstances) replace person agreement\index{agreement} by
cliticizing\index{clitics} to the verb\index{verbs} stem. Person agreement\index{agreement} morphology is a domain of
verbs and will be dealt with in more detail in \autoref{subsec:persnumagr}.
...
Possessive pronouns use the genitive forms in \autoref{tab:perspro}. They are
special compared to regular personal pronouns in that,
like adjectives\index{adjectives}, they need \rayr{d/}{da-} as a supporting particle in order to
stand alone. The main use for the genitive\index{case!genitive} pronouns as listed in \autoref{tab:perspro} is
...
(\ref{ex:procompldist}). Instead, they are used as modifiers, as
(\ref{ex:adjgen}) shows, which makes them superficially similar to adjectives.
...
*\xayr{naa/vaa}{*nā-vā}{*myest}). Fronting\index{constituent order} them in predicative\index{grammatical function!predicative complement} statements like
...
\footnotetext{This can be translated more literally as `That is the one of the 
closed boa snake'.}
}
...
\rayr{AdreNF}{adareng} as the dummy agent\index{semantic role!agent} in predicative\index{grammatical function!predicative complement} contexts over the
...
As nominal elements they are declined for case\index{case}---but not for number\index{number}, 
...
\a\begingl
	...
	\glc look-\TsgN{} like=happy=so one-\Aarg{} little //
...
\a\label{ex:danyatop}\begingl
	...
	\glc \PatT{}= want=\Fsg{}.\Aarg{} one-\Top{} red //
...
\ex\label{ex:redone}\begingl
	...
	\glc \PatT{}= want=\Fsg{}.\Aarg{} one=red-\Top{} //
	\glft `I want the red one.' //
\endgl\xe
\end{figure}
...
Interrogative pronouns are all formed with \rayr{si/}{si-}, combined with a
lexical element; \rayr{si/}{si-} is also related to the relativizer
\rayr{si}{si}. The interrogative pronouns are listed in
\autoref{tab:interpro}. All interrogative pronouns share the property of being
placed \fw{in situ}: they appear in the same position as the phrase they stand
in for, so there will not be movement of the question word to the front as in
English\index{English}. Additionally, impersonal interrogative pronouns cannot
be topicalized\index{grammatical function!topic} since they do not inflect for
case\index{case}, which preempts the difference between zero-marked
topicalized\index{grammatical function!topic} and overtly case-marked
untopicalized\index{grammatical function!topic} forms. This is illustrated in
(\ref{ex:qprondist}).
...
Thus, there is also no dedicated question word for `why', since one
...
From \posscite{haspelmath1997} concluding statistics it looks as though there is
...
inanimates\index{animacy}, similar to the distinction in the demonstrative pronouns between 
...
negative\index{negation} series is reguarly marked by the negative\index{negation} suffix \rayr{/Oj}{-oy},
...
generalize from this \citep[2--3]{haspelmath1997}.\footnote{The example sentences appear here
...
relative clause. If word material is intervening, as in
...
left. Relative pronouns do not agree\index{agreement} in number\index{number} with their heads, and in
...
ungrammatically, forms the controller of topic\index{grammatical function!topic} agreement\index{agreement} on the verb in the 
...
the verb agrees\index{agreement} accordingly. There is no resumptive agent pronoun here either,
...
so is verb agreement\index{agreement} with the declined relative pronoun. Similarly, in
...
transitive\index{verbs!transitive} clauses---agents\index{semantic role!agent} and patients\index{semantic role!patient}---cannot precede\index{constituent order} the embedded verb of
...
agents\index{case!agent} or patients\index{case!patient}, and neither can it stand in directly as the agent\index{case!agent} or
patient\index{case!patient} NP\index{phrase types!noun phrase} of the relative clause. It is interesting in this
regard that Ayeri \emph{does}, however, allow this for recipients\index{semantic role!recipient}, maybe since
by their nature as goals\index{semantic role!goal} they carry something of a locative\index{case!locative} connotation
(compare (\ref{ex:reldat})). This would make them less tightly integrated with verbs,
occupying a middle ground between core arguments and adverbials like the
locative\index{case!locative} proper.\footnote{It would be interesting to explore this in terms of
...
marker. In this respect, it would be necessary as well to explore whether the
...
pronominal suffix as in (\ref{ex:sitangquant}). Appending a quantifier\index{quantifiers} directly
to the conjugated verb\index{verbs} itself can be ambiguous; consider
(\ref{ex:nositangquant}). It appears that \rayr{sitNF}{sitang} does not act as
the controller of the verbal topic\index{grammatical function!topic} marker, however. This is illustrated also by
...
compound follows\index{word order} the conjugated verb and its adjuncts\index{grammatical function!adjunct}, as in example (\ref{ex:sitangqtorder}).
...
of comparison, Ayeri uses enclitics\index{clitics} on adjectives. The markers involved
...
comparison, \xayr{tiNFrti\_asF bnF/ENF}{tingracas ban-eng}{a better musician},
...
negation, and to \fw{not} for pragmatic negation. \rayr{/Oj}{-oy} absorbs the final
...
Adjectives can also be derived from verbs with the causative\index{case!causative} suffix\index{suffixes} 
\rayr{/Is}{-isa}, which often correspond to adjectives derived from the 
...
Adpositions are one of the few parts of speech whose stem itself does not 
inflect. Ayeri's most basic adpositions are derived from relational nouns like
\fw{top}, \fw{front}, \fw{bottom}. This is also likely the reason why Ayeri 
mostly employs prepositions\index{adpositions!prepositions}, with postpositions
and ambipositions being less important placement patterns
\parencites[110--111]{hagege2010}[81\psqq]{lehmann2015}.

Adpositions in their most basic use trigger locative marking on the governed
NP\index{phrase types!noun phrase}, the adpositional object\index{grammatical
function!adpositional object}.\footnote{For allative and ablative meanings, an
NP may also appear in the dative and the genitive, respectively, though without
being governed by an adposition, as described in
\autoref{subsubsec:dative}. Also compare \autoref{phsec:lingavandat} (p.~\pageref{phsec:lingavandat}~ff).}
The conceptual metaphor \textsc{time equals space}, with the future\index{tense!future} conceptualized 
...
non-preposition meaning is still transparent, they are still rather complex in
their phonology, and some of them are even polysyllabic in spite of not being
...
automatization of neuro-motor sequences [\dots]. Such reductions are systematic
...
With regards to (\ref{ex:lingnn}), it is worth to mention
\posscite{hagege2010} `Proof by Anachrony Principle'
...
notably not the case in Ayeri, where something like (\ref{ex:behindback1}) is
not possible. In this example, \rayr{pNF}{pang} is used in both of its meanings,
so that the preposition \xayr{pNF}{pang}{behind} governs the original noun
\xayr{pNF}{pang}{back}.
...
Examples like (\ref{ex:lingpr}) show that there is a tendency in Ayeri towards 
grammaticalization\index{grammaticalization} of nouns which used
...
\a \xayr{\larger tpYF/ djrinF}{tapy- dayrin}{save (valuable assets)} (put
	aside)
\a \xayr{\larger tpYF/ midj}{tapy- miday}{put on} (put around)
\a \xayr{\larger tur/ mNsh}{tura- mangasaha}{forward} (send towards)
\xe
\end{figure}

The verbs listed in (\ref{ex:particleverbs}) do not govern a prepositional object\index{grammatical function!adpositional object} in the locative case in their
idiomatic meaning, as displayed by (\ref{ex:mgsrobjdep}), in which
\rayr{btNimnF}{batangiman} and \rayr{s AgYaanF}{sa Ajān} do not serve as
arguments of \rayr{lMtYo}{lanco} or \rayr{mNsr}{mangasara}, but as arguments of the phrasal
verb \rayr{lMtF/ mNsr}{lant- mangasara}. Colloquially,
\rayr{mNsh}{mangasaha} and \rayr{mNsr}{mangasara} may also be shortened to just
\rayr{sh}{saha} and \rayr{sr}{sara}.
...
verb is used in Ayeri for both the plain English\index{English} verb and the one extended by a
...
clauses\index{relative clause}. Generally, it is possible for them to be hosts of quantifier\index{quantifiers} clitics\index{clitics} 
...
\a\label{ex:kongtemp}\begingl
	\gla Ang @ mirāyn kong bihanya sam. //
	\glb ang= mira=ayn.Ø kong bihan-ya sam //
	\glc \AgtT{}= do=\Fpl{}.\Top{} inside week-\Loc{} two //
	\glft `We will do it within two weeks.' //
\endgl

\a\label{ex:mgshtemp}\begingl
	\gla Ang @ girenja mangasaha pidimya-kay. //
	\glb ang= girend=ya mangasaha pidim-ya=kay //
	\glc \AgtT{}= arrive=\TsgM{}.\Top{} towards hour-\Loc{}=few //
	\glft `He will arrive in a few hours.' //
\endgl

\a\label{ex:lingtemp}\begingl
	\gla Ang @ layaye-ikan {} @ Pila ling yeng pakur. //
	\glb ang= laya-ye=ikan Ø= Pila ling yeng pakur //
	\glc \AgtT{}= read-\TsgF{}=much \Top{}= Pila on \TsgF{}.\Aarg{} sick //
...
idiomatic. While the prepositions\index{adpositions!prepositions} in these two examples each take an NP
...
the verb phrase (VP) is often also marked with a clitic indicating the topic\index{grammatical function!topic} of the
sentence and the topic\index{grammatical function!topic} NP's role in Ayeri's case system, which can be
...
\autoref{tab:biconsconj} for the verb \xayr{AnFlF/}{anl-}{bring}. The second-person
conjugation of this verb is not *\rayr{AnFlFv}{*anlva} because the cluster
\fw{-nlv-} is illegal, but rather \rayr{AnFlv}{anlava}. Since Ayeri treats two
...
expect, but \rayr{silFv̔}{silvva}. A further exception to this are verbs ending
in -Cs, since -Cs-C- is commonly resyllabified as -C-sC- (see
...
following discussion. Unlike English\index{English}, Ayeri does not use agent\index{case!agent} pronouns\index{pronouns!personal} in
...
there is only anaphoric reference to \rayr{AgYaanF}{Ajān}; the full agent\index{case!agent} NP is
...
pronoun\index{pronouns!personal} involved at all, which is why the person suffix\index{suffixes} in (\ref{ex:vbclt}) was
...
we need to consider, namely, that the topic\index{grammatical function!topic} morpheme on NPs is zero.
That is, the absence of overt case\index{case} marking on a nominal element indicates that
it is a topic\index{grammatical function!topic}; the verb in turn marks the case\index{case} of the topicalized\index{grammatical function!topic} NP with a
...
independent morpheme.\footnote{Also, perhaps a little untypically, topic\index{grammatical function!topic} NPs in
Ayeri are not usually pulled to the front\index{constituent order} of the phrase (at least not in the
written language; see \cite[120--122]{lehmann2015}), so topic-marked\index{grammatical function!topic} pronouns\index{pronouns!personal}
stay \fw{in-situ}. Which NP constitutes the topic\index{grammatical function!topic} of the phrase is marked on
...
(\ref{ex:protop}) may also be used as a free pronoun\index{pronouns!personal} in equative statements
...
verbs, the assumption so far has been that they are clitics,
especially since the marking strategy displayed in (\ref{ex:passive}) is the
grammatical one in absence of an agent\index{case!agent} NP (compare
...
agreement with the agent\index{case!agent} on the one hand, and agent\index{case!agent} or patient\index{case!patient} pronouns\index{pronouns!personal} just
stacked onto the verb stem on the other. For an overview, compare
...
NP dependent. Also consider (b) in the type~3 transitive\index{verbs!transitive}
cell. The question for this example is whether it should not better be analyzed as
\AgtT{}=\dots-\TsgM{}.\Top{} \dots-\Top{} \dots-\Parg{}, with co-indexing of the topic\index{grammatical function!topic} on 
...
Examples (transitive)
	& \fw{sa}$_i$ \fw{\dots=yāng \dots-Ø}$_i$\newline
		\PatT{}=\dots=\TsgM{}.\Aarg{} \dots-\Top{}
	%
	& \fw{ang}$_i$ \fw{\dots=ya.Ø}$_i$ \fw{\dots-as}\newline
		\AgtT{}=\dots=\TsgM{}.\Top{} \dots-\Parg{}
	%
	& \begin{tabu} to \linewidth {@{} l @{\quad} l @{}}
		a.	& \fw{ang}$_i$ \fw{\dots-ya}$_i$ \fw{\dots-Ø}$_i$ \fw{\dots-as} \\
			& \AgtT{}=\dots-\TsgM{} \dots-\Top{} \dots-\Parg{} \medskip
		\\
		
		b.	& \fw{sa}$_i$ \fw{\dots-ya}$_j$ \fw{\dots-ang}$_j$ \fw{\dots-Ø}$_i$ \\
			& \PatT{}=\dots-\TsgM{} \dots-\Aarg{} \dots-\Top{}
		\\
	\end{tabu}
\\
...
sentence, and additional NPs are optional. If pronominal affixes are
the primary arguments, then they agree in the way that anaphoric pronouns agree
[\dots]. In terms of morphology, pronominal affixes are bound to the verb; typically
they are obligatory}

This seems to be exactly what is going on, for instance, in
...
\citep[45--46]{comrie1989}.\footnote{The topic\index{grammatical function!topic} NP marked on the verb may be
...
one constituent, that is, the agent\index{case!agent} NP. Topic\index{grammatical function!topic} marking should, in my opinion, be
...
resembles the emphatic pronoun and derives from it}, Ayeri does the opposite at least in
part and uses the case-unmarked\index{case} form of personal pronouns\index{pronouns!personal} for
...
NPs)---not topics\index{grammatical function!topic} as such. Ayeri, thus, has subject\index{grammatical function!subject} agreement. Agreement with a
patient\index{case!patient} NP may seem a little counterintuitive, but is licensed by Ayeri's
...
subject\index{grammatical function!subject} function. Formally, also, agent NPs usually follow\index{constituent order} the verb, and it
...
can agree with patients\index{semantic role!patient} as well if the agent NP is absent. Taking into account
that the grammaticalization\index{grammaticalization} process is still ongoing
also makes this seem less strange---there is still some relative freedom in how
morphemes may be used if a paradigm has not yet fully settled
\citep[148--150]{lehmann2015}. Formally, thus, verbs simply become agreement
targets of the closest semantically plausible nominal constituent which can
serve as a subject\index{grammatical function!subject}.
...
Syntax:
	& \multicolumn{1}{c|}{non-argument}
	& \multicolumn{2}{c}{argument}
\\
...
Morphology:
	& \multicolumn{2}{c|}{inflectional form}
	& \multicolumn{1}{c}{free form}
\\
...
in \autoref{tab:persinfltypes} above, it becomes evident that type~1 definitely
fulfills these criteria, and type~2 does so as well, in fact, in that there is
no agent\index{case!agent} NP that could serve as a controller if the verb inflection in type~2
...
suffix\index{suffixes} in column 3 needs support from an NP in the same clause as a source of
...
Since person marking of the types 1 and 2 is \emph{referential}, as shown in
(\ref{ex:agrctrl}ab), it is best counted as
consisting of cliticized pronouns\index{pronouns!personal} \citep[103]{corbett2006}. Since mere
agreement as in type~3 needs support from an NP within the verb's scope\index{scope},
...
semantic features of the clause's subject\index{grammatical function!subject}; it requires a subject\index{grammatical function!subject} NP to exist
concurrently.

As for \posscite{corbett2006} \emph{balance of information} criterion,
...
indicated by the first two inflection types that are not expressed by NPs. 
Under a very close understanding of \citet{corbett2006}, though,
...
by the NP. \citet{corbett2006} writes that in the world's languages, this feature
...
subject\index{grammatical function!subject} NP is omitted, but plural\index{number!plural} number\index{number} still surfaces in the agreement suffix\index{suffixes}
...
agreement is therefore with the whole agent\index{case!agent} NP rather than just with
...
However, verb agreement with a free pronoun\index{pronouns!personal} is also not possible even though it
...
see \autoref{clitics_postverb_person} (p.~\pageref{clitics_postverb_person})
...
that pronouns\index{pronouns!personal} are not allowed as agreement controllers as one might expect, but
only properly nominal NPs. Information on agreement with committee nouns and
coordinated NPs with incongruent agreement features can be found in
\autoref{subsubsec:resolution} (p.~\pageref{subsubsec:resolution}).
...
were auxiliary verbs\index{grammaticalization}. Ayeri applies head-initial word order\index{constituent order} to subordinating
...
The quote in (\ref{ex:neuromancer}) is the famous opening line of
...
but is clearly set in a future world.\footnote{\citet{christian2017} reports that Gibson
...
\textcquote[113]{dahl1985}{[m]ore common than marking narrative contexts [\dots]
...
adverbials as \xayr{tsel}{tasela}{tomorrow}, \xayr{mNsh perikYnFy}{mangasaha
pericanya}{in a year}, \xayr{metj}{metay}{sometime}, or \xayr{bihnY
mrrY}{bihanya mararya}{next week}, as in (\ref{ex:futadvbl}). It is possible as
well to explicitly mark the verb for future tense, for example, to make a
promise, or to otherwise emphasize that the future condition will come to pass,
as illustrated in (\ref{ex:futemph}).
...
verb for past time, in difference to the common lack of morphological
...
languages in \posscite{dahl1985} sample. Typical of progressives, this form of
the verb is not limited to present\index{tense!present} contexts, as exemplified in
...
Ignoring the constructedness of the examples in (\ref{ex:nonpresprog}), the time adverb\index{adverbs} is located 
in the relative clause in both sentences. For illustrative 
...
\rayr{mN m/—}{manga mə-\dots} (\Prog{}=\Pst{}-\dots) are possible. Strictly 
...
Unlike the few instances of habitual marking in \posscite{dahl1985} survey
\citep[96]{dahl1985}, Ayeri possesses a suffix\index{suffixes} for marking such actions on
the verb: \rayr{/As}{-asa}, where the first \fw{-a} replaces the terminal vowel
of a verb stem if present, compare example (\ref{ex:habvwl}).\index{morphophonology} The
habitual aspect stresses that an action is carried out as a habit,
that is, not just a few times, but with regular frequency. Essentially, verbs
marked with the habitual can be translated by adding the adverb\index{adverbs}
...
Iterative aspect marks actions that are repeated at least once by
reduplication\index{reduplication}. Its equivalent in English\index{English} is to use the adverb \fw{again} or the
...
Besides giving an example of the correct and incorrect order\index{affix order} of attachment of
the past prefix\index{prefixes} \rayr{m/}{mə-} with a partially reduplicated verb, the example
...
\rayr{myis}{mayisa} (\ref{ex:cessative}) are expressed by
adverbs\index{adverbs} which are regularly following their heads. They tend to
precede\index{word order} other adverbs\index{adverbs} due to a higher amount
of semantic bondedness than descriptive adverbs\index{adverbs}. For this
reason, as well as for expressing a grammatical function rather than lexical
meaning with the original meaning still transparent, they appear to be on the
verge of grammaticalization\index{grammaticalization}. In contrast, the
inchoative verb \rayr{kYunF/}{cun-} (\ref{ex:inchoative}) is part of a
periphrastic verb construction, that is, \rayr{kYunF/}{cun-} requires a
semantically contentful VP as a complement\index{grammatical function!open
complement} rather than an NP. The content/main verb appears in an infinite
...
Besides various aspects\index{aspect}, Ayeri also marks mood other than realis:
irrealis, imperative, hortative, and negative. These are expressed by
suffixes\index{suffixes} on the verb and follow\index{affix order}
aspectual\index{aspect} marking where it is expressed by a
suffix\index{suffixes}, that is, the habitual suffix\index{suffixes}
\rayr{As/}{-asa}. The following subsections discuss each category expressed by
suffixes\index{suffixes}; modal particles proper will be discussed in
...
Negative mood is used to negate verbs, which is separate from irrealis\index{mood!irrealis}
...
\autoref{subsec:indefpro}), multiple negatives do not cancel each other out, but
amplify the negation instead, as displayed in (\ref{ex:dblneg}). This is to say
that Ayeri allows for multiple negation as a means to emphasize the
...
Notably, imperative-marked verbs behave essentially as infinite\index{verbs!infinite} forms in that
...
is likewise ungrammatical, since imperatives generally imply a direct
...
addition to mark the difference. Regarding agreement\index{agreement} morphology, the same 
restrictions as with imperatives\index{mood!imperative} apply.

\begin{figure}
\pex\label{ex:hortmorph}
\a\begingl
	\gla Sahu! //
	\glb saha-u //
	\glc go-\Imp{} //
...
Example (\ref{ex:hortmorph}a) contains an imperative\index{mood!imperative} verb form,
\rayr{shu}{sahu}, addressing a second person singular or plural. Example
...
another verb. As a clitic\index{clitics}, it rather acts similar to a prefix, like the
...
However, it is not common either to use this verb as a dummy to carry the inflection
instead of the modal verb like in (\ref{ex:modaldummy}). While such a
...
possesses another infinite\index{verbs!infinite} form called the participle.\footnote{It might as
...
subordinating verbs. For instance, \xayr{kYunF/}{cun-}{begin} or
...
(\ref{ex:intrcompl_2}). As (\ref{ex:trcompl_1}) shows in comparison to
...
% ear.']---\textcquote[6]{behaghel1932}{Gesetz der wachsenden Glieder [\dots]; es
% besagt, daß von zwei Gliedern, soweit möglich, das kürzere vorausgeht, das
% längere nachsteht.} [`Law of the growing constituents [\dots]; it signifies that
...
\rayr{d/}{da-} is also applicable to verbs. \rayr{d/}{da-} can therefore act
as a pro-verb. As a clitic, it leans on the verb, preceding all
...
\ref{subsec:adjaffx}). The English\index{English} translation in this context may rather be `as
though' than `like', as indicated in (\ref{ex:likecompl}), but the
...
enclitics, these suffixes lean on the inflected verb, as in (\ref{ex:intsfvb}).
...
difference from the same sentence with the verb negated\index{negation} instead of the adverb
...
normally express affirmative/agreeing and negative/disagreeing\index{negation} responses as answers to closed
...
Besides this emphatic use, both \rayr{maaj}{māy} and \rayr{voj}{voy} can also be
used in tag questions. In this context, they reflect the expectation of the
person asking with regards to the answer. Example (\ref{ex:posexpect}) poses a
question with the expectation of an affirmative answer. This is indicated by
using the affirmative particle, \rayr{maaj}{māy}, after\index{word order} the
verb. Example (\ref{ex:negexpect}), on the other hand, indicates that the asker
has doubts about the issue in question and expects their opposite to decline or
to disagree. The negative\index{negation} particle, \rayr{voj}{voy}, is placed
in the position of adverbs---after\index{word order} the verb---accordingly.
...
the stance of the speaker, to concede an argument, or simply in order to
connect clauses in an argumentative chain.
...
These adverbs are usually placed after\index{word order} the verb like any other attributive 
...
they may follow\index{word order} the verb or be placed at the end of the clause. Example
...
accepts either an NP\index{phrase types!noun phrase} or a complementizer phrase\index{phrase types!complementizer phrase} (CP) as a complement\index{grammatical function!closed complement}.
...
either after\index{word order} the verb or at the end of a clause, these words are usually found
...
`pure', logical conjunctions \xayr{nj}{nay}{and} and
\xayr{soyNF}{soyang}{or}.\footnote{Logical `not' is usually expressed by a negative\index{negation} suffix\index{suffixes} on
the adjective\index{adjectives} or the verb, compare sections \ref{subsec:adjneg} and
...
\xayr{kd/kd}{kada-kada}{so that \dots again},
...
\a\begingl[glspace=.25em]
	...
	\glft `I usually shut the door at night so that I can sleep better.' //
\endgl
...
adverbs \xayr{kd/kd}{kada-kada}{so that \dots again} and \xayr{naaroj}{nāroy}{but
...
As described above (\autoref{subsubsec:iterative}), partial
reduplication\index{reduplication} of the verb expresses iterative aspect\index{aspect!iterative}, which in Ayeri is used
to mean both `\textsc{verb} again' and `\textsc{verb} back'.
The reduplicated form \rayr{kd/kd}{kada-kada}, as displayed in
(\ref{ex:sothatagain}), is irregular if we assume that it is
formed from \xayr{kdaare}{kadāre}{so that}; the regular outcome of
iterative reduplication\index{reduplication} would be *\rayr{k/kdaare}{*ka-kadāre}. As a
conjunction, however, it is relatively frequent. It does not seem odd that
...
(\ref{ex:butnot}) likewise exhibits slight irregularity if we
consider that it is the negated form of \xayr{naarY}{nārya}{but};
the regular outcome would have been *\rayr{naarYoj}{*nāryoy}, which simplified
to \rayr{naaroj}{nāroy}, presumably as well due to its relatively high token 
frequency.
...
complementary distribution\index{complementary distribution} with full NPs\index{phrase types!noun phrase}, since they are pro-forms. Using
them as modifiers of NPs\index{phrase types!noun phrase} as in (\ref{ex:edayamod}) is thus not possible, while using
...
is thus very untypical in using a number\index{numerals} base
other than 10---none of the languages in \posscite{wals131} sample are listed as
duodecimal.
...
report that in Maldivian\index{Maldivian}, the \textcquote[21]{caingair2000}{numeral \fw{fas
...
be an exception: as a quantifier\index{grammaticalization}, \rayr{kj} {kay} means `a little,
...
want to comment first on how complete entities are handled with regards to
...
compounds\index{compounds} with lower numerals to fill in the $12^0$ numerals 11, 12, 13, \dots,
...
\tl\quad\begin{minipage}[t]{\linewidth}
	...
	\rayr{\larger menFlnF/kj}{menlan-kay} (13), \\
	\smallskip etc.
	\end{minipage}
\end{minipage}
~
...
\tl\quad\begin{minipage}[t]{\linewidth}
	...
	\rayr{\larger smFlnF/kj}{samlan-kay} (23), \\
	\smallskip etc.
	\end{minipage}
\end{minipage}
...
% 	\rayr{\larger tmFlnF/kj}{tamlan-kay} (\elv3), \\
%	\smallskip etc.
\xe
\end{figure}
...
to numerals:
\rayr{menNF}{menang}  (←~\xayr{menF}{men}{1} + \rayr{/nNF}{-nang}),
\rayr{smNF}{samang}   (←~\xayr{smF}{sam}{2}  + \rayr{/nNF}{-nang}),
\rayr{kjnNF}{kaynang} (←~\xayr{kj}{kay}{3}   + \rayr{/nNF}{-nang}),
etc.\ While \rayr{menNF}{menang} is used for 100, higher
...
numerals in turn, for instance, as in (\ref{ex:unitmod}).
...
agreement\index{agreement} in spite of being formally singular, which is illustrated in
(\ref{ex:keynamunmkd}). In (\ref{ex:keynammkd}), \rayr{kejnmF}{keynam} still receives
...
\begin{table}\centering
\caption[Simple fractions from 1/2 to 1/\elv]{Simple fractions from 1/2 to
1/\elv}
\begin{tabu} to .75\linewidth {X[c] I X[c] I}
\toprule\tableheaderfont
Numeral
...
\citep{teoh2003}.} In Ayeri, ordinals are firmly treated as noun-like elements
due to the derivational\index{derivation} suffix\index{suffixes} \rayr{/AnF}{-an} (compare
\autoref{subsec:nominalization}). Since nouns are the heads of NPs\index{phrase types!noun phrase}, this
...
between the two. An example of both uses of a multiplicative numeral is given by 
(\ref{ex:multnumwo}).
...
\ex\label{ex:distnum}
\begingl
	\gla Ang @ sarayon burangye kong besonya samaneri. //
	\glb ang= sara-yon burang-ye-Ø kong beson-ya sam-an-eri //
	\glc \AgtT{}= go-\TplN{} animal-\Pl{}-\Top{} inside ship-\Loc{} 
		two-\Nmlz{}-\Ins{} //
...
The formation of composite numerals resembles that of multiplicative
...
modifier. Example (\ref{ex:distnuminfl}) illustrates inflected forms of
distributive numerals.

\begin{figure}
\ex\labels\label{ex:distnuminfl}
\begin{minipage}[t]{.5\remaining}
\tl\quad\begingl
	\gla henlan-yaneri //
...
of case marking\index{case} is not possible. On the other hand, adpositions take both NPs
and CPs as complements, so that an adjective should be able to act as a
complement of an adposition as well. Since the numeral in the adpositional phrase (PP)\index{phrase types!prepositional phrase} is treated
...
\begingl
	...
	\glft `Pila counts from 1 to \elv{} (=\,1 \dots 11\tsub{10}).' //
\endgl
\xe
\end{figure}
...
range using a combination of genitive and dative case\index{case!dative}, or again
...
the end points using dative case. Since \rayr{menFlnF}{menlan} in (\ref{ex:rangecase2}) is 
...
label `quantifier' here) do not only follow\index{word order} verbs, nouns,
adpositions, adjectives, or other adverbs, but they cliticize to them, that is,
they are dependent morphemes (compare \autoref{clitics_quant},
p.~\pageref{clitics_quant}). The word stem---a lexical head which is usually
inflected---serves as the host for the clitic in all of these cases. Examples of
degree and quantifier suffixes and how they interact with different parts of
speech have already been given in all the relevant sections; an example from
each section is repeated here in (\ref{ex:posquant}) for convenience. As we will
see below, there are common quantifiers which behave like regular, free words as
well. It is possible to combine both the suffixed and the free kinds with other
quantifiers as long as those quantifiers permit modification with regards to
degree.
...
\xayr{kmo}{kamo}{equal(ly), likewise} to form correlative conjunctions, that is,
conjunctions made up of two parts which act together to form a construction.
...
(\ref{ex:andphrases}), then, two 
%
% agent NPs, \xayr{ynNF}{yanang}{a/the boy} and \xayr{lyNF}{layang}{a/the girl},
% together form the subject of the verb \xayr{AgYynF}{ajayan}{play}.
%
patient NPs, \xayr{pFriknFlej npj}{prikanley napay}{spicy soup} and
\xayr{betjlej pso}{betayley paso}{sweet berry}, together form the object of the 
verb, \xayr{vlYye}{valyye}{(she) enjoys}.
%
Lastly, (\ref{ex:andclauses}) shows two main clauses coordinated, that is,
\xayr{naaNF pisu}{nāng pisu}{we are tired} on the one hand, and
\xayr{tpnFnNF}{tapannang}{we are thirsty} on the other.
...
\a\label{ex:andphrases}\begingl
	\gla Ang @ valyye \textup{[\tsub{NP}~[\tsub{NP~}} @ prikanley napay
		@ \textup{]} nay \textup{[\tsub{NP~}} @ betayley paso @ \textup{]]}. //
	\glb ang= valy=ye.Ø {} prikan-ley napay {} nay {} betay-ley paso {} //
	\glc \AgtT{}= enjoy=\TsgF{}.\Top{} {} soup-\PargI{} spicy {} and {}
		berry-\PargI{} sweet {} //
	\glft `She enjoys spicy soup and sweet berries.' //
\endgl

\a\label{ex:andclauses}\begingl
	\gla \textup{[\tsub{IP}~[\tsub{S}~} @ nāng pisu @ \textup{]} nay 
...
Alternatively, a construction akin to English `either \dots or' may be used to 
...
choice is between two NPs, \xayr{kronF sti}{karon sati}{cold water} and
\xayr{gli sti}{gali sati}{cold juice}, which jointly form the object of
...
\a\label{ex:orphrases}\begingl
	\gla Le @ no @ ginvāng \textup{[\tsub{NP}~[\tsub{NP}~} @ karon sati @ \textup{]} 
		soyang \textup{[\tsub{NP}~} @ gali sati @ \textup{]]?} //
	\glb le= no= gin=vāng {} karon-Ø sati {} soyang {} gali-Ø sati {} //
	\glc \PatTI{}= want= drink=\Second{}.\Aarg{} {} water-\Top{} cold {} or {} 
		juice-\Top{} cold {} //
	\glft `Do you want to drink cold water or cold juice?' //
\endgl

\a\label{ex:orclauses}\begingl
	\gla \textup{[\tsub{IP}~[\tsub{IP}~} @ Beratu edauyi @ \textup{]} soyang
...
\fw{either \dots\ or},
\fw{neither \dots\ nor},
\fw{rather \dots\ than}, and
\fw{the \dots\ the}.
Ayeri uses the adverb\index{adverbs} \xayr{kmo}{kamo}{equally, same, likewise} together with a
conjunction for many of these. \xayr{kmo—nj}{kamo \dots\ nay}{equally \dots\ and}
is equivalent to `both \dots\ and': the correlative construction emphasizes that
...
transitive verb, \xayr{vtYF/}{vac-}{like}. With predicative adjectives\index{adjectives!predicative}, the
...
predicative terms here. In order to express literal `be as \dots\ as', thus,
...
\rayr{kmo—nj}{kamo \dots\ nay} is used to express `the \dots\ the', that is, a 
...
construction \xayr{kmo—soyNF}{kamo \dots\ soyang}{equally \dots\ or}, as
illustrated by (\ref{ex:eitheror}). For its negative\index{negation} opposite, `neither \dots\
nor', the verb\index{verbs} is negated\index{mood!negative}. \xayr{miry}{miraya}{(he) does} and \xayr{kmtNF}
{kamatang}{they are equally} in (\ref{ex:eitheror}) thus need to change to \xayr{mirojy}{miroyya}{(he) does not}
and \xayr{kmojtNF}{kamoytang}{they are not equally} to express the sentences' negative\index{negation} counterpart.
...
\a\label{ex:eitherorpred}\begingl
	\gla Kamatang mabo soyang krito mirampaluy. //
	\glb kama=tang mabo soyang krito mirampaluy //
	\glc be.equal=\TplM{}.\Aarg{} hungry or angry otherwise //
