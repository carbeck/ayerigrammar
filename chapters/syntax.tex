...
part of the verb's a-structure. Especially the pseudocleft test---labeled `Moves with \xhead{V}' in \autoref{tab:drvargres}---%
...
%
	& \rotatebox[origin=l]{90}{\fw{sara-} `go'}
	& \rotatebox[origin=l]{90}{\fw{mit-} `live'}
	& \rotatebox[origin=l]{90}{\fw{tapy-} `put'}
	& \rotatebox[origin=l]{90}{\fw{nara-} `speak'}
	& \rotatebox[origin=l]{90}{\fw{tiya-} `make'}
	\\
...
\xayr{sr/}{sara-} in their behavior in that they may be modified by a locative
and an instrumental NP, respectively. This means that there are no prepositions
involved which could provide any contentful information, however, the case
markers for these adverbials themselves are to be taken literally as expressing
a location and a material or means. There is no free choice about these case
markers, even though the degree of grammaticalization of these adverbial cases
is maybe not as high as with the core cases---although it was noted before how
Ayeri's core cases do not just express function, but that semantics still need
to be factored in. The degree of grammaticalization is definitely higher than
that of a free PP adjunct. The score is also 6 to 2 in favor of complementhood.
...
\begin{forest} shorter edges,
[{\anno[\{\pass{\Compl}\\ |~\pass{\XCompl}\\ |~\pass{\Adjc}\}]{CP}}
%	[\anno{\xbar{C}}
...
phrase type---IP or a VP, but also plain S. This means that the complement
clause may be finite or infinite. The CP itself may fulfill different
grammatical functions: open or closed complement, as well as adjunct. Ayeri is
rather sparse about conjunctions, compare \autoref{subsubsec:conjadv} 
(p.~\pageref{subsubsec:conjadv}). The conjunction corresponding to English 
\fw{that} is zero, so that simple complement clauses are simply juxtaposed.
Examples of complement clauses are provided in (\ref{ex:complcls}).
