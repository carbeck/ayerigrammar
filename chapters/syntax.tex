...
\begin{description}
	\item[Locative (\Loc):] Standard case for prepositional objects, indicates 
	a location (\textsc{location}).

	\item[Dative (\Dat):] Indicates motion towards the direction the adposition
	indicates (\textsc{goal, direction}) in cases where just locative marking
	would be ambiguous, for instance, `up to' instead of `onto', since \rayr{mN
	liNF}{manga ling} can mean both, by itself.

	\item[Genitive (\Gen):] Indicates motion from the direction the adposition
	indicates (\textsc{source, origin}) in cases where just locative marking
	would be ambiguous, for instance, `down from' instead of `under', since
	\rayr{mN AvnF}{manga avan} can mean both, by itself.
\end{description}
...
% argument structure as in English \fw{talk to someone}. Ayeri instead often uses
% an NP complement marked with one of the cases above. In the case of
% \xayr{nr/}{nara-} {speak, talk}, the complement appears in the locative case,
% but as an NP, not as a PP.

% \pex
% \a\label{ex:talkcstruct}%
% John talks to Mary. \\
