...
an equally instrumental NP to express both authorship and tool use. These
...
\begingl
	\gla Ang @ tiyāy sitang-rī (nay) harueri. //
	\glb ang= tiya=ay.Ø sitang=rī nay haru-eri //
	\glc \AgtT{} make=\Fsg{}.\Top{} self=\Fsg{}.\Ins{} and hammer-\Ins{} //
	\glft `I made it by myself (and) with a hammer.' //
\endgl

\a\label{ex:multiins_2}%
\begingl
	\gla Ang @ tiyāy para (nay) harueri. //
	\glb ang= tiya=ay.Ø para nay haru-eri //
	\glc \AgtT{} make=\Fsg{}.\Top{} quickly and hammer-\Ins{} //
	\glft `I made it quickly (and) with a hammer.' //
\endgl
\xe
\end{figure}
...
\ljudge*\begingl
	\gla Tiyayam adareng si ang @ mirāy harueri. //
	\glb tiya-yam ada-reng si ang= mira=ay.Ø haru-eri //
	\glc make-\Ptcp{} that-\AargI{} \Rel{} \AgtT{}= do=\Fsg{}.\Top{}
		hammer-\Ins{} //
	\glft `*What I did with a hammer is make it.' //
\endgl
...
complements \citep[408]{needhamtoivonen2011}.
...
beneficiary---is mapped to [+\,o]. In contrast to English ditransitive clauses
with dative movement (\fw{x gives y the z}), object and secondary object appear
in the order \Obj{}---\SObj{}.\footnote{The example with `cook-for' in \citet
[338--339]{bresnan2016} is confusing insofar as they chose to analyze \fw{cook
sth. for sb.} as having the argument structure 
\astruct{cook-for}{\textsc{agent}\tsub{[–\,o]}
\textsc{beneficiary}\tsub{[–\,r]} \textsc{patient}\tsub{[+\,o]}} with the
beneficiary as a primary object rather than as an secondary object as one might
expect. At least in the German \fw{Sie kochen den Kindern Mittagessen} `They
cook lunch for the children', where the verb could arguably be given as
`cook-for' in English translation, \fw{den Kindern} `for the children' is
realized as an indirect/secondary object in the dative. On the other hand,
compare (\ref{ex:engditrpass_pass1}) below, where \fw{Mary}, the recipient,
needs to be [–\,r] in order to be a passive subject.} An example of an active
ditransitive sentence is given in (\ref{ex:ditrorder}).
...
German, on the other hand, appears to work a little differently from English at
least for the verb \fw{berichten} `tell, report, relate'.\footnote{The typical
ditransitive verb, \fw{geben} `give', works this way as well.} This is
...
recipient by the dative case, and the theme by the accusative case.
