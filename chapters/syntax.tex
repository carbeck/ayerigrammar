...
and genitive complements, respectively, to mark the difference, compare
...
for `atelic' uses of the preposition.
...
\a\adjustbox{valign=t}{%
	...
	\rayr{\larger AvnF}{avan}
		...
		\begin{tabular}[t]{l l l}
%			\ups{\Tel} = $+$ & $\implies$ & \ups{\Obj{} \Case{}} \req{} \Loc \\
			\ups{\Tel} = $-$ & $\implies$ & \ups{\Obj{} \Case{}} \req{} \Gen \\
		\end{tabular}
	\end{tabu}%
}
\xe
...
the lexical entry of the word (\fw{clean up}, \fw{give in}, \fw{sign on}) or
...
(p.~\pageref{ex:particleverbs}). One effect of English's preposition-happiness
is that they can also appear stacked, for instance, as in \fw{crawl out from
under the table}, \fw{get out from inside the room}. Ayeri, however, cannot
stack adpositions like this, and may use nouns instead, which is no surprise as
most basic prepositions are, in fact, derived from nouns (compare
\autoref{tab:prepos}, p.~\pageref{tab:prepos}). Hence, a phrase like \fw{out
from under} might be rendered the following way:
...
Strictly speaking, however, \rayr{AgonnF}{agonan} is not even
necessary, since the first NP complement already indicates a motion from
somewhere.\footnote{A grammaticalization process akin to that of
\fw{inside of the X} to \fw{inside the X} may be a logical further
development from here. What would be possible moreover as a result is the
functionalization of the triad \Loc{}--\Dat{}--\Gen{} to indicate $loc$,
$goal$, and $src$ also with adpositions in general, either with
\rayr{mN}{manga} grammaticalizing further to really just marking direction,
necessitating an obligatory \Dat{}/\Gen{} complement to indicate which way
around, or with \rayr{mN}{manga} withering to zero, since the cases are already
enough to mark direction. The way to express the difference between \fw{top/up}
and \fw{bottom/down} would have to change, however, though
\xayr{riNF/}{ring-}{rise, lift} and \xayr{les/}{lesa-}{fall} would be good
candidates to generate new adpositions from. Alternatively,
\rayr{sh}{saha} could become an equivalent of \rayr{mN}{manga} to indicate
direction \fw{from} the indicated place.}
