...
based in constituent structure hierarchies, but in functional and semantic
structure. Binding is thus conceptualized by way of coindexing between a
referent and an anaphora \citep[231]{bresnan2016}. There is also the
requirement that a referent outrank its anaphora on the functional hierarchy:

\ex\label{ex:funchier}%
	Functional hierarchy \citep[229]{bresnan2016}:\medskip \\
	$\Subj > \Obj > \SObj > \Oblique > \Compl > \Adjc$
\xe

Furthermore, reflexive pronouns are considered `nuclear', that is, that it is
required that they be bound in their minimal f-structure nucleus that contains
the reflexive and a subject outranking it. Personal pronouns need to be free in
their minimal f-structure nucleus; other nominals must be free completely
\citep[237]{bresnan2016}. This is basically a rephrasing of the Binding
Principles A, B, and C as quoted in (\ref{ex:gbprinciples}), except that this
version is based in functional structure, not in constituent structure. A
minimal f-structure nucleus is the predicator of an f-structure together with
the bare argument functions the a-structure of the verb specifies.
