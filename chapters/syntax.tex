...
\draw [rounded corners=1ex] ([yshift=1ex]{pic cs:ctrlinc_top})
	-- ++(east:11em) |- ([yshift=1ex]{pic cs:ctrlinc_obj});
\draw [rounded corners=1ex] ([yshift=1ex]{pic cs:ctrlinc_subj1})
	-- ++(east:1em) |- ([yshift=1ex]{pic cs:ctrlinc_subj2});
\end{tikzpicture}
\xe
\end{figure}
...
control verb rather than for `look after, care for'.} Instead of using verbal
...
\xayr{kraaneri}{krāneri}{by tendency} and
\xayr{nilj}{nilay}{probably}. English has raising verbs like
\fw{expect}, \fw{order} or \fw{want}, which may take an object and a verbal
complement, but whose syntactic object is the semantic subject of the
subordinate verb. Ayeri, however, lacks the raising-to-object mechanism and
instead requires a complement clause.
...
its \Subj{} is defined as an athematic subject. This is indicated by placing
the function label outside of the pointed brackets. The same goes for the
\Obj{} in (\ref{ex:raisestruct_2}): here as well, the object is not strictly an
argument of the matrix verb. In the accompanying \Avm{}s, the respective
athematic functions in $f$ are connected to the \Subj{} function in $g$ in both
cases to indicate coherence.

The example sentence in (\ref{ex:raisestruct_2}) is also marked ungrammatical
in contrast to (\ref{ex:ctrlstruct_1}). Even though both sentences may be
...
\a\label{ex:ctrlraisecomp_raiseo}%
	\ljudge*\astruct[\ups{\Obj}]{believe}{\ups{\Subj} \ups{\XCompl}}
\xe

While \xayr{nelF/}{nel-}{help} subcategorizes for a thematic object,
\xayr{pronF/}{paron-}{believe} does not. Apparently, Ayeri has no problem
unifying a thematic object with the subject of a subordinate verb, in spite of
...
athematic object cannot become a subordinate thematic subject. Note, however,
that even raising-to-subject is only common in Ayeri with \xayr{surFpF/}{surp-}
{seem}, so Ayeri more generally seems to have an aversion towards raising. Or,
assuming that the difference to English is lexical, Ayeri's verbs do not allow
for athematic functions with the notable exception of \rayr{surFpF/}{surp-}.
Instead of an \XCompl{} and an athematic function, they subcategorize for a
\Compl{}.
...
\draw [rounded corners=1ex] ([yshift=1ex]{pic cs:raiseinc_subj1})
	-- ++(east:1em) |- ([yshift=1ex]{pic cs:raiseinc_subj2});
\end{tikzpicture}
\xe
\end{figure}

Just as with control verbs, it is possible to generate the subordinating verb
as a complement of the matrix verb in \xhead{I}. This way as well, it is
possible to topicalize arguments of the subordinate verb, the condition being
that no argument functions are doubled here as well. Again, the subordinated
verb and the VP with its dependent NPs have to be annotated \elem{\XCompl}, and
the topicalized NP has to be marked with \uncertain{\XCompl}{\Top}~=~↓ in
accordance with the position of the topic marker as adjoined to the left edge
of the matrix verb's f-structure, that is, since the subordinated verb is
non-finite and thus cannot carry topic marking.
