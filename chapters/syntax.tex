...
by means of morphojlogy. For tenses, Ayeri distinguishes an unmarked present
...
Personal pronouns occur in an undeclined base form if topicalized (see
\autoref{subsec:perspro}). Morphologic topic marking is limited to finite verbs
which subcategorize more than one grammatical function, so a verb whose
...
argument part of the matrix verb's argument structure. However, as described
in \autoref{subsubsec:raising} (p.~\pageref{subsubsec:raising}), only
to-subject raising is permitted in Ayeri.
