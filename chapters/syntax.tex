...
a different argument of the verb often is.\footnote{Compare \cite[70--71]
...
agreement with an \emph{external} entity is conceptualized as a constraining
equation where the relative pronoun requires being included in the \Adjc{} of a
\GF{} which contains \Case{} and \Anim{} features set to the values the
relative pronoun specifies. Any roles which the relative pronoun receives from
an \emph{internal} predicator are realized by the \Case{} and \Anim{} feature.
Internal additional case marking is limited to those grammatical functions
which are not subject or object.
...
\draw [rounded corners=1ex] ([yshift=1ex]{pic cs:relcsi_top2})
	-- ++(east:7em) |- ([yshift=1ex]{pic cs:relcsi_subj});
\end{tikzpicture}
\end{minipage}
\hfill
\begin{forest} narrower nodes, shorter edges, italic leaves,
...
agreement, the f-structure would not change if the relativizer were \rayr{sas}
{ssF}, that is, marked for an animate-patient controller outside of the
relative clause.
