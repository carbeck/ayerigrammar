...
analogy to \textsc{html} et al., inside-out functional uncertainty is
essentially like walking the \textsc{dom} tree in jQuery with 
\texttt{\$(this).parent($elem$).attr()}.} \citet[144]{dalrymple2001} gives an 
example from Warlpiri (from \cite[136]{nordlinger1998}, attributed to
\cite{simpson1991}) which contains a noun with double case marking, which may
serve as a template to analogize from here:
...
f-structure $f$ is the value of its \Subj{} feature, since \Erg{} case
identifies the NP as the subject.
...
(\ref{ex:ex:danyaastuvoavm}) above this way, since \rayr{d/tuvolej}{da-tuvoley}
...
(\ref{ex:datuvoleycstr}). Secondly, we do not have to assume this way that the
adjective itself untypically inflects for case.
