...
Kroeger rather than Schachter. Either way, we will have to test number
agreement, quantifier float, control of secondary predicates, relativization,
and the behavior of both raising and control verbs, . First of all, it will be
helpful, however, to define some terms which will be used in the discussion
below.
...
argument of the complement clause subsequently is realized as a null element
coindexed with the patient of the matrix clause, that is, the raised argument.
In English, one would speak of to-object raising, though here the patient of
\fw{gusto}, \fw{sila}, is in its nominative form, so syntactically, \fw{ng
Nanay} `mother', the actor, is the object in this clause. In
(\ref{ex:tagrais2_1}), the verb of the complement clause marks its patient
argument as the subject. Yet, the non-subject agent, \fw{Charlie}, is raised to
occupy the patient role in the matrix clause. The position of the non-subject
agent in the complement clause is subsequently realized as a resumptive
pronoun, \fw{niya}, coindexed with the raised NP. Example (\ref{ex:tagrais2_2})
shows that it would be ungrammatical to have a null element in its stead.
...
an \Abs{}-\Erg{} language would mark subjects. Unfortunately,
\citet{kroeger1991} only gives examples of `to-patient' raising, but not of
`to-agent' raising \citep[430]{carnie2013}. As we will see below, Ayeri has no
...
{surp- koronyam} in (\ref{ex:ayrrais2}), literally `seems knowing', with all of
the arguments of the embedded clause becoming arguments of the matrix clause,
that is, the matrix verb is interpreted as a transitive clause and may carry
topic marking for any of its syntactic (rather than semantic) arguments.
...
president' is a non-actor subject of the subordinate verb here as well. So it
is in the matrix clause as well, however, as the matrix verb,
\fw{napagbintangan} `be accused of', is marked for directional voice as well.
...
\a\label{ex:tagrais3_1}\begingl[aboveglbskip=1.5em, aboveglftskip=1.75em]
	...
	\glb \Nvol{}-close
		already
		$\underbrace{\text{\Nom{}=Manuel}}_{\text{S/A}%
...
\a\label{ex:tagrais3_2}\begingl[aboveglbskip=1.5em, aboveglftskip=1.75em]
	...
	\glb \Nvol{}.\Pfv{}-accuse-\Dv{}%
		...
		$\underbrace{\text{\Gen{}=syndicate}}_{\text{A}}$
		$\underbrace{%
...
Now that a few tests were conducted, let us collect the results. ...
