...
appears in the `wrong' position due to scope\index{scope} effects. In terms of
\Lfg{}, this can probably best be explained with an extended head (\cite[136]
...
still found as daughters of VP. Similarly, we may want to analyze the heads of
such NPs to actually reside in \xhead{D}, with adjuncts of the noun adjoined to
\xbar{D}, and the complement remaining as a daugther of NP. The structure
formula in (\ref{ex:dpstruct}) and the c-structure tree in (\ref{ex:dpcstruct})
thus only describe the makeup of DP for the situation where \xhead{D} is a
modifier of some \xhead{N}. As a supplement to the above rules, we can construe
the respective structures for the situation of extended noun heads as in
(\ref{ex:dpstruct2}) and (\ref{ex:dpcstruct2}).
...
the noun phrase. Example (\ref{ex:npext}) picks up (\ref{ex:adjscope}) to
illustrate the analysis of `misplaced' nominal complements proposed above. As
this example shows, both kinds of DP outlined above can be combined.

\begin{figure}
\ex\label{ex:npext}
\begin{minipage}[t]{.33\remaining}
\begingl
	\gla limu hiro sapari nā //
	\glb limu hiro sapa-ri nā //
	\glc shirt new wool-\Ins{} \Fsg{}.\Gen{} //
	\glft `my new shirt of wool' //
\endgl
\end{minipage}
\hfill
...
becomes rather elaborate in c-structure, \Lfg{} allows us to capture it in terms
of f-structure in a very straightforward manner.
