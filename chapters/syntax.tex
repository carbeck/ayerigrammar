...
It is tempting to assume that the adjective has been moved into the place where
its nominal referent would usually occur in, as in (\ref{ex:adjmove}). However,
\Lfg{} has been designed specifically with the idea that there is no movement,
so this is not a viable approach either if we want to stick to our theoretical
framework.

\ex\label{ex:adjmove} \begin{forest} shorter edges,
[NP
	[\xbar{N}
		[\xhead{N}
			[Cl
		...
		[AP
			[tuvo, roof, name=origin]
		]
	]
]
\draw[-latex,min distance=1cm] (origin) to [out=south, in=south] (target);
\end{forest}
\xe

% \ex~\labels %
% \begin{tabular}[t]{@{} l @{\quad} l}
% \tl\quad\ques\begin{forest} shorter edges,
% [{\anno[\pass{\Obj}]{NP}}
% 	[\anno{\xbar{N}}
% 		[\anno{\xhead{N}}
% 			[\anno{Cl}
...
% \tl\quad \ques\adjustbox{valign=t}{%
% \begin{avm}
% \[\Obj & \[
% 			\Pred	& `red one' \\
% 			\Anim	& $+$ \\
