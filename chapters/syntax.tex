...
so-called Austronesian Alignment---Tagalog, an Austronesian language of the
Malayo-Polynesian branch, spoken mainly in the Philippines
...
Tagalog's system in a number of ways, though, probably towards the more
conventional from the point of view of Standard Average European. It will,
thus, be informative to compare both systems to one another based on the work
of \textcites{kroeger1991}{kroeger1993}, who provides an analysis in terms of
the \Lfg{} framework as well.\footnote{As mentioned in the introductory
chapter, I started Ayeri in late 2003---then still in high school, and not
knowing much about linguistics. Of course, I had to go and pick the syntactic
alignment which has long been \textquote{a notorious problem for both
descriptive grammarians and theoretical syntacticians} to the point where it
\textcquote[41]{kroeger2007}{sometimes seems as if Austronesian specialists can
talk (and write) of nothing else}.}
...
Such a structure, though, does not do Ayeri justice in that the verb, for
instance, shows agreement usually with the constituent expressing the actor.
% ; the fact that adjectives are unmarked for case also does not favor
% scrambling in the way seen in an example of Warlpiri given in
% \citet[4]{bresnan2016}; \citet{bresnan2016} note that every permutation of
% the words in the given sentence, meaning `The two small children are chasing
% the dog', are possible as long as the auxiliary occurs in the second
% position:
%
% \ex\rc{Warlpiri}
% \begin{forest}
% [S
% 	[NP
% 		[{\textit{wita-jarra-rlu}\\ small-\Du{}-\Erg{}}, roof]
% 	]
% 	[Aux
% 		[{\textit{ka-pala}\\ \Prs{}-\Tdu{}.\Subj{}}, roof]
% 	]
% 	[V
% 		[{\textit{wajilipi-nyi}\\ chase-\NPst{}}, roof]
% 	]
% 	[NP
% 		[{\textit{yalumpu}\\ that-\Abs{}}, roof]
% 	]
% 	[NP
% 		[{\textit{kurdu-jarra-rlu}\\ child-\Du{}-\Erg{}}, roof]
% 	]
% 	[NP
% 		[{\textit{maliki}\\ dog.\Abs{}}, roof]
% 	]
% ]
% \end{forest}
% \xe
%
Let us tentatively assume, thus, that the actor NP is priviledged over the
undergoer NP. The verb in the initial position also shows inflection, so one
might rather construe it as an \xhead{I}, projecting an IP, which frees up VP
for other purposes while we can use IP to govern both \xbar{I} and S. In fact,
this is basically the conclusion \citet{chungmccloskey1987} come to for Irish,
which is also a VSO language (\ref{ex:irishcstruct}). \citet{bresnan2016} give
the similar chart in (\ref{ex:welshcstruct}) for Welsh, equally a VSO language
(also compare \cite [66]{dalrymple2001}, which sources \cite{sadler1997}).
\citet{kroeger1991} suggests the two structures depicted in
...
\Lfg{} is built specifically without any movement. There is not supposed to be
a trace of V left behind, since \Lfg{} avoids empty categories, as there is no
...
\xhead{I}. The Welsh example furthermore does not violate endocentricity, since
the finite verb in \xhead{I} still functions as the head in the f-structure.
With regards to constituent structure, \xhead{V}, if present, c-commands its NP
sister; both \xhead{V} and NP are dominated by VP:
...
of VP, which itself lacks a categorial head. In any case, tests need to be
performed to see whether one of the analyses presented in
(\ref{ex:vsocstructs}) holds true for Ayeri as well.

\subsubsection{Ayeri as a `trigger language'}

\begin{itemize}
	\item Where does the term originate?
	\item What does it mean?
	\item What is the historic misunderstanding of the `real thing', i.e.
	Austronesian alignment? \parencites[also compare][]{schachter1976}
	{schachter2015}
\end{itemize}
...
\begin{itemize}
	\item What is a subject? → definitions:
		\textcites[132\psqq]{carnie2013}[104\psqq]{comrie1989}
		[76]{dixon2010}[151\psqq]{dixon2012}[]{lithompson1976}; regarding
		AA: \citet[19--54]{kroeger1991}
...
\pex
\a nominative--accusative alignment (S\tsub{A}---P):\medskip
...
\a ergative--absolutive alignment (S\tsub{P}---A):\medskip
...
\subsubsection{Tests on subjecthood}

\begin{itemize}
	\item Does Ayeri have a VP, since it's VSO? (i.e. is it configurational?);
	\citet[118--119]{bresnan2016} reminds that configurationality is basically
	a gradient
