...
the label XP here, which here stands for AdvP %, NP, DP, PP, 
...
adjunct instead.
% How do we know that the extensions to the adjective in (\ref{ex:adjmod}) are
% complements rather than adjuncts, though? \citet[181--182]{carnie2013}
% suggests replacement of \xbar{X} with a pro-form, so we can try where nodes
% can be snipped off the tree, basically. X-bar theory suggests that
% complements and adjuncts are on different \xbar{X} branches---a complement is
% a sister to the head whereas an adjunct is, so to speak, its niece:
