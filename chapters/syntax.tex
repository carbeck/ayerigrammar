...
verb \citep[289\psqq]{bresnan2016}, also compare the English examples in 
(\ref{ex:engctrlstruct}). Typical S-control verbs in Ayeri are
\xayr{Ep/}{epa-}{refuse},
\xayr{liMk/}{linka-}{try},
...
Since topicalization does not apply across complete f-structures (?),
% FIXME: OR DOES IT AND THE RULE IS THAT NO VERB MAY INTERVENE AND ROLES MUST
% NOT APPEAR TWICE IF NO VERB IS IN BETWEEN???
the \xayr{nyiNsF}{nayingas}{roof} in both (\ref{ex:ctrlstruct}ab) cannot be the
topic of the verb in the respective matrix clauses, \xayr{liMky}{linkaya}{(he)
tries} and \xayr{nelFye}{nelye}{(she) helps}. Likewise, since topicalization
does not apply to non-finite verbs, the subordinate verb in both sentences,
...
\[
	...
	\XCompl	&	\tikzmark{ctrlstruct1_xcomp} \[
...
\draw [-latex] ([yshift=1ex]{pic cs:ctrlstruct1_sid})
	to [out=east, in=west] ([yshift=1ex]{pic cs:ctrlstruct1_xcomp});
...
\draw [rounded corners=1ex] ([yshift=1ex]{pic cs:ctrlstruct1_subj1})
	-- ++(east:1em) |- ([yshift=1ex]{pic cs:ctrlstruct1_subj2});
\end{tikzpicture}

\a\label{ex:ctrlstruct_2}%
Object control in Ayeri:\\
...
\begin{avm}
\[
	\Pred	&	\astruct{help}{\ups{\Subj} \ups{\Obj} \ups{\XCompl}} \\
	...
	\XCompl	&	\tikzmark{ctrlstruct2_xcomp} \[
		\Pred	&	\astruct{repair}{\ups{\Subj} \ups{\Obj}} \\
...
\draw [-latex] ([yshift=1ex]{pic cs:ctrlstruct2_sid})
	to [out=east, in=west] ([yshift=1ex]{pic cs:ctrlstruct2_xcomp});
...
\draw [rounded corners=1ex] ([yshift=1ex]{pic cs:ctrlstruct2_obj3})
	-- ++(east:10em) |- ([yshift=1ex]{pic cs:ctrlstruct2_subj2});
\end{tikzpicture}

\xe
\end{figure}

Furthermore, the process which places the matrix verb in \xhead{I} can also be
seems to be applicable as well to the subordinate verb if this leads to no
doubling of semantic roles in the linear order of constituents in the clause.
The subordinate verb probably becomes a complement of \xhead{I}, and both VPs
need to be annotated with \elem{\XCompl} instead of plain \pass{\XCompl}, since
both \xhead{V} and its complements are part of the \XCompl{} of the matrix
verb, only that the lexical material belonging to this function is not
contiguous anymore. An example is given in (\ref{ex:ctrlinc}).
...
[IP
	[\xbar{I}
		...
		[VP \tikzmark{ctrlinc_sid1}
			[sidejam]
		]
	]
	[S
		[NP \tikzmark{ctrlinc_mic}
			[{ang Mican}, roof]
		]
		[VP
			[VP \tikzmark{ctrlinc_sid2}
				[NP \tikzmark{ctrlinc_nay}
					[nayingley]
				]
			]
		]
...
\[
	\Pred	&	\astruct{try}{\ups{\Subj} \ups{\XCompl}} \\
	\Subj	&	\tikzmark{ctrlinc_subj} \[
		\avmspan{``Mican''} \\
	\] \tikzmark{ctrlinc_subj1} \\
	\XCompl	&	\tikzmark{ctrlinc_xcomp} \[
...
\end{avm}
\begin{tikzpicture}[remember picture, overlay]
\draw [-latex] ([yshift=1ex]{pic cs:ctrlinc_mic})
	to [out=east, in=west] ([yshift=1ex]{pic cs:ctrlinc_subj});
\draw [-latex] ([yshift=1ex]{pic cs:ctrlinc_sid1})
	to [out=east, in=south west] ([yshift=1ex]{pic cs:ctrlinc_xcomp});
\draw [-latex] ([yshift=1ex]{pic cs:ctrlinc_sid2})
	to [out=east, in=south west] ([yshift=1ex]{pic cs:ctrlinc_xcomp});
\draw [-latex] ([yshift=1ex]{pic cs:ctrlinc_nay})
	to [out=east, in=west] ([yshift=1ex]{pic cs:ctrlinc_obj});
