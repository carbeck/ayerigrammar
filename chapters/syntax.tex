...
\a \xbar{I} → \anno*{\xbar{I}} \anno*[\elem{\XCompl}]{VP}
...
\elem{\XCompl} instead of plain \pass{\XCompl}, since both \xhead{V} and its
complements are part of the \XCompl{} of the matrix verb, only that the lexical
material belonging to this function is not contiguous anymore. The purpose of
this `verb raising' operation is that NPs subcategorized by the subordinate
verb can now also be topicalized. The topic particle, as usual, appears as a
proclitic before the verb. In order to adress the \Top{} function of the matrix
verb in case an embedded NP is topicalized, we have to use inside-out
functional uncertainty and annotate the NP with \uncertain{\XCompl}{\Top}~=~↓
to signal that \Top{} is an attribute of the f-structure containing \XCompl{}.
An example is given in (\ref{ex:ctrlinc}).
...
subordinate verb and the VP with its dependent NPs have to be annotated
\elem{\XCompl}, and the topicalized NP has to be marked with
...
Theoretically, there is also \xayr{verY/}{verya-}{be least},\footnote{From
...
compare \cite{beermannetal2005}. What is described has similarities to a
...
However, Ayeri does not possess serial-verb constructions. Instead, in Ayeri,
comparative verbs take the quality as a modifier, essentially in the way of an
adverb. Nonetheless, the subject forms the comparee, and the object the
standard. An example is given in (\ref{ex:ayrexceed}). More generally, the
...
\ex~\label{ex:ayrexceedscheme}%
	\textsc{comparee} exceeds \textsc{quality} \textsc{standard}
\xe
...
fact, the \Avm{} in (\ref{ex:lfgpredcomp}) has a certain similarity to those of
control and raising verbs, compare (\ref{ex:ctrlstruct}) and
(\ref{ex:raisestruct}). In all cases, there is a subordinate predicator
subcategorizing for a complement. What if Ayeri generalizes the way subordinate
verbs can be fronted in order to enable topicalization of the subordinate
verb's dependents to adjectives, treating the adjective like a verbal
complement so that superficially, the embedded object appears superficially
like a complement of the matrix verb? 
...
\xayr{verY/}{verya-}{be least}. Only the definitions extending those in 
