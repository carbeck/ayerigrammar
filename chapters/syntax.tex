...
Tests on configurationality which cannot be applied to Ayeri are those
concerning the weak crossover effect, since Ayeri's question words are \fw{in
situ}, and noun incorporation, since Ayeri does not have noun incorporation.
...
ungrammatical again because \fw{she} is c-commanded and thus not free. Also
compare the diagrams in (\ref{ex:flatbindchart}).

The examples above assume a stucturalist approach, while I have so far tried to
use a more flexible functional approach. \Lfg{}'s approach to binding are not
based in constituent structure hierarchies, but in functional structure. It
distinguishes nuclear (positive) and non-nuclear (negative) binding
constraints using the inside-out principle to establish a relation between
referents \citep[248]{bresnan2016}:

\pex\label{ex:nucbind}
\a\label{ex:nucbind_1}%
	Positive (nuclear) binding constraint:\medskip \\
	((DomainPath~↑) Antecedent \Index) = \ups{\Index}
\a\label{ex:nucbind_2}%
	Negative (non-nuclear) binding constraint:\medskip \\
	((DomainPath~↑) Antecedent \Index) ≠ \ups{\Index}
\xe

The definition in (\ref{ex:nucbind_1}) states that for the current pronominal
f-structure element there is a path to an antecedent which matches its
\Index{}; (\ref{ex:nucbind_2}) states that there is not. Essentially, this is
the difference between \fw{Mary$_i$ likes her$_i$ mother}, in which Mary likes
her own mother (\Index{} $i$ and \Index{} $i$ are matching), and \fw{Mary$_i$
likes her$_j$ mother}, in which Mary likes someone other \fw{her}'s mother
(\Index{} $i$ and \Index{} $j$ are not matching).

For pronominal f-structures, there is also a condition which states that the
pronoun must be outranked by its antecedent on the functional hierarchy; the
DomainPath is required to have a top attribute \GF{} which is \textcquote
[248--249]{bresnan2016}{which is dominated on the functional hierarchy [...]
by the antecedent}:

\needspace{3\baselineskip}
\pex
\a\label{ex:funchier}%
	Functional hierarchy \citep[229]{bresnan2016}:\medskip \\
	$\Subj > \Obj > \SObj > \Oblique > \Compl > \Adjc$

\a\label{ex:fsdpcond}%
	F-structure DomainPath condition \citep[249]{bresnan2016}:\medskip \\
	All DomainPaths in positive and negative binding constraints [...] must be
	constructed so that: (i)~Domain Path = \GF{} $\alpha$ for some possibly
	empty attribute string $\alpha$; (ii)~Antecedent = \xbar{\GF}; and
	(iii)~\xbar{\GF} > \GF{} in the functional hierarchy [...].
\xe
