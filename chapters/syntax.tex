...
chapter. Furthermore, this is a first attempt to formally analyze Ayeri's
syntax and my first time attempting to learn more about and to apply a
syntactic formalism. The present analyses thus do not claim to be complete or
flawless.} Since Ayeri is a verb-initial language, it is probably rather
...
\a\label{ex:npstruct_Adj}%
	NP → \anno*{\xbar{N}} \anno*[{\elem{\Adjc}}]{AP}
...
possessors have some properties in common}, which is why they are variably
...
form of \rayr{sinY}{sinya} depends on the \GF{} it is linked to.
...
join modifiers to \xbar{D}, compare (\ref{ex:itradj}).
...
literature \parencites{dalrymple2001}{falk2001} sometimes includes a rule for
interrogative pronouns to change the sentence type to \fw{wh-question} or
similar. Since I am not using such a feature here, I am simply connecting the
focus NP with the \Q{} attribute to mark that the clause is supposed to be a
question. The problem then is, however, what to do with the \Prontype{}
feature.} The interrogative pronoun asking for quantity,
\xayr{siknF}{sikan}{how much, how many}, likewise acts like a nominal modifier,
and only ever does---that is, unlike \rayr{sinY}{sinya} it does not have a
double role. It may combine with any countable or quantifiable noun, as shown
in (\ref{ex:sikanadj}), to query about this noun.
...
pursued here, quantifiers are co-heads of \xhead{N}, and thus place their
information in the same functional core as \xhead{N}. However, since
...
f-structure core of \xhead{N}, as with adjuncts and possessors. Since
...
\xayr{/IknF}{-ikan}{much, many; very}. In order to illustrate modification by
an intensifier as mentioned above, \xayr{EkeNF}{ekeng}{too, over-, overly} has
been added. This intensifier modifies not the head of NP,
\xayr{ErFsy}{ersaya}{cook}, but the head of the DP, which is an empty node
here whose information content is, however, provided functionally by the clitic
\rayr{/IknF}{-ikan} on the noun. Since \Lfg{} restricts specifiers of
...
specifier of \xhead{D}. Instead, it has been analyzed here as an adverb for
lack of a better understanding. A question resulting from this
analysis---assuming that it is accurate---is how to ensure that \xhead{A} is
understood as modifying \xhead{D} rather than \xhead{N}, since \xhead{D} and
\xhead{N} share a functional core. An analysis whereby the quantifier is a head
subcategorizing for a nominal complement, for instance, like \astruct{many}
...
English this word may constitute a preposition or conjunction, so \rayr{ku/}
{ku-} might thus be treated as either, functionally, even though it surfaces as
a clitic. More information on either function of \rayr{ku/}{ku-} may be found
...
\phantomsection\label{phsec:kuprep}
The clitic \rayr{ku/}{ku-} as used with depictive secondary predicates
(\autoref{subsubsec:depict}, p.~\pageref{subsubsec:depict}) constitutes a
special case. At least in English, \fw{as} is a preposition; in Ayeri, the head
...
functionally. Unlike other adpositions, however, \rayr{ku/}{ku-} does not
...
\phantomsection\label{phsec:kuconj}
As discussed previously (\autoref{phsec:kuprep}, p.~\pageref{phsec:kuprep}),
the clitic expressing likeness, \rayr{ku/}{ku-}, may be analyzed functionally
as a preposition when its complement is not marked for case. However, there are
also occasions where it is. These permit being analyzed as \rayr{ku/}{ku-} (or 
\rayr{/ku}{-ku}, depending on its syntactic environment) taking an elliptical
complement clause, so \rayr{ku/}{ku-} should constitute a conjunction in those
cases.
