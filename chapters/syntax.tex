...
\begin{tikzpicture}[remember picture, overlay]
\draw [-latex, min distance=1cm] ([yshift=0.5ex]{pic cs:engqcstruct_CP})
	to [out=east, in=west] ([yshift=0.5ex]{pic cs:engqcstruct_f});
\draw [-latex, min distance=1cm] ([yshift=0.5ex]{pic cs:engqcstruct_NP})
	to [out=east, in=west] ([yshift=0.5ex]{pic cs:engqcstruct_g});

\draw ([yshift=0.5ex]{pic cs:engqcstruct_g2})
	to [out=east, in=east] ([yshift=0.5ex]{pic cs:engqcstruct_q});
\draw ([yshift=0.5ex]{pic cs:engqcstruct_g2})
	to [out=east, in=east] ([yshift=0.5ex]{pic cs:engqcstruct_obj});
\end{tikzpicture}
...
\begin{tikzpicture}[remember picture, overlay]
\draw [-latex, min distance=1cm] ([yshift=0.5ex]{pic cs:ayrqcstruct_IP})
	to [out=east, in=west] ([yshift=0.5ex]{pic cs:ayrqcstruct_f});
\draw [-latex, min distance=1cm] ([yshift=0.5ex]{pic cs:ayrqcstruct_NP})
	to [out=east, in=west] ([yshift=0.5ex]{pic cs:ayrqcstruct_g});

\draw [dashed] ([yshift=0.5ex]{pic cs:ayrqcstruct_Top})
	to [out=east, in=east] ([yshift=0.5ex]{pic cs:ayrqcstruct_obj});
\draw ([yshift=0.5ex]{pic cs:ayrqcstruct_Foc})
	to [out=east, in=east] ([yshift=0.5ex]{pic cs:ayrqcstruct_q});
\draw ([yshift=0.5ex]{pic cs:ayrqcstruct_Foc})
	to [out=east, in=east] ([yshift=0.5ex]{pic cs:ayrqcstruct_subj});
\draw ([yshift=0.5ex]{pic cs:ayrqcstruct_Foc})
	to [out=east, in=east] ([yshift=0.5ex]{pic cs:ayrqcstruct_subj});
\end{tikzpicture}
\xe
...
assumed to be a VP, though with an empty head position, the verb instead
appearing as the head of IP. Arrows between the c-structure tree and the \Avm{}
are analogous to the ones in (\ref{ex:engqcstruct}) for easier orientation.

In difference to the English example above, the interrogative pronoun is marked
for case, and thus also encodes animacy for agents and patients. A full line
connects the \Foc{} value to both the value of \Q{} and \Subj{} to indicate
correspondences between \DF{}s and \GF{}s; the connection between \Foc{} and
\Subj{} is also contrary to the English example above, since we are asking for
the subject/agent in this case, not for the object/patient as in the previous
example. (\ref{ex:ayrqcstruct}) also has a patient topic, which is indicated,
though the line connecting \Top{} and its associated \Obj{} is dashed to
indicate that it is there, though not relevant to our interest here.
