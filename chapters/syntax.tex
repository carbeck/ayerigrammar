...
DPs\index{phrase types!determiner phrase} which embed the NP as a complement. Example (\ref{ex:nounmods}) provides an instance of
...
The NP has thus been `extended' in a way, so that one NP is nested within another, as indicated in (\ref{ex:npcstruct}b).
Regarding functional annotation, such an analysis creates a problem, however, since
\citet{bresnan2016} require that if a phrase is adjoined to a phrasal
constituent, this phrase must either not be annotated or not embody an argument
function (\cite[100, 107]{bresnan2016}; also compare (\ref{ex:functions}) of
...
Due to Ayeri's consistent use of case\index{case} marking, I see no problem in invoking a
...
is a complement of \xayr{limu}{limu}{shirt}; see an example in (\ref{ex:complextrapos}). Extraposed relative clauses\index{relative clause} are
less problematic, since they are adjuncts\index{grammatical function!adjunct}, and \Adjc{} is not an
argument function.
...
here that, like Welsh\index{Welsh}, there is a parametric choice not to use the
specifier position \parencite[130]{bresnan2016}. Thus, possessors and other 
determiners have to be analyzed as heads of DP which take an NP complement as a
co-head.
...
additionally refer to a possessor\index{semantic role!possessor}: \rayr{d/naa}{da-nā} in (\ref{ex:nomzposs})
...
the an\-a\-pho\-ric reference, \xayr{d/naalej}{da-nāley}{mine}.
