...
\xayr{yom/}{yoma-}{be, exist} instead of juxtaposition, as in 
...
\Plink{} type \citep[??]{butt1999}. He maintains
\textcquote[105]{attia2008}{the closed complement analysis is the default
...
paradigmatic alternations that do not affect the syntactic function}. Relatedly
he claims that, moreover, the \textcquote[107]{attia2008}{presence or absence
of a copula is a parameter of variation. The copula itself is considered
semantically redundant}. \citet{attia2008}, in reference to
\citet{dalrymple2004}, describes the structure of a copula clause with a
zero-copula essentially in the following way:

\ex\label{ex:copstruct}
	S → \anno*[\pass{\Subj}]{NP} \anno*[%
		\ups{\Pred}~=~\astruct{null-be}{\ups{\Subj} \ups{\Plink}}%
	]{$\epsilon$} \anno*[\pass{\Plink}]{XP}
\xe
...
[103]{attia2008}. XP in this case may be NP, AP, PP, or CP; \fw{null-be} is
basically a dummy since we still need a label for what assigns \Pred{} in the
clause. The phrase structure rule in (\ref{ex:copstruct}) results in the
c-structure---a small clause---in (\ref{ex:copcstruct}) and corresponds to the
f-structure in (\ref{ex:copfstruct}).

\ex\label{ex:copcstruct}
\begin{forest} shorter edges,
[S
	[{\anno[\pass{\Subj}]{NP}}]
	[{\anno[\pass{\Plink}]{XP}}]
