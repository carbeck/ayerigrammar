...
\subsubsection{Gender resolution in third-person plural personal pronouns}
...
referred to by the plural pronoun \fw{them} (number resolution occurs). Since
Ayeri's personal pronouns distinguish gender in the plural as well as in the
singular, however, there needs to be a way to deal with groups whose person
features cannot easily be unified, which an example is given of in
(\ref{ex:mfayeri}). In addition to this, Ayeri has a two-tier system where
three genders---masculine, feminine, neuter---are grouped together as animate,
which is in opposition to inanimate gender, compare (\ref{ex:gramgend}).
...
\a\label{ex:mfayeri}\begingl
	\glpreamble \textsc{context:} Ajān and Pila //
...
agreement, it is to be expected that this feature bundle is simply carried
through the conversation. In either case, an animacy hierarchy operates in that
animate referents outweigh inanimate ones. Mixed animate groups often default
to masculine, though not in all cases. The rules which operate are the
following:
...
Resolution to the animate entity's gender more generally is also possible.
...
as the default. Resolution to the non-asexual entity's gender (\M{} and \F{}
versus \N{}) is possible here as well.
...
Polish, and, to a lesser extent, French that these languages, the first
