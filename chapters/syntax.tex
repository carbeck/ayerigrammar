...
\a \xbar{N} → \anno*{\xbar{N}} $\left(\anno*[\pass{\Possr}]
	{\{NP | DP\}}\right)$
\a \xbar{N} → \anno*{\xbar{N}} $\left(\anno*[\pass{\XCompl}]{NP}\right)$
...
(\ref{ex:ppcstruct}). Prepositional phrases, if not subcategorized by the verb,
are optional information and thus not governed by the verb; the respective PP
is thus part of the set of adjuncts. Some verbs, like \xayr{tpY/}{tapy-}{put},
may take PP complements headed by an adposition, for instance, in cases like
\fw{Mary puts the book {\normalfont [\tsub{PP}~[\tsub{P}} on {\normalfont
]~[\tsub{NP}} the pile {\normalfont ]]}}. The PP \fw{on the pile} is not an
adjunct here because \fw{She does so on the pile} makes no sense; the PP must
be an argument of the verb in this case, not additional, optional information.
...
verbs which subcategorize more than one grammatical function, so a verb whose
argument structure defines, for instance, \astruct{...}{\ups{\Subj} \ups{\Oblq
{loc}}} also qualifies for topic marking even though \Oblq{loc} is not
necessarily a semantic core role for this verb but only an adjunct. There have
already been numerous examples of topic marking above, but for the sake of
completeness, see (\ref{ex:topmkg}). Topic marking also is restricted to the
core f-structure of the verb, that is, a matrix verb cannot mark topic for
arguments of a subordinate verb. Raising is required to make an embedded
argument part of the matrix verb's argument structure. However, as described in
...
The fact that topic marking operates only within a verb's nucleus prohibits
possessors or instrumental complements from being topicalized, since they are
separate, embedded f-structures. They thus have their own predicator that is
not the verb, but a noun which they modify. According to \citep{bresnan2016}'s
annotations, possessees are conceptualized as having the power to subcategorize
a possessor: the noun's \Pred{} value is modified by appending \astruct{...-of}
{\ups{\Poss}}. Presumably, we want instrumental complements in Ayeri to work
along these lines, except that the attributive, rather than possessive,
relationship is expressed by a difference in case marking.
...
which carries the arguments subcategorized by the verb apart from the subject.
Here as well, if there is an adverbial modifier in the VP, it follows the head,
and \xhead{V}'s complement and other modifiers are shifted to the right/up. The
XP in the phrase structure rules in (\ref{ex:vppstruct}) and their c-structure
equivalent in (\ref{ex:vpcstruct}) may be formed by an NP, DP, AP, PP, or CP in
decreasing order on the functional hierarchy; the position may also be left
