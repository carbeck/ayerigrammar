...
\adjustbox{valign=t}{%
	...
	...
		...
		& \begin{tabu}[t]{l l X[l]}
			\ups{\Pred} & = & `...' \\
			\ups{\Tense} & = & \{\NPst, \Pst, \RPst, \NFut, \Fut,
				\RFut\} \\
			\ups{\Asp} & = & \{\Prog, \Hab, \Iter\} \\
			\ups{\Mood} & = & \{\Irr, \Neg, \Imp, \Hort\} \\
...
analyzed as \xhead{C} in German, however, since the non-finite content verb
...
affixes for voice marking which are not obviously related to case markers on
nouns. For instance, non-subject actors are marked by the genitive clitic
\fw{ng}, while actor voice is marked by \fw{mag-} or \fw{-um-} \parencites[74,
78]{schachterotanes1972}[16--18]{kroeger1991}. In Ayeri, on the other hand,
...
predicates when with the nominative NP if the nominative argument of the clause
is plural. This is independent of whether the nominative argument is also the
actor of the clause or not \citep[24--25]{kroeger1991}:
...
The arrows in (\ref{ex:tagagr}) mark government and agreement relationships:
the verb governs role and case assignment (top arrow), while the subject NP
controls plural agreement on the verb (bottom arrow). As the arrows illustrate,
the relationship between the assignment of the subject role and thus nominative
case and plural agreement on the verb are congruent: the verb agrees in both
(\ref{ex:tagagr_1}) and (\ref{ex:tagagr_2}) with the respective subject NP,
whether it is the agent (\ref{ex:tagagr_1}) or not (\ref{ex:tagagr_2}).

As described before, person agreement in Ayeri is essentially fixed to the
agent NP in canonical cases, whether it is the topic of the clause or not.
Thus, in (\ref{ex:ayragr_1}), we can see the verb determining that the agent
argument is also the topic, with the verb agreeing itself in person with the
agent: \rayr{AgYaanF}{Ajān} is a male name. In (\ref{ex:ayragr_2}), however,
...
agreement, while \rayr{pil}{Pila} is a female name. The verb thus continues to
...
agreement on the verb; the agent NP thus remains the subject. This is a very 
\Nom{}--\Acc{} trait.
...
regard to what an \Abs{}--\Erg{} language would be expected to do.
...
Ayeri behaves a little oddly with regards to this, it may be interesting as
well to perform another test on declarative statements and their syntactic
pivot. A simple test which \citet[111--114]{comrie1989} devises in this regard
is to test coreference in coordinated clauses. Commonly, in coordinated
clauses, the subject of the second conjunct may drop out. Thus, in English,
which behaves very much in terms of \Nom{}--\Acc{} alignment in this regard, we
...
What can be observed in Tagalog, thus, is that in (\ref{ex:tagpivot_1}), the
dropped S argument in the second conjunct, \fw{bago umalis ...} `before ...\
leaves', is coreferential with \fw{Marvin}, since he is marked as the subject
of the first conjunct. Since \fw{Marvin} is the theme (P) of \fw{tanong} `ask',
the clause needs to be marked for objective voice. On the other hand, in
...
the agent (A) \rayr{AknF}{Akan} and the recipient (R) \rayr{mrnF}{Maran}. Of
the remaining non-topic NPs, Ayeri considers the agent to rank higher as a
secondary topic on the thematic hierarchy \citep[329]{bresnan2016} than the
recipient. The agent hence forms the preferred controller for
...
floating of \rayr{/ArilF}{-aril} is avoided as well.
...
occupy. There will be further tests to this end in
\autoref{subsec:verbcstruct}.
...
single argument of any caluse can be relativized, that argument must be the
...
accessible for relativization, relativization is not a suitable criterion for
...
the subject. If what was identified as the topic would be the subject like in
...
\begingl
	\gla gusto sila$_i$ ng=Nanay (na) mag-aral \_$_i$ mamayang.gabi //
	\glb want \Tpl{}.\Nom{} \Gen{}=Mother \Comp{} \Av{}-study {} tonight //
...
\a\label{ex:tagrais1_1}\begingl
	\gla gusto ko si=Charlie$_i$ na lutu-in niya$_i$ ang=suman //
	\glb want \Fsg{}.\Gen{} \Abs{}=Charlie \Comp{} cook-\Ov{}
		\Tsg{}.\Gen{} \Abs{}=rice.cake //
...
Presumably, \citet{kroeger1991} switches to labeling the raised NP as \Abs{} in
...
indicating this by object-voice marking. The raised logical subject of the
embedded clause is present as the non-subject actor pronoun \fw{niya}.
\citet{kroeger1991} also unfortunately only gives examples of to-object
raising, but not of to-subject raising \citep[430]{carnie2013}. As we will see
below, Ayeri has no problem with to-subject raising, however, it cannot do
to-object raising. First of all, let us look at to-subject raising, however.

\pex\label{ex:ayrrais1}
\a\label{ex:ayrrais1_1}\begingl
	\gla Surpreng, koronye ang @ Pada$_i$ guratanley. //
	\glb surp=reng koron-ye ang= Pada guratan-ley //
	\glc seem=\Tsg{}.\AargI{} know-\TsgF{} \Aarg{}= Pada answer-\PargI{} //
	\glft `It seems that Pada, she knows the answer. //
\endgl

\a\label{ex:ayrrais1_2}\begingl
	\gla Surpye ang @ Pada$_i$ \_$_i$ koronyam guratanley. //
	\glb surp-ye ang= Pada {} koron-yam guratan-ley //
	\glc seem-\TsgF{} \Aarg{}= Pada {} know-\Ptcp{} answer-\PargI{} //
	\glft `Pada seems to know the answer.' //
\endgl

\a\label{ex:ayrrais1_3}\ljudge*\begingl
	\gla Ang @ surpye Pada koronyam guratanley. //
	\glb ang= surp-ye Pada koron-yam guratan-ley //
	\glc \Aarg{}= seem-\TsgF{} Pada know-\Ptcp{} answer-\PargI{} //
...
In (\ref{ex:ayrrais1}), \rayr{pd}{Pada} is both the topic and the logical
subject of \xayr{koronF/}{koron-}{know}, but not of
\xayr{surFpF/}{surp-}{seem}, as (\ref{ex:ayrrais1_4}) shows. However,
\rayr{pd}{Pada} can be made the subject of the matrix clause, as shown in 
(\ref{ex:ayrrais1_2}). Raising results in an intransitive matrix clause, which
means that topicalizing the only argument of the verb is blocked, as
illustrated by the ungrammaticality of (\ref{ex:ayrrais1_3}). The verb in
(\ref{ex:ayrrais1_2}) also becomes non-finite, like in English. Unlike in
Tagalog, it cannot carry any marking for grammatical relations. Furthermore, it
is possible in Ayeri to form a complex predicate like \rayr{surFpF/ koronYmF}
{surp- koronyam}, literally `seems knowing', with all of the arguments of the
embedded clause becoming arguments of the matrix clause, that is, the matrix
verb may carry topic marking for any of its syntactic (rather than semantic)
arguments:

\ex\label{ex:ayrrais2}\begingl
	\gla Ang @ surpye koronyam Pada guratanley. //
	\glb ang= surp-ye koron-yam Pada guratan-ley //
	\glc \Aarg{}= seem-\TsgF{} know-\Ptcp{} Pada answer-\PargI{} //
...
\a\label{ex:tagrais3_1}\begingl
	\gla malapit na si=Manuel$_i$ na hulihin ng=polis \_$_i$ //
	\glb \Nvol{}-close already \Nom{}=Manuel \Comp{} catch-\Ov{}
		\Gen{}=police {} //
	\glft `Manuel is about to be arrested by the police.' //
\endgl

\a\label{ex:tagrais3_2}\begingl
	\gla napagbintangan ang=pangulo=ng sinuhulan ng=Sindikato \_$_i$ //
	\glb \Nvol{}.\Pfv{}-accuse-\Dv{} \Nom{}=president$_i$=\Comp{}
		\Pfv{}-bribe-\Dv{} \Gen{}=syndicate {} //
...
In (\ref{ex:tagrais3_1}), thus, \fw{Manuel} is the one arrested, so he is the
patient of the subordinate clause which acts as the subject of the matrix
clause. The fact that \fw{Manuel} is a patient-subject of the subordinate verb,
\fw{hulihin} `be caught', is reflected in its being marked for objective voice.
The English translation is consequently given with the subordinate clause
phrased in the passive voice. Similarly, in (\ref{ex:tagrais3_2}), the
subordinate verb, \fw{sinuhulan} `be bribed', is marked for directional voice.
According to this, \fw{ang pangulo} `the president' is a non-actor subject of
the subordinate verb here as well. So it is in the matrix clause as well,
however, as the matrix verb, \fw{napagbintangan} `be accused of', is marked for
directional voice as well.

As we have seen above, the marking of the privileged NP on the verb has
no effect on grammatical relations; making a transitive verb agree with another
NP but the agent NP was also judged questionable. Thus, we would expect Ayeri
to not allow for the same flexibility as Tagalog. The next two sets of example
sentences, (\ref{ex:ayrrais3}) and (\ref{ex:ayrrais4}), thus feature non-actor
topics in the complement clause in the (a) examples which are attempted to be
raised into the subject position of the matrix clause in the (b) examples.

\pex\label{ex:ayrrais3}
\a\label{ex:ayrrais3_1}\begingl
	\gla Surpreng, le @ koronye ang @ Pila guratan$_i$. //
	\glb surp-reng le= koron-ye ang= Pila guratan-Ø //
	\glc seem-\Tsg{}.\AargI{} \PatTI{}= know-\TsgF{} \Aarg{}= Pila
		guratan-\Top{} //
	\glft `It seems that, as for the answer, Pila knows it.' //
\endgl

\a\label{ex:ayrrais3_2}\ljudge*\begingl
	\gla Surpara guratanreng$_i$ ang @ Pila koronyam \_$_i$. //
	\glb surp-ara guratan-reng ang= Pila koron-yam {} //
	\glc seem-\TsgI{} answer-\AargI{} \Aarg{}= Pila know-\Ptcp{} {} //
...
\a\label{ex:ayrrais4_1}\begingl
	\gla Silvreng, yam @ lataya ang @ Maran disaley {} @ Apitu$_i$. //
	\glb Silv=reng yam= lata-ya ang= Maran disa-ley Ø= Apitu //
	\glc look=\Tsg{}.\AargI{} \DatT{}= sell-\TsgM{} \Aarg{}= Maran
		soap-\PargI{} \Top{}= Apitu //
	\glft `It appears that, as for Apitu, Maran sold her the soap.' //
\endgl

\a\label{ex:ayrrais4_2}\ljudge*\begingl
	\gla Silvye ang @ Apitu$_i$ ang @ Maran latayam disaley \_$_i$. //
	\glb silv-ye ang= Apitu ang= Maran lata-yam disa-ley //
	\glc look-\TsgF{} \Aarg{}= Apitu \Aarg{}= Maran sell-\Ptcp{} 
		soap-\PargI{} //
...
between the marking on the verb and subject assignment in Ayeri and also
evidence in favor of an interpretation of the actor NP as the subject, it is
possibly the fact that the subordinate verb appears in a non-finite form when
raising occurs that prevents some of the flexibility of Tagalog observed above.
Even if Ayeri were to work like Tagalog large and by, since finiteness in Ayeri
also includes topic marking, it would not be possible for the non-finite verb
to mark the assignment of grammatical roles to its complements, overt or
covert.
...
\a\label{ex:ayrrais5_2}\ljudge*\begingl
	\gla Ang galamye {} @ Sipra sa @ Ijān sibunjam sangalas. //
	\glb ang galam-ye Ø= Sipra sa= Ijān sibund-yam sangal-as //
	\glc \AgtT{}= expect-\TsgF{} \Top{}= Sipra \Parg{}= Ijān \TsgM{}.\Parg{}
		tidy-\Ptcp{} room-\Parg{} //
...
The example sentences in (\ref{ex:ayrrais5}) shows that to-object raising is
...
{galam-}{expect}, in the way of (\ref{ex:ayrrais2}).
...
clause---subject or object---and this argument is said to act as a controller
for the subject of the subordinate verb. The main clause predicate thus is
thought to assign two thematic roles. In \textsc{gg} it is assumed that the
subject of the lower clause is a silent \textsc{pro} element which is coindexed
with the controller \citep[442--445, 451]{carnie2013}.

\pex\label{ex:engctrl1}
\a Subject control:\medskip \\
	\fw{John$_i$ tries that John$_i$ gets a job}\\
	= \fw{John$_i$ tries \textsc{\upshape pro}$_i$ to \textup{t$_\textsc{pro}$}
	get a job}
\a Object control:\medskip \\
	\fw{The officer ordered Mary$_i$ that Mary$_i$ turn back}\\
	= \fw{The officer ordered Mary$_i$ \textsc{\upshape pro}$_i$ to \textup{t$_\textsc{pro}$} turn back}
\xe
...
a further complication in acceptability, but these details do not need to
...
cases, it is necessarily the actor which is coindexed, as the bottom arrow
shows. In (\ref{ex:ayrctrl1_3}), the bottom arrow does not show coindexing, but
the relation of verb agreement. The arrow on top, as before, shows what the
...
\a\label{ex:ayrctrl2_1}\begingl[aboveglcskip=1.5em, aboveglftskip=1.75em]
	...
	\glc try-\TsgM{}
		...
		{}
		//
	\glft \textit{Literally:} `Maran tried that, as for the chicken, he feeds
		them.' //
\endgl
\begin{tikzpicture}[remember picture, overlay]
\draw [-latex]
...
Again, it is generally possible to use a complement clause 
(\ref{ex:ayrctrl3_1}) as well as complementing the verb in the matrix clause
with a non-finite clause with object control (\ref{ex:ayrctrl3_2}). However,
the incorporation strategy is not possible here because this would cause a
doubling of case roles (\ref{ex:ayrctrl3_3}). As we will see below, however,
...
\a\label{ex:ayrctrl3_1}\begingl%
	[aboveglcskip=1.5em, aboveglftskip=1.75em]
	\gla {Sa pinyaya} {ang Amān} Kagan, \textup{[\tsub{CP}} {ang ilya} kasuley
		{yam Pada} \textup{]}. //
	\glb sa=pinya-ya ang=Amān Ø=Kagan {} ang=ilya kasu-ley yam=Pada {} //
	\glc \PatT{}=ask-\TsgM{}%
		...
		$\underbrace{\text{\Top{}=\smash{Kagan}}%
			...
			\tikzmark{ayrctrl3a_pat1}%
		}$
		{}
		\AgtT{}=give=$\underbrace{\text{\smash{\TsgM{}}.\Top{}}}_{\text{S/A}%
			\tikzmark{ayrctrl3a_sb2}%
		}$%
			\tikzmark{ayrctrl3a_vb2top}
		$\underbrace{\text{basket-\PargI{}}}_{\text{T}}$
		$\underbrace{\text{\Dat{}=Pada}}_{\text{R}}$
		{}
		//
	\glft `Kagan, Amān asks him that he give Pada the basket.' //
\endgl
\begin{tikzpicture}[remember picture, overlay]
\draw [-latex]
	([xshift=-.5*width{"pt=ask-3sg.m"}, yshift=2.25ex]%
		{pic cs:ayrctrl3a_vb1top})
	|- ++ (north:0.75em) -|
...
\draw [-latex]
	([xshift=-.5*width{"p"}, yshift=-.75ex]%
		{pic cs:ayrctrl3a_pat1})
	|- ++ (south:0.75em) -|
	([xshift=-.5*width{"s/a"},yshift=-.75ex]%
		{pic cs:ayrctrl3a_sb2});

\draw [-latex]
	([xshift=-.55*width{"at=give-3sg.m.top"}, yshift=2.25ex]%
		{pic cs:ayrctrl3a_vb2top})
	|- ++ (north:0.75em) -|
	([xshift=-.45*width{"at=give-3sg.m.top"}, yshift=2.25ex]%
		{pic cs:ayrctrl3a_vb2top});
\end{tikzpicture}

\a\label{ex:ayrctrl3_2}\begingl[aboveglcskip=1.5em, aboveglftskip=1.75em]
	\gla {Sa pinyaya} {ang Amān} Kagan \textup{[\tsub{S}} {} ilyam kasuley {yam
		Pada} \textup{]}. //
	\glb sa=pinya-ya ang=Amān Ø=Kagan {} {} il-yam kasu-ley yam=Pada //
	\glc \PatT{}=ask-\TsgM{}%
			\tikzmark{ayrctrl3b_vbtop}%
			\tikzmark{ayrctrl3b_vbbot}
		$\underbrace{\text{\Aarg=Amān}}_{\text{S/A}%
		...
		{}
		//
	\glft `Kagan, Amān asks him to give Pada the basket.' //
\endgl
\begin{tikzpicture}[remember picture, overlay]
\draw [-latex]
	([xshift=-.5*width{"pt=ask-3sg.m"}, yshift=2.25ex]%
		{pic cs:ayrctrl3b_vbtop})
	|- ++ (north:0.75em) -|
	([xshift=-.5*width{"top=Kagan"}, yshift=2.25ex]%
...
\a\label{ex:ayrctrl3_3}\ljudge*%
\begingl[aboveglcskip=1.5em, aboveglftskip=1.75em]
	\gla {Sa pinyaya} ilyam {ang Amān} Kagan kasuley {yam Pada}. //
	\glb sa=pinya-ya il-yam ang=Amān Ø=Kagan kasu-ley yam=Pada //
	\glc \PatT{}=ask-\TsgM{}%
		...
		$\underbrace{\text{\Dat{}=Pada}}_{\text{R}}$
		//
	\glft \textit{Literally:} `Kagan, Amān asks giving him Pila the basket.' //
\endgl
\begin{tikzpicture}[remember picture, overlay]
\draw [-latex]
	([xshift=-.5*width{"pt=ask-3sg.m"}, yshift=2.25ex]%
...
\draw [-latex]
	([xshift=-.5*width{"s/a"}, yshift=-.75ex]%
		{pic cs:ayrctrl3c_sb1})
	|- ++ (south:0.75em) -|
	([xshift=-.5*width{"pt=as-3sg.m"},yshift=-.75ex]%
		{pic cs:ayrctrl3c_vbbot});
\end{tikzpicture}
...
\begingl[aboveglcskip=1.5em, aboveglftskip=1.75em]
	\gla {Sa pinyaya} \textup{[\tsub{VP}} {} ilyam kasuley {yam Pada}
		\textup{]} {ang Amān} Kagan //
	\glb sa=pinya-ya {} {} il-yam kasu-ley yam=Pada {} ang=Amān Ø=Kagan //
	\glc \PatT{}=ask-\TsgM{}%
		...
		//
	\glft \textit{Literally:} `Kagan, Amān asks to give Pila the basket 
		him.' //
\endgl
\xe
...
\ex\label{ex:ayrctrl5}%
\begingl[aboveglcskip=1.5em, aboveglftskip=1.75em]
	\gla {Ang nosaya} nimpyam Amān {sa Pada}. //
	\glb ang=nosa-ya nimp-yam Ø=Amān sa=Pada //
	\glc \AgtT{}=order-\TsgM{}%
			\tikzmark{ayrctrl5_vbtop}%
			\tikzmark{ayrctrl5_vbbot}
		run-\Ptcp{}
		$\underbrace{\text{\Top{}=Amān}%
			\tikzmark{ayrctrl5_top}%
		}_{\text{S/A}%
			\tikzmark{ayrctrl5_sb}%
		}$
