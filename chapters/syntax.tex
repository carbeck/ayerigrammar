...
directional equivalents `up to' and `down from' qualify as atelic as well is
debatable, since they also suggest arriving at a destination eventually. I will
still keep the terminology for the time being, however, for the lack of a
better way to make a binary distinction. Either way, dative and genitive
complements rather relate to the path, whereas locative complements relate more
to the destination.} Instead, \rayr{liNF}{ling} and \rayr{AvnF}{avan} double
for `up' and `down' with dative and genitive complements, respectively, to mark
the difference, compare \autoref{tab:updowncompl}. Note, however, that Ayeri
does not possess intransitive adpositions, with the exception of a few verbs
where the adposition is a lexicalized part of the expression, for instance,
...
\begin{tabu} to \linewidth {I X[c] X[c] X[c] X[c]}
...
\begin{avm}
\[
	\Pred	&	\astruct{bottom of}{\ups{\Obj}} \\
%	\PSem	&	$loc$ \\
	\Tel	&	$+$ \\
	\Obj	&	\[
		\Pred	&	`...' \\
