...
[S
	[NP
		[\textsc{subject}, tier=end]
	]
	[VP
		[\xbar{V}
			[\textsc{verb}, tier=end]
			[NP
				[\textsc{object}, tier=end]
			]
		]
	]
]
...
Such a structure, though, does not do Ayeri justice in that the verb shows
agreement usually with the constituent expressing the actor. Let us tentatively
assume, thus, that the actor NP is priviledged over the undergoer NP. The verb
in the initial position also shows inflection, so one might rather construe it
as an \xhead{I}, projecting an IP, which frees up VP for other purposes. In
fact, \citet{chungmccloskey1987} come to the conclusion that Irish, which is
also a VSO language, organizes its constituents in the way depicted by
(\ref{ex:irishcstruct}). \citet{bresnan2016} give the charts in
(\ref{ex:welshcstruct}) for Welsh, equally a VSO language (also compare \cite
[66]{dalrymple2001}, which sources \cite{sadler1997}). \citet{kroeger1991}
...
\a Welsh \citep[134]{bresnan2016}:\label{ex:welshcstruct}\medskip \\
\begin{forest} shorter edges,
[IP
	[\anno{I}
		[{\textit{gwelodd}\\ `see-\Tsg{}.\Pst{}'}]
	]
	[\anno{S}
		[{\anno[\pass{\Subj}]{NP}}
			[{\textit{Siôn}\\ `John'}]
...
Apparent V-to-I movement (except in \Lfg{} it's base-generated) etc. etc. We
will need to test whether this analysis holds true for Ayeri as well.
