...
structure of NPs should thus look like depicted in (\ref{ex:npstruct}).
...
(\ref{ex:nounmodord}) illustrates the unmarked order of several different modifiers.
...
\ex\labels\label{ex:romdef}%
	Romanian \citep[297]{dindelegan2013}:\medskip\\
	\begin{minipage}[t]{.5\remaining}
	\tl\quad\begingl
		\gla acest om //
		\glb acest-Ø om-Ø //
...
morphemes; pronominal clitics cannot be modified by quantifiers as described in
\autoref{subsec:reflrec}. A reflexive particle is needed here to attach the
quantifier to in order to avoid ambiguities in scope. The examples in
(\ref{ex:nonounmods}) illustrate key differences between nouns and personal
pronouns with regards to the distribution of modifiers.
