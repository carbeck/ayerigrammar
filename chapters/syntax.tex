...
(\ref{ex:vppstruct}), with its translation as a c-structure tree in
(\ref{ex:vpcstruct}).

\begin{figure}
\pex\label{ex:vppstruct}
\a IP → \anno*{\xbar{I}}
\a \xbar{I} → \anno*{\xbar{I}} $\left(\anno*{\{S | CP\}}\right)$
\a \xbar{I} → \anno*{\xhead{I}} $\left(\anno*[\elem{\Adjc}]{AdvP}\right)$
\a \xhead{I} → \anno*{Cl} \anno*{\xhead{I}}
\xe
\end{figure}
\begin{figure}
...
The explanation about YP in the definition of S given above (compare
(\ref{ex:copstruct})) stated that there may also be a VP complement of a
nominal fulfilling the role of a predicate. Since VP carries the non-\Subj{}
arguments of a verb, making a clause transitive, I want to defer the discussion
of VP's setup to the next section, \ref{subsec:vps}. This means, we will only
be dealing with intransitive clauses in this section.
