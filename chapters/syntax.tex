...
Example (\autoref{ex:copadv_2}) shows that \xayr{kYuymF}{cuyam}{really} is
...
be nominalized as \rayr{d/kYuymF}{da-cuyam}{really so}. The permutation test in
(\ref{{ex:copadvperm}}) reveals that all sequences are licit. Thus, I will
assume the structure in (\ref{ex:copadvstruct}) for the example in
(\ref{ex:copadv_1}): since copula clauses form a small clause constituent,
S may govern more than two elements. Case marking ensures that the different
phrases map on their intended grammatical functions.

\begin{figure}
\ex\label{ex:ex:copadvstruct}
\begin{minipage}[t]{.5\remaining}
\begin{avm}
\[
	\Pred	&	\astruct{null-be}{\ups{\Subj} \ups{\Plink}} \\
