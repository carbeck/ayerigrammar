...
\tl\quad\label{ex:daadjcaseavm} \adjustbox{valign=t}{%
\begin{avm}
\[\Obj & \[
			\Pred	& `pro' \\
			\Deix	& $such$ \\
...
\Adjc{} relation (\ref{ex:daadjcaseavm}). Since \Lfg{} postulates that all
...
encodes \ups{\Pred} = `red'. Now, if they are both dominated by the same node
(\xhead{A} in \ref{ex:daadjcase}), that node would have clashing values for
...
value}, that is, no attribute (like \Pred{}) may have more than one value
assigned to it. Thus, in order not to violate this principle, the question is
whether a list of \Pred{} attributes as values of an anonymous attribute within
a function (like \Adjc{} lists, or whenever coordination is involved) would be
acceptable:
...
\noindent Either way, \citet{bresnan2016} mention the problem of complex
predicates---the authors admit that since \textcquote[346]{bresnan2016}{each
\Pred{} is uniquely instantiated [...], two parts of the same \Pred{} cannot be
unified in the formalism presented in this book}.
