...
of a passive. Other semantic roles are annotated [–\,o] as well. The roles in
(\ref{ex:argfunctfeat}) map to the closest available role in the thematic
hierarchy (\ref{ex:themhier}) \citep[329]{bresnan2016}.
...
the agent, \fw{John}, with the most prominent semantic role (\thetahat), which
...
\subsubsection{Passives of transitive verbs}
...
patient subject and to reintroduce the agent as an oblique argument in either
the agent or the instrumental case (\ref{ex:passzn2_inst}). Instead, one may
rather keep the active phrasing and choose to topicalize the patient argument
as illustrated in (\ref{ex:passzn2_pt}).
...
\subsubsection{Passives of ditransitive verbs}
