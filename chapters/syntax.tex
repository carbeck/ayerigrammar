...
\a\label{ex:nomdepict_1}\begingl
...
\a\label{ex:nomdepict_2}\begingl
...
\a\label{ex:restrns_pp2}%
	\begingl
		...
		\gla Ang @ hiyaye {} @ Gada suranley \textbf{manga} @ \textbf{avan}
			\textbf{turayyam}. //
		\glb ang= hiya-ye Ø= Gada suran-ley manga= avan turay-yam //
		\glc \AgtT{}= roll-\TsgF{} \Top{}= Gada ball-\PargI{} \Dir{}= bottom
			hill-\Dat{} //
...
as is the case with \xayr{mN AvnF turjymF}{manga avan turayyam}{down the hill}
in (\ref{ex:restrns_pp2}): compared to the other examples in
(\ref{ex:restrns}), this phrase consists of three words and contains its own
complement. While (\ref{ex:restrns_np}) also contains a modifier, this phrase
only consists of two words, and the modifier is not an argument but an adjunct.
The phrase is thus relatively more light and thus more easily parseable.
...
apparent here:
% The examples in (\ref{ex:respass}) illustrate the change from an active
% transitive sentence to an intransitive passive one. Note, however, that in
% \Lfg{} passivization is not analyzed as a transformational process, but as a
% lexical one. According to theory, there are two lexical entries for
% \xayr{hyrF/}{hayar-}{chop} here, one active, one passive. In
% (\ref{ex:respass_a}), the subject and the object of the verb are given with
% the annotations [–\,o] and [–\,r] for `unobjective' and `unrestricted',
% respectively, which are the key semantic features of these roles: subjects
% are not complements, and objects are not restricted as to their semantic role
% \citep[324\psqq]{bresnan2016}. If the [–\,o] argument of the verb drops out
% in the passive version of the verb, the subject function will move up to the
% next argument, which is [–\,r], as shown in (\ref{ex:respass_p}).
unlike with unaccusative verbs,
% where marking of the [–\,r] subject changes to agent,
Ayeri retains the patient case marking for subjects of passive verbs. The
[–\,r] marking on the passive subject refers to its role in the argument
structure as thematically unrestricted \citep[324\psqq]{bresnan2016}.
...
Adding the reflexive as an object has the advantage of being able to serve as a
subject for the resultative adjective \rayr{pisu}{pisu} in the \XCompl{}
function, however. This way, the state achieved by running---being tired---can
be attributed indirectly to the controller of the reflexive, the subject
\rayr{tiplF}{Tipal}. The \Obj{} function in (\ref{ex:fakerefl}) has been added
to the argument structure of the verb. As its position outside of the pointed
brackets shows, this object---the fake reflexive---does not receive its
syntactic role from the argument structure of the verb, so it must be athematic
(compare \autoref{subsubsec:raisvb}, p.~\pageref{subsubsec:raisvb}).
\xayr{niMpF/}{nimp-}{run} is normally intransitive and thus does not
subcategorize for an object in its argument structure, compare
(\ref{ex:runtranswrong}). Adding the reflexive as an object has the advantage
of being able to serve as a subject for the resultative adjective
\rayr{pisu}{pisu} in the \XCompl{} function, however. This way, the state
achieved by running---being tired---can be attributed indirectly to the
