...
(\ref{ex:adjres}). The sentence in (\ref{ex:adjres_pp2}) is adapted from
...
\a\label{ex:adjres_np}%
	\begingl
		\glpreamble object NP + resultative NP: //
		\gla Ang @ visaya \textbf{ku-nernanye} \textbf{kivo} ang @ Amān
			seygoley. //
		\glb ang= visa-ya ku=nernan-ye kivo ang= Amān seygo-ley //
		\glc \AgtT{}= cut-\TsgM{} as=piece-\Pl{} small \Aarg{}= Amān 
			apple-\PargI{} //
		\glft `Amān cuts the apple into little pieces.' //
	\endgl

\a\label{ex:adjres_pp1}%
	\begingl
...
in (\ref{ex:adjres_pp2}): compared to the other examples in (\ref{ex:adjres}),
this phrase consists of three words and contains its own complement. While
(\ref{ex:adjres_np}) also contains a modifier, this phrase only consists of two
words, and the modifier is not an argument but an adjunct. The phrase is thus
relatively more light and thus more easily parseable.

According to \citet{simpson1983}, the structure all of the examples in
(\ref{ex:adjres}) have in common is something along the lines of what is
illustrated by (\ref{ex:adjresstr}). This, however, is an interpretation which
...
requiring a subject and a predicative element as its arguments here as well.
This f-structure $g$ forms an open complement of the verb in $f$. The \XCompl{}
has no structural equivalent in the c-structure tree in (\ref{ex:adjresstr}),
however, the XP node of the resultative element should be annotated
\pass{\XCompl{} \Plink}. This way, \XCompl{} is represented functionally.
...
mandatory---an effect born out of the many-to-one correspondence between the
mapping from c-structure into f-structure (the $\phi$ function). Furthermore,
with the economy of expression principle of \Lfg{} in mind, it maybe reasoned
that since the resultative XP is also a complement of the verb according to
both \citet{simpson1983} and \citet{christie2013}, we probably do not want it
to be included inside an S sister of the object NP if there is usually nothing
present in this place. As we have seen, the resultative mostly occupies the
spot to the right of the verb, so this S would mostly not occur due to pruning
empty nodes. There is little reason, thus, to include it just to force a
one-to-one correspondence between f- and c-structure (the $\phi^{-1}$ function)
for the minority of cases.

\begin{figure}
\ex\label{ex:adjresstr}
\begin{minipage}[t]{.5\remaining}
...
$f$: \[
	...
	\Obj	&	\[
		\avmspan{...} \\
	\] \tikzmark{adjresstr_obj} \\
	
	\XCompl	&	$g$: \[
		\Subj	&	\tikzmark{adjresstr_subj2} \\
		\Pred	&	\astruct{null-be}{\ups{\Subj} \ups{\Plink}} \\
		\Plink	&	\[
			\avmspan{...} \\
		\] \tikzmark{adjresstr_plink} \\ % ~\enspace~\\
	\] \tikzmark{adjresstr_xcomp} \\ % ~\enspace~\\
\]
\end{avm}
\end{minipage}
\hfill
...
[IP
	[\xbar{I}
		[\xhead{I}]
		[XP\tikzmark{adjresstr_XP1}]
	]
	[S
		[NP]
		[VP
			[\tikzmark{adjresstr_NP}NP]
%			[\tikzmark{adjresstr_S}S
				[(XP)\tikzmark{adjresstr_XP2}]
%			]
		]
	]
]
\end{forest}
\begin{tikzpicture}[remember picture, overlay]
\draw [rounded corners=1ex] ([yshift=1ex]{pic cs:adjresstr_obj})
	-- ++(east:5em) |- ([yshift=1ex]{pic cs:adjresstr_subj2});
%
\draw [-latex] ([yshift=1ex]{pic cs:adjresstr_plink})
	to [out=east, in=south]
	([xshift=-.5*width("XP"), yshift=-.5ex]{pic cs:adjresstr_XP1});
%
\draw [-latex, dashed] ([yshift=1ex]{pic cs:adjresstr_plink})
	to [out=east, in=south]
	([xshift=-.5*width("(XP)"), yshift=-.5ex]{pic cs:adjresstr_XP2});
%
\draw [-latex] ([yshift=1ex]{pic cs:adjresstr_obj})
	to [out=east, in=west]
	([xshift=-width(" "), yshift=1ex]{pic cs:adjresstr_NP});
%
% \draw [-latex] ([yshift=1ex]{pic cs:adjresstr_xcomp})
% 	to [out=east, in=south west]
% 	([xshift=-width(" "), yshift=1ex]{pic cs:adjresstr_S});
\end{tikzpicture}
\xe
\end{figure}
