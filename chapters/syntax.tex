...
% THIS IS NOT COMPLETELY CERTAIN??? IF INTENSIFIERS MODIFYING QUANTIFIERS *ARE*
% NON-PROJECTING WORDS, WOULDN'T SPEC,NP BE FREE FOR QUANTIFIERS?
Quantifiers can be modified by intensifiers, so it makes sense to treat them as
phrasal heads. However, exceptionally, independent quantifiers happen not to be
semantically compatible with modification by intensifiers like \xayr{EkeNF}
{ekeng}{over-, overly, too} or \xayr{kgnF}{kagan}{excessively, far too}.
Considering the clitic quantifiers, however, such an approach may be
preferable, as we will see further on. Otherwise, we would have to treat the
likes of \xayr{heNsF}{hengas}{almost all},
...
specifier of \xhead{D}. Instead, it has been analyzed here as a non-projecting
word \citep[116--117]{bresnan2016} for lack of a better understanding.
