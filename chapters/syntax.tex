...
\a\label{ex:adjmod_dp}%
	\begin{minipage}[t]{.667\remaining}
	\begingl
		\glpreamble adjective + DP \enquote{complement}: //
		\gla Yang mino ankyu yayam. //
		\glb yang mino ankyu yayam //
		\glc \Fsg{}.\Aarg{} happy really \TsgM{}.\Dat{} //
		\glft `I am really happy for him.' //
	\endgl~\\
...
\a\label{ex:adjmod_cp}%
	\begin{minipage}[t]{.5\remaining}
	\begingl
		\glpreamble adjective + CP \enquote{complement}: //
		\gla Yang valuy ankyu, yomavāng edaya. //
...
(\ref{ex:adjmod}) are complements rather than adjuncts, though, especially
since they don't occur in the position normally expected for a complement?
X-bar theory suggests that complements and adjuncts are on different \xbar{X}
branches---a complement is a sister to the head whereas an adjunct is, so to
speak, its niece:
...
English has the quantifying adverb precede the adjective in cases like the ones
given above, so that the complement remains a complement. Ayeri, however, has
modifiers very consistently follow their heads. Thus, as it looks like for
(\ref{ex:adjmod_dp}) and (\ref{ex:adjmod_cp}), the DP and CP, respectively,
have been kicked out of the complement position they would usually occupy by
the quantifier \xayr{AMkYu}{ankyu}{really} and moved to the end of the phrase:

\pex
\a \textit{Yang mino yayam.}
\a \textit{Yang valuy, yomavāng edaya.}
\xe

\pex~\label{ex:intrusivequant}
\a \textit{Yang mino \_\tikzmark{intrusivequant1_1} ankyu 
	yayam\tikzmark{intrusivequant1_2}.}\medskip
\a \textit{Yang valuy \_\tikzmark{intrusivequant2_1} ankyu, 
	$[$yomavāng edaya\tikzmark{intrusivequant2_2}$]$.}

\begin{tikzpicture}[remember picture, overlay]
\draw [-latex] ([xshift=-.25em,yshift=-.75ex]{pic cs:intrusivequant1_1}) |- ++
(south:.75em) -| ([xshift=-.5*width("yayam"),yshift=-.75ex]{pic
cs:intrusivequant1_2});
...
structure is what creates a sense of cohesion. Thus, even though the DP and CP
of (\ref{ex:adjmod_dp}) and (\ref{ex:adjmod_cp}) appear dislocated, the
information is still found in the `right' place in f-structure independent of
the diverging c-structure, compare the \Avm{}s in those examples. In
(\ref{ex:adjmod_dp}), thus, the adjective is given as having an oblique nominal
complement, and what surfaces as the DP adjunct is annotated accordingly as
\Oblq{goal}, not as \Adjc{}. The same goes for (\ref{ex:adjmod_cp}), where the
adjective is indicated to have a phrasal complement, which the CP supplies even
though in c-structure it appears in the position of an adjunct.
...
%
\draw (marked) circle [radius=1em];
\end{forest}
\xe

If we check replacement by \rayr{d/}{da-} for the other examples in
(\ref{ex:adjmod}), we will see that this is not possible, since the outcomes
produced are ungrammatical (\ref{ex:soreplacement_2}). And even ignoring the
adverb \rayr{AMkYu}{ankyu}, the sentences in (\ref{ex:soreplacement_2_2}) and
(\ref{ex:soreplacement_2_3}) do not become more grammatical
(\ref{ex:soreplacement_3}).
...
\a\label{ex:soreplacement_2_2}\ljudge*\begingl
	\gla Yang da-ankyu yayam. //
	\glb yang da=ankyu yayam //
	\glc \Fsg{}.\Aarg{} so=really \TsgM{}.\Dat{} //
	\glft `I am really so for him.'\\
		\textit{Literally:} `I am him really so.' //
\endgl

\a\label{ex:soreplacement_2_3}\ljudge*\begingl
	\gla Yang da-ankyu yomavāng edaya. //
	\glb Yang da=ankyu yoma=vāng edaya //
	\glc \Fsg{}.\Aarg{} so=really exist=\Second{}.\Aarg{} here //
	\glft `I am really so you are here.' //
\endgl
\xe

\pex~\label{ex:soreplacement_3}
\a\label{ex:soreplacement_3_1}\ljudge*\begingl
	\gla Yang da-yayam. //
	\glb yang da=yayam //
