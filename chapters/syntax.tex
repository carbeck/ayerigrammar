...
understood as a discontinuous constituent and hence annotated \elem{\Plink}
rather than just \ups{\Plink}. The complement of the adjective is generated in
its canonical position as a daughter of the AP in S. Alternatively, it should
be possible for the \xhead{A} not to be fronted, just as the subordinate verb
in (\ref{ex:ctrlstruct}) and (\ref{ex:raisestruct}) appears as the head of the
subordinate VP. However, this leads to ambiguity in that an adjective, then,
directly follows a noun, so modification relationships would not be entirely
clear. The preferred way is, thus, to generate the adjective as a complement of
\xhead{I}.
...
[IP
	[\anno{\xbar{I}}
		...
		[{\anno[\elem{\Plink}]{AP}}% \tikzmark{compcstruct_par}
			[para]
		]
	]
	[\anno{S}
		...
		[\anno{VP}
			[{\anno[\elem{\Plink}]{AP}}% \tikzmark{compcstruct_ap}
...
So far, we have only considered comparison of adjectives, but NPs may undergo
comparison as well, though again, if modified by an adjective.
