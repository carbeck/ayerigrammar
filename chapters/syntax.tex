...
Example (\ref{ex:copadv_2}) shows that \xayr{kYuymF}{cuyam}{really} is
...
{yang}{I} in (\ref{ex:copadv_3}). If it were the predication, it would have to
...
structure in (\ref{ex:copadvstruct}) for the example in (\ref{ex:copadv_1}).
...
least for the verb \fw{berichten} `tell, report, relate'. This is illustrated
by the example sentences in (\ref{ex:gerditrpass}). In
...
Names in German are only marked for genitive case, but replacing \fw{Anna} with
a pronoun makes case marking more clear. If we replace \fw{Anna} with the
