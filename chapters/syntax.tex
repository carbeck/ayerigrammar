...
\pass{\XCompl{} \Plink}. This way, \XCompl{} is represented functionally. Since
the resultative XP is a complement of the verb according to both
\citet{simpson1983} and \citet{christie2013}, we probably do not want it to be
included inside an S sister of the object NP (annotated \pass{\XCompl}) if
there is anything present in this place at all. As we have seen, the
resultative mostly occupies the spot to the right of the verb, so this S would
mostly be pruned anyway according to \Lfg{}'s version of X-bar theory.
