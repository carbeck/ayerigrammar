...
\a \xbar{N} → \anno*{\xhead{N}} \anno*[\pass{\Compl}]{XP}
\xe
\end{figure}
...
clause, CP), adjunct (adjective phrase, AP), and relative clause (CP). This can
be represented as a constituent-structure tree in the way described in
(\ref{ex:npcstruct}). The maximal projection of \xhead{N} (that is, NP) is
...
Besides, nouns may be modified by possessors and quantifiers. These, however,
may better be analyzed as forming DPs which embed the NP as a complement.
...
nominal complement with \fw{one} in English is acceptable at least to some
