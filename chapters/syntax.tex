...
of the presence of a numeral, for instance as in (ex:plovermkg).
...
(\ref{ex:advoradj}). Due to \Lfg{}'s functional approach, however, both
c-structures in (\ref{ex:ayradvorder}ab) map to the f-structure shown in
...
for case. Personal pronouns occur in an undeclined base form if topicalized
(see \autoref{subsec:perspro}). Morphologic topic marking is limited to finite
verbs which subcategorize for more than one grammatical function, so a verb
whose argument structure defines, for instance, \astruct{...}{\ups{\Subj}
\ups{\Oblq {loc}}} also qualifies for topic marking even though \Oblq{loc} is
not necessarily a semantic core role for this verb but only an adjunct. There
have already been numerous examples of topic marking above, but for the sake of
completeness, see (\ref{ex:topmkg}). Topic marking also is restricted to clause
level, that is, a matrix verb cannot topic-mark arguments of a subordinate
verb. Raising is required to make an embedded argument part of the matrix
verb's argument structure. However, as described in
...
(\ref{ex:nptopcorr}) tries to capture. The fact that topic marking operates
only within a verb's nucleus prevents possessors or instrumental complements
from being topicalized, since they are separate, embedded f-structures. They
are subcategorized for, but not by the verb, but by the noun which they modify.
...
\astruct{...-of}{\ups{\Poss}}---at least in their English examples \citep[315]
{bresnan2016}. I will, however, omit the \fw{-of} and more generally indicate
nouns taking a possessor as \astruct{noun}{\Poss} with the embedded NP marked
for genitive case. Presumably, we want instrumental complements in Ayeri to
work along these lines as well for the reason of structural similarity of the
construction, except that the attributive, rather than possessive, relationship
of the complement is expressed by a difference in case marking: instrumental
instead of genitive. An example is given in (\ref{ex:transnoun}).

% FIXME: § 4.1.3 contains an example where a possessor NP has been topicalized.
% But normally, only arguments of the verb can be topic-marked?
...
\begin{minipage}[t]{.5\remaining}%
\begingl
	\gla Ang @ vacay veneyas na @ Kaman. //
	\glb ang= vac=ay.Ø veney-as na= Kaman //
	\glc \AgtT{}= like=\Fsg{}.\Top{} dog-\Parg{} \Gen{}= Kaman //
	\glft `I like Kaman's dog.' //
\endgl
\end{minipage}
~
\begin{avm}
$f$: \[
	...
	\Top	&	\[
		\avmspan{``I''} \\
	\] \tikzmark{npembedfull1_top} \\

	\Subj	&	[~\tikzmark{npembedfull1_subj}~] \\
...
\draw [rounded corners=1ex] ([yshift=1ex]{pic cs:npembedfull1_top})
	-- ++(east:1em) |- ([yshift=1ex]{pic cs:npembedfull1_subj});
\end{tikzpicture}
...
\begingl
	...
	\glft \textit{Intended:} `Kaman, I like his dog.' //
\endgl
\end{minipage}
~
\begin{avm}
$f$: \[
	...
	\Obj	&	\[
		\Pred	&	\astruct{dog}{\ups{\Poss}} \\
		\Poss	&	$g:$ [~\tikzmark{npembedfull2_poss}~] \\
	\]~\hspace{2em} \\
\]
\end{avm}
\begin{tikzpicture}[remember picture, overlay]
...
the verb. In (\ref{ex:npembedfull}), the subject pronominal suffix has been
topicalized and thus only appears as \rayr{/Aj}{-ay} instead of full
\rayr{/yNF}{-yang}. If we try to topicalize the possessor, though, as in 
(\ref{ex:npembedfull_2}), the relationship between topic and its corresponding
function, \Poss{}, skips into a subordinate f-structure which is not an
argument of the verb.

...

% IDEA: Kamanᵢ, I like himᵢ hisᵢ dog (Yam vacyang Kaman veneyas yana). This
% should also work for pronominal binding, since the binder precedes the
% bindee, the binder is a free nominal and the pronoun is likewise free in its
% nucelus. However, it is unclear how POSS ranks in terms of the functional
% hierarchy (Bresnan et al. 2016: 100), so I don't know if OBJθ > POSS (by
% intuition, yes, if only by precedence, both in f- and c-structure).

\subsubsection{Clitic quantifiers}
...
DP, AP, PP, or CP in decreasing order on the functional hierarchy; the position
may also be left unoccupied for intransitive verbs.

\begin{figure}
\pex\label{ex:vppstruct}
\a VP → \anno*{\xbar{V}} \anno*[\pass{\AF}]{XP*}
