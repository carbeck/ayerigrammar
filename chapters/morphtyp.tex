...
\citet{spencerluis2012}, with recourse to \citet{zwickypullum1983}, some typical characteristic traits are:
...
order to drive their point home that the border between clitics and affixes is
often fuzzy. Given this definitional fuzziness, it comes as no surprise that
...
is only of limited value. The traits listed above are thus sufficient
...
some additions and corrections. Since clitics sit at the junction of morphology and syntax, it will be necessary at times to deal with topics roughly which will be elaborated on later in more detail.}
...
transparently derived from nouns. \rayr{mN}{manga} marks on adpositions that
there is motion along a path. The dynamic marker \rayr{mN}{manga} is thus a
functional morpheme and it always appears before the adposition itself.
Prepositions do not otherwise inflect, but \rayr{mN}{manga}, due to its
functional nature, could reasonably be construed as inflection, in spite of
appearing as a function word. This double nature makes it a good candidate for
a clitic. Applying a shuffling or coordination test here to figure out whether 
...
syntactically privileged; \rayr{mN}{manga} exhibits special syntax. Even
though it may be derived from the verb \rayr{mN/}{move}, the latter does not
constitute the particle's asssociated full form:
...
\a\label{ex:clitics_manga2}\ljudge{!}\begingl
...
Example (\ref{ex:clitics_manga2}) assumes that the correct place of the verb
\xayr{mN/}{manga-}{move} to appear in is as an infinite complement to the main
verb in the sentence, \xayr{sr/}{sara-}{go}. While not ungrammatical \fw{per
se}, the sentence would imply that \rayr{AgYaanF}{Ajān} walks away in order to
move around in the house, which is not what (\ref{ex:clitics_manga1})
posited. There is thus reason to assume that \rayr{mN}{manga} is a special
clitic in \citet{zwicky1977}'s terminology. When testing with parenthetical 
word material, it becomes clear that \xayr{mN koNF}{manga kong}{into} indeed
forms a syntactic unit, which is demonstrated in 
(\ref{ex:clitics_mangacohesion}). \rayr{mN}{manga} is a bound morpheme, and
thus not a function word proper with adpositions as well.
...
the extent that two adpositions in coordination---\xayr{midj}{miday}{around} 
and \xayr{koNF}{kong}{inside}---share the same adpositional object, 
\xayr{nN}{nanga}{house}.
...
and has aspects of both function words and inflection.
...
\a\label{ex:clitics_50c} \ljudge{!} \fw{kipisānye bino-ikan kāryo} \\
	`very colorful big paintings'

\a\label{ex:clitics_50d} \ljudge{!} \fw{kipisānye bino kāryo-ikan} \\
...
\a\label{ex:clitics_51c} \ljudge{!} \fw{kipisānye bino-ikan nay kāryo} \\
...
\a\label{ex:clitics_52b}\ljudge{?}\begingl
...
\a\label{ex:clitics_53a}
	\ljudge{?} \fw{keynam}[\fw{-ikan soyang -kay}] \\
		`[few or many] people'
...
\a\label{ex:clitics_59a}\begingl
	...
	\glc \AgtT{} own=\TplM{}.\Top{} book-P and map-\PargI{}=many //
