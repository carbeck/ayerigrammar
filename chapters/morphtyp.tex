...
\rayr{/ymF}{yam}. Example (\ref{ex:relative}) is one we have already encountered 
...
\footnote{\rayr{mN}{manga} has, in fact, a verbal 
...
following example in which \rayr{miNF}{ming} is inflected like a regular verb:
...
which it links the relative clause. This typically happens---in terms of linear 
succession of words at the surface level of the clause---if the relativizer 
cannot be immediately adjacent to the NP which the relative clause modifies, for 
example, when an adjective or a possessive pronoun is following the noun:
