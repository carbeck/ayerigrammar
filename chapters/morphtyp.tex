...
demonstrative prefixes\index{prefixes} on nouns. In the following example, the prefix\index{prefixes} 
...
border between inflection and function word. This is the clitic\index{clitics}
...
better parallel might be the so-called \fw{rheinische verlaufsform} `Ripuarian
...
%
In (\ref{ex:dyn}), \rayr{mN}{manga} modifies the preposition\index{adpositions!prepositions},
to indicate that it is directional: \rayr{lug}{luga} by itself means
...
\Tsg{}-\F{}-\Aarg{}). This also gives an explanation to \posscite{boga2016} 
...
instrument\index{semantic role!instrument}, so both of the sentences in (\ref{ex:sitangpronvb}) are equivalent in meaning.
...
the prefix \rayr{d/}{da-} can be used with NPs\index{phrase types!noun phrase} and is part of
...
\xayr{Ikpis}{ikapisa}{bound, dependent}, which is an example of combination 
...
are counted as parts of the IP\index{phrase types!inflectional phrase} but they do not interact with it regarding stress\index{stress}
assignment: topic\index{grammatical function!topic} particles are always unstressed\index{stress}. Besides this, they are consistently
placed in clause-initial position, preceding\index{word order} any other preverbal elements. 
...
be marked for plural\index{number!plural} with the suffix \rayr{/ye}{-ye}, and verb\index{verbs} roots may
...
As mentioned above, both quantifiers\index{quantifiers} and intensifiers may appear as enclitics\index{clitics} on almost any type of
content word, like \xayr{/m}{-ma}{enough} on the adverb \xayr{pr}{para}{fast} in (\ref{ex:advclts}),
for instance.
...
This morphophonemic process can be reinterpreted so that vowel\index{vowels} length/stress\index{stress} itself is what signifies
the agreement\index{agreement} of the relativizer\index{pronouns!relative} with its antecedent. Which grammatical role
the relativizer's\index{pronouns!relative} antecedent represents is
...
\phantomsection\label{clitics_preverb}
...
subject NP \xayr{nmaaNF smF —}{namāng sam \dots}{two legs \dots} precede the verb
...
\phantomsection\label{clitics_preverb_top}
...
\phantomsection\label{clitics_preverb_prog}
...
patients\index{case!patient}---the animacy\index{animacy} value of the topicalized\index{grammatical function!topic} NP, so the
...
arguments: a subject\index{grammatical function!subject} and an oblique\index{grammatical function!oblique} argument in the form of a location
...
\phantomsection\label{clitics_preverb_modal}
...
Unlike verbs, modal particles\index{modals} in Ayeri resist
...
supposed to} with an irrealis-marked\index{mood!irrealis} verb together indicates that the speaker
...
constituent parts of the whole verb complex. The same goes for negation\index{negation}: only
...
and verb\index{verbs}, likewise cannot be interrupted by parenthetical material like
...
\phantomsection\label{clitics_prenoun_case}
...
\phantomsection\label{clitics_prep_dyn}
...
even though it may be derived\index{derivation} from the verb \xayr{mN/}{manga}{move}, this
...
\phantomsection\label{clitics_preverb_da}
...
prefixes\index{prefixes}, which \emph{do} express a property of the verb, as shown in 
(\ref{ex:clitics_15}).
...
The \rayr{d/}{da-} prefix\index{prefixes} satisfies the criteria of being a
...
wide scope\index{scope} over conjuncts, is attached outside of inflection for
...
is also no way in (\ref{ex:clitics_18}) to place words between \rayr{d/}{da-}
and the verb stem.
...
\phantomsection\label{clitics_preverb_refl}
...
also does not have\index{typology} any verbs which appear as grammatically reflexive to
indicate unaccusativity like in Romance languages; see (\ref{ex:clitics_19}) and
(\ref{ex:clitics_20}) for comparison. The reflexive\index{pronouns!reflexive} marking in
...
\phantomsection\label{clitics_prenoun_dem}
...
occurs in Turkish\index{Turkish}, for instance, where the plural suffix 
\fw{-l\textsc{e}r} and subsequent suffixes can be left out in coordination
...
marker and the proper noun\index{nouns!proper} itself. This is unproblematic for lexical
...
\phantomsection\label{clitics_preadj_da}
...
be reduplicated for diminution, and also cannot be pluralized\index{number!plural}. Since
...
adjectives\index{adjectives} results in an individuating reading, as in (\ref{ex:clitics_29a}). It
...
simple clitic according to \posscite{zwicky1977} definition, compared to the
...
case. However, the nature of the condition is not determined in
\citet{spencerluis2012}, when they elaborate on \posscite{zwicky1977} definition
that
...
\phantomsection\label{clitics_prenoun_ku}
...
adjuncts\index{grammatical function!adjunct}) follow\index{constituent order} nouns and cannot appear in the position of \rayr{ku/}{ku-}.
...
also have scope\index{scope} over two individuated NP\index{phrase types!noun phrase} conjuncts. Besides nouns,
\rayr{ku/}{ku-} is applicable to pronouns\index{pronouns} as well, which makes
...
As (\ref{ex:clitics_35b}) shows, \rayr{ku/}{ku-} can have wide scope\index{scope} over
conjuncts. What further distinguishes \rayr{ku/}{ku-} from a prefix\index{prefixes} here is
that it does not undergo crasis if the adjective\index{adjectives} begins with an /u/, hence we find
...
\phantomsection\label{clitics_preverb_ku}
Besides attaching to words, \rayr{ku/}{ku-} is furthermore able to subordinate
infinite\index{verbs!infinite} CPs\index{phrase types!complementizer phrase}. Since \rayr{ku/}{ku-} leans on a whole phrase in
...
\phantomsection\label{clitics_postverb_person}
...
pronominal affixes, they should behave exactly like clitics with respect to
...
While there is no defaulting\index{default} to a certain person\index{person} in the presence of
an overt subject\index{grammatical function!subject} NP\index{phrase types!noun phrase} as such in Ayeri, there is still the effect of complementary
...
that functionally, the inflection takes the role of the subject\index{grammatical function!subject} relation. This also explains why
...
pronominal NP\index{phrase types!noun phrase}.
% This is ungrammatical, since Ayeri basically supplants person\index{person}
% agreement\index{agreement} with a pronominal suffix in those cases, as we have seen.
If \rayr{/yaaNF}{-yāng} were a simple inflectional affix, one of the two
...
The behavior of pronominal person\index{person} marking on the verb is rather
complex, and decidedly unlike inflection. What looks like an affix on the verb
is also an argument of it, like a pronoun\index{pronouns}, as displayed in
(\ref{ex:clitics_42}). Complexity increases further in that such an incorporated
pronoun\index{pronouns} is also eligible for topicalization\index{grammatical
function!topic}. As shown above, topic marking on nouns\index{nouns} is realized
by suppressing the realization of the overt case\index{case} marker, whether it
is a proclitic or a suffix. The topic-marked\index{grammatical function!topic}
forms of pronouns\index{pronouns} are also underspecified for case\index{case},
and they happen to be the same as those of the
person-agreement\index{person}\index{agreement} suffixes, as exemplified by
\rayr{/y}{-ya} in (\ref{ex:clitics_40a}). Thus, a topic-marked\index{grammatical
function!topic} pronominal suffix on the verb looks exactly like ordinary
agreement\index{agreement} with a full NP\index{phrase types!noun phrase},
except that there is no full NP\index{phrase types!noun phrase} to agree
with---hence the subscript numbers in (\ref{ex:clitics_44}) and
(\ref{ex:clitics_45}) to distinguish between both kinds of
\rayr{/y}{-ya}.\footnote{It is premature to assume that the person\index{person}
suffix on the verb always co-indexes the topic\index{grammatical function!topic}
and a distinction between a person-agreement\index{person}\index{agreement}
suffix and a homophonous topicalized\index{grammatical function!topic}
pronominal suffix is thus unnecessary. While the topic\index{grammatical
function!topic} of a clause may consist of any NP\index{phrase types!noun
phrase} in a clause, the verb\index{verbs} generally does not agree with
non-agent or non-patient NPs\index{phrase types!noun phrase}.}
...
\phantomsection\label{clitics_quant}\index{quantifiers|(}
...
\citep{bittner1995}. According to \posscite{bittner1995} terminology,
...
quantifier suffixes into the verb. Ayeri---not a polysynthetic\index{synthesis}
...
\a\label{ex:clitics_53a}
	\ljudge{\ques\ques} \begin{minipage}[t]{\remaining}
		\fw{keynam}[\fw{-ikan soyang -kay}] \\
		`[few or many] people'
	\end{minipage}

\a\label{ex:clitics_53b}
	\ljudge{\ques} \begin{minipage}[t]{\remaining}
		[\fw{keynam$_i$-ikan}]\fw{ soyang }[\fw{\_$_i$-kay}]	\\
...
actually be the lexical head of a DP\index{phrase types!determiner phrase}, which creates the need for
...
in that they can have scope\index{scope} over a conjunct as a whole, although not
...
grammaticalization\index{grammaticalization} has stripped them of some of the
...
All examples in (\ref{ex:clitics_comp}) show that in principle, both
interpretations of \rayr{/ENF}{-eng}, as a quantifier and as a
comparative\index{comparison} suffix, are legitimate. Thus, distinguish between
...
\rayr{/ENF}{-eng} (\Comp{}) and \rayr{/vaa}{-vā} (\Supl{}) are
stressed\index{stress} \parencite[compare][90--92]{spencerluis2012}. With
\rayr{/ENF}{-eng} we can at least test whether it undergoes crasis if
appended to an adjective\index{adjectives} stem ending in \rayr{/E}{-e}. As we
...
host. The only adjective\index{adjectives} ending in \rayr{/E}{-e} currently
listed in the dictionary is \xayr{nke}{nake}{large, tall}. Example
(\ref{ex:clitics_nake}) shows what happens when \rayr{nke}{nake} is combined
with both kinds of \rayr{/ENF}{-eng}.
...
verb\index{verbs}, which manifests when the NP following\index{constituent order} the verb\index{verbs} (agent or patient) is not
...
that, in terms of morphology, the verb\index{verbs} may be analyzed as agreeing with one of the NPs\index{phrase types!noun phrase}
...
inflects for. In terms of syntax, however, the topicalized NP\index{phrase types!noun phrase} depends on the verb\index{verbs},
