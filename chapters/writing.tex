...
come with the redeeming point that they push off the surrounding text for the 
inclusion rather than encapsulating text within them, so the graphical effect 
is slightly different. The name brackets \ayr{[*]} are interesting and maybe 
useful insofar as many names in Ayeri are derived from common words, for 
...
above, is a large blank that would of course be invisible at the end of a line.
...
that the very common small word \rayr{nj}{nay} may be abbreviated as 
\rayr{\&}{\&}. Based on this, the reduplicated form \rayr{njnj}{naynay} that 
means `furthermore, also' may be abbreviated as \ayr{+}.
...
is definitely a difference. The `angular' style is also the one that is 
comparable in function to our bold face or italic style letters, since it is 
used to highlight special text, either within running text or, for example, in 
captions. This letter style (\xayr{hinY}{hinya}{angular}) is displayed in 
\autoref{fig:thangular}. 
...
As can be seen, many letter shapes become significantly simplified, especially 
\ayr{g}~\orth{ga}, \ayr{k}~\orth{ka}, \ayr{n}~\orth{na}, \ayr{N}~\orth{nga}, 
and the vowel \ayr{*i}~\orth{i}. The abbreviation \xayr{\&}{nay}{and} is used 
throughout. \ayr{n}~\orth{na} is taken from the `angular' style \ayr{\itshape 
n}, which opens the possibility that this is actually the basic shape rather 
than the `book' style's \ayr{n}.
...
experiment all the more interesting to due to it being decidedly 
non-`canon'. \autoref{fig:thblack} shows what the passage may have looked like 
interpreted by European scribes in the high to late middle ages, when Gothic 
book hands ruled.
