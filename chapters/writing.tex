...
\ayr{n}~\orth{na} changes into a simple descending line, the vowel carrier
\ayr{ʔ} to a flattened \textit{O}-like circle, and the bottom curl in
\ayr{t}~\orth{ta} becomes a wedge. While the right side of the
\ayr{s}~\orth{sa} character in the `book style' consists of two strokes---a
flag and a separate downwards bow---they connect here to form an \emph{R}-like
shape.
...
and birch bark from Inner Asia}. In this respect, there are many historically
attested media besides parchment and papyrus which support being inscribed with
styluses or ink pens.} Metal plates can be inscribed with metal styluses and
should allow similar shapes as modern pens. Wax tablets should as well allow
for relative freedom of stroke direction, so the character shapes are probably
not implausible even without assuming that pen and paper are (widely)
...
the the vowel length diacritic, \ayr{*aa}, which is simplified to a shape like
ɔ. The abbreviation \xayr{\&}{nay}{and} is used throughout, though in a shape
that is more similar to its `angular' form
\raisebox{-.75ex}{\includegraphics{images/hinya-nay-300dpi.png}}.
\ayr{n}~\orth{na} is also taken from the `angular' style
\raisebox{-.75ex}{\includegraphics{images/hinya-na-300dpi.png}}, which means
that it is possibly the acutal basic shape, rather than the `book' style's
\ayr{n}, or both are different developments from a shared ancestor.
...
\begin{figure}[ht]\centering
\includegraphics[width=0.75\linewidth]{images/tahanoblack-300dpi-bw-2.png}
