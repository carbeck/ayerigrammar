...
vowels, as does the homorganic nasal diacritic \ayr{*M} in this rather extreme 
example:

\pex[lingstyle=thex]\label{ex:caampuluy}\begingl
...
random or up to the aesthetic eye of the writer, but it is governed by rules. 
The prepended forms listed in \autoref{fig:thdiapreallo} are thus triggered 
...
\item when a large subscript diacritic would be added after another large 
	...
	from the beginning, as we saw in (\ref{ex:stemless}), and just 
	like in (\ref{ex:stacking}), this principle continues:
...
\item with consonants directly following na, to avoid a clash with its 
	descender:
	
	\pex[lingstyle=thex,everyglb=\itshape,numoffset=\leftmargin]
	\begingl
		\gla \ayr{n} $+$ \ayr{paa} → \ayr{npaa} //
		\glb na {} pā {} napā //
	\endgl\xe
	...
	\ayr{n}~\orth{na} may reduce its descender in size to accommodate the 
	following prepended diacritic:
	
	\pex[lingstyle=thex,everyglb=\itshape,numoffset=\leftmargin]
	\begingl
		\gla \ayr{n} $+$ \ayr{pj} → \ayr{npj} //
		\glb na {} pay {} napay //
	\endgl\xe
...
\item in other cases where a clash of subscript diacritics needs to be avoided:
	...
	\begingl
		...
		% commands, which is very clumsy. Past me: shame on you!
