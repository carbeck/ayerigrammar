...
\xayr{njnj}{naynay}{furthermore, also} may be abbreviated as \ayr{+}.
...
at some point. As mentioned above as well, there are no subscript letters and 
in Ayeri, and the number of large swirling diacritics is also rather low, so 
this is definitely a difference. The `angular' style is also the one that is 
comparable in function to our bold face or italic style letters, since it is 
used to highlight special text within running text or in captions. This letter 
style (\xayr{hinY}{hinya}{angular}) is displayed in \autoref{fig:thangular}. 
...
style by hand goes rather slowly, so I was wondering at some point what daily 
handwriting could look like. Of course, this is once again based on pen and 
paper, though \citet[377]{salomon1996} mentions that inscriptions of Brāhmī have 
been found in the shape of copper plates and plates made of other metals, 
besides stone. Metal plates can be inscribed with metal styluses and should 
allow similar shapes and dexterity as modern 
pens.
...
As can be seen, many letter shapes become significantly simplified, 
specifically \ayr{b}~\orth{ba}, \ayr{g}~\orth{ga}, \ayr{k}~\orth{ka}, 
\ayr{n}~\orth{na}, \ayr{N}~\orth{nga}, and the vowel \ayr{*i}~\orth{i}. The 
abbreviation \xayr{\&}{nay}{and} is used throughout. \ayr{n}~\orth{na} is taken 
from the `angular' style \ayr{\itshape n}, which opens the possibility that this 
is actually the basic shape rather than the `book' style's \ayr{n}.
...
This, of course, is very much a difference to Ayeri's usual look and feel. 
% both as a spoken and written language, which made the experiment all the more 
%interesting, though decidedly non-`canon'.
\autoref{fig:thblack} shows what the passage might have looked like at a time 
when Gothic book hands ruled.
