...
\ex\begingl
\glpreamble \textit{Ilta manga nimpongye Sipra edauyi.} \\
	/ˌilta ˌmaŋa nimˈpoŋje | ˈsipra | eˈdaui/ //

\gla Ilta manga nimpongye Sipra edauyi. //
\glb ilta manga nimp-ong-ye Sipra edauyi //
\glc be.supposed.to \Prog{} run-\Irr{}-\TsgF{} Sipra now //
\glft `Sipra should be running now.' //
\endgl\xe

\ex\begingl
\glpreamble Le rua ming sideja badanang ajam. \\
	/le rua miŋ siˈdeja | badaˈnaŋ | adʒam/ //

\gla Le rua ming sideja badanang ajam. //
\glb le rua ming sideg-ya badan-ang ajam //
\glc \PatTI{} must can repair-\TsgM{} father-\Aarg{} toy //
\glft `The toy, father must be able to repair (it).' //
